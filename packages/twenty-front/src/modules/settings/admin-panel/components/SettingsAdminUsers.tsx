...
const StyledCopyTextButton = styled.button<{ $muted?: boolean }>`
  ...
  border: none;
  color: ${({ theme, $muted }) =>
    $muted ? theme.font.color.tertiary : theme.font.color.primary};
  cursor: pointer;
  flex: 1 1 auto;
  font-family: inherit;
  font-size: ${({ theme, $muted }) =>
    $muted ? theme.font.size.xs : theme.font.size.sm};
  max-width: 100%;
  min-width: 0;
  overflow: hidden;
  padding: 0;
...
export const SettingsAdminUsers = () => {
  ...
  const handleCopy = useCallback(
    (value: string, label: string) => {
      ...
      enqueueSnackBar(t`${label} copied to clipboard`, {
        variant: SnackBarVariant.Success,
        icon: <IconCopy size={theme.icon.size.md} />,
        duration: 2000,
      });
    },
    [enqueueSnackBar, t, theme.icon.size.md],
  );
  ...
  const handleConnectUnipile = async (
    ...
    try {
      ...
      if (!result) {
        enqueueSnackBar(t`No response from Unipile connect`, {
          variant: SnackBarVariant.Error,
        });
        return;
      }
