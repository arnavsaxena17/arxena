...
export const SettingsAdminOrgChartClientIps = () => {
  ...
  >(UPSERT_ORG_CHART_CLIENT_IP_RULE, {
    client: apolloAdminClient,
    onError: (e) => {
      enqueueErrorSnackBar({ message: e.message });
    },
  });
  ...
  >(DELETE_ORG_CHART_CLIENT_IP_RULE, {
    client: apolloAdminClient,
    onCompleted: () => {
      enqueueSuccessSnackBar({ message: 'Rule removed' });
      void refetch();
    },
    onError: (e) => {
      enqueueErrorSnackBar({ message: e.message });
  ...
  >(RESET_ORG_CHART_CLIENT_IP_RULE_COUNTERS, {
    client: apolloAdminClient,
    onCompleted: () => {
      enqueueSuccessSnackBar({ message: 'Counters reset' });
      void refetch();
    },
    onError: (e) => {
      enqueueErrorSnackBar({ message: e.message });
  ...
  const handleAdd = async () => {
    ...
    if (trimmed.length === 0) {
      enqueueErrorSnackBar({ message: 'Enter an IP address or CIDR' });
      return;
    }
    ...
    if (isDefined(res.data?.upsertOrgChartClientIpRule)) {
      enqueueSuccessSnackBar({ message: 'Rule saved' });
      setIpInput('');
      await refetch();
    }
  };
