...
| Layer | Role |
| --- | --- |
| **Ephemeral companies** | Redis cache keyed by `projectId` (`GET/PUT /outreach-command/cache/companies`) — Ask AI / discovery working set |
| **Ephemeral people** | Redis cache keyed by `projectId` (`GET/PUT /outreach-command/cache/people`) via `upsert_outreach_target_people` — Ask AI search hits until user confirms CRM |
...
On entry, Ask AI prefills an ICP onboarding kickoff (user hits Enter to send). Chrome is two rows: **PageHeader** (`Outreach` + project picker / New project / CRM / Menu) and **main tabs** (Companies / People / Workflow; workflow mode + outreach picker trail on the same row). Switch projects via the header Project picker (**New project** creates a Project). Workflow tab prefers / auto-creates **`Outreach — Per Enrolled Candidate`** and binds `Project.outreachWorkflowId` when the Project has none. The Stage B dropdown lists ACTIVE workflows only; selecting one rebinds the Project pin and prefills Ask AI with that `outreachWorkflowId` context.
