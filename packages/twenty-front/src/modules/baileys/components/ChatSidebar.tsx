...
import { Jobs, PersonNode, UnreadMessageListManyCandidates } from "twenty-shared";
    ...
    <FixedHeader>
      ...
      <DropdownContainer ref={jobDropdownRef}>
          ...
          <DropdownContent isOpen={isJobDropdownOpen}>
              {jobs.map((job) => (
                <CheckboxLabel key={job.id}>
                  <Checkbox type="checkbox" checked={selectedJobs.includes(job.id)} onChange={() => handleJobToggle(job.id)} /> {job.name} ({jobCounts[job.id] || 0})
                </CheckboxLabel>
              ))}
            </DropdownContent>
        </DropdownContainer>
