...
export const persistSearchResultsCache = async (
  ...
  if (!payload.projectId || payload.projectId === 'project-id' || !accessToken)
    return;
  const baseUrl = getBaseUrl();
  if (!baseUrl) return;
  try {
 ...
 * Stores transformed candidate objects which extend PersonCandidateDraft from the backend
 ...
 */
export const searchResultsState = createAtomState<CandidateTableRow[]>({
...
export const addSearchResults =
  (setSearchResults: any, projectId?: string) =>
  (
    newResults: any[],
    onComplete?: (result: { added: number; duplicates: number }) => void,
    sortByScore?: boolean,
  ) => {
    ...
    setSearchResults((prev: any[]) => {
      console.log(
        '=== addSearchResults - inside setSearchResults callback ===',
        {
      ...
      if (!currentProjectId || currentProjectId === 'project-id') {
        console.error(
          '=== addSearchResults - projectId became invalid during execution, aborting ===',
          {
            currentProjectId,
            prevCount: prev.length,
            newResultsCount: newResults.length,
      ...
      persistSearchResultsToStorage(updatedResults, currentProjectId).catch(
        (error) => {
          console.error(
            'Failed to persist search results (non-blocking):',
            error,
          );
        },
      );
...
export const persistSearchMetadataToStorage = (
  ...
  fetchSearchResultsCache(projectId, options.accessToken)
    .then((cached) => {
      const results = cached?.results ?? [];
...
export const removeSavedFromSearchResults =
  (setSearchResults: any, projectId?: string) => (savedCandidates: any[]) => {
    setSearchResults((prev: any[]) => {
      // Extract unique string keys from saved candidates
      const savedUniqueKeys = new Set(
        savedCandidates
          .map((candidate) => candidate.people?.uniqueStringKey)
          .filter(Boolean),
      );
      ...
      console.log(
        `Removed ${prev.length - filteredResults.length} saved candidates from search results`,
      );

      // Update localStorage with projectId
      // Fire and forget - don't block state update on storage operation
      persistSearchResultsToStorage(filteredResults, projectId).catch(
        (error) => {
          console.error(
            'Failed to persist search results after removal (non-blocking):',
            error,
          );
        },
      );
...
export const handleUploadProfilesSuccess =
  (setSearchResults: any, setSearchMetadata: any, projectId?: string) =>
  (response: any) => {
    if (response.status === 'ok' && response.savedCandidates) {
      console.log(
        `Successfully saved ${response.savedCandidates.length} candidates`,
      );

      // Remove saved candidates from search results
      removeSavedFromSearchResults(
        setSearchResults,
        projectId,
      ...
      setSearchMetadata((prev: any) => ({
        ...
        totalCount: Math.max(
          0,
          prev.totalCount - response.savedCandidates.length,
        ),
      }));

      console.log(
        'Updated search results and metadata after successful upload',
      );
    }
  };
