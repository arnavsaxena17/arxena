...
import { useLeaveTableFocus } from '@/object-record/job-table/hooks/internal/useLeaveTableFocus';
import { useMoveEditModeToTableCellPosition } from '@/object-record/job-table/hooks/internal/useMoveEditModeToCellPosition';
...
export const useOpenRecordTableCellV2 = (tableScopeId: string) => {
  ...
  const openTableCell = useRecoilCallback(
    ({ snapshot }) =>
      ({ initialValue, cellPosition, isReadOnly, pathToShowPage, objectNameSingular, customCellHotkeyScope, fieldDefinition, entityId, isActionButtonClick }: OpenTableCellArgs) => {
        if (isReadOnly) {
          return;
        }
    ...
    [setDragSelectionStartEnabled, toggleClickOutsideListener, leaveTableFocus, setHotkeyScope, initDraftValue, moveEditModeToTableCellPosition, openRightDrawer, setViewableRecordId, setViewableRecordNameSingular, navigate],
  );
