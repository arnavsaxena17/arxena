...
export const useMoveSoftFocusToCellOnHoverV2 = (recordTableId: string) => {
  ...
  const { currentTableCellInEditModePositionState, isTableCellInEditModeFamilyState } = useRecordTableStates(recordTableId);
  ...
  const moveSoftFocusToCell = useRecoilCallback(
    ...
    [currentTableCellInEditModePositionState, isTableCellInEditModeFamilyState, setSoftFocus],
  );
