...
import { useCloseRecordTableCell } from '@/object-record/job-table/record-table-cell/hooks/useCloseRecordTableCell';
    ...
    <RecordTableScope recordTableScopeId={scopeId} onColumnsChange={onColumnsChange}>
      <FieldContext.Provider
        ...
        <RecordTableRowContext.Provider value={recordTableRow}>
          <RecordTableCellContext.Provider value={{ ...recordTableCell, columnIndex: 0 }}>{children}</RecordTableCellContext.Provider>
  ...
  it('should work as expected', async () => {
    const { result } = renderHook(
      () => {
        ...
        const currentTableCellInEditModePosition = useRecoilValue(currentTableCellInEditModePositionState);
        const isTableCellInEditMode = useRecoilValue(isTableCellInEditModeFamilyState(currentTableCellInEditModePosition));
        return {
