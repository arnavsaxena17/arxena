...
export const useOpenRecordTableCell = () => {
  ...
  const { initDraftValue: initFieldInputDraftValue } = useRecordFieldInput(`${entityId}-${fieldDefinition?.metadata?.fieldName}`);
  ...
  const openTableCell = useRecoilCallback(
    ...
    [isReadOnly, isFirstColumnCell, isEmpty, setDragSelectionStartEnabled, setCurrentTableCellInEditMode, initFieldInputDraftValue, toggleClickOutsideListener, customCellHotkeyScope, leaveTableFocus, navigate, pathToShowPage, setHotkeyScope],
  );
