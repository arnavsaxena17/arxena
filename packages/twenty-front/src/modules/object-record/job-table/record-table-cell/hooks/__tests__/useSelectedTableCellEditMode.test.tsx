...
describe('useSelectedTableCellEditMode', () => {
  it('should have property setSelectedTableCellEditMode', async () => {
    ...
    const { result } = renderHook(() => useSelectedTableCellEditMode({ scopeId }), {
      wrapper: RecoilRoot,
    });

    act(() => {
      result.current.setSelectedTableCellEditMode(tableCellPosition.row, tableCellPosition.column);
    });
