...
import { useMoveSoftFocusToCurrentCellOnHover } from '@/object-record/job-table/record-table-cell/hooks/useMoveSoftFocusToCurrentCellOnHover';
...
const mockCallbackInterface = {
  ...
  snapshot: {
    getLoadable: (recoilValue: { key: string }) => ({
      getValue: () => {
        if (recoilValue.key === mockSoftFocusPositionState.key) return { column: 1, row: 0 };
        if (recoilValue.key === mockCurrentTableCellInEditModePositionState.key) return { column: 1, row: 0 };
        else if (recoilValue.key === mockIsTableCellInEditModeFamilyState.key) return false;
        else if (recoilValue.key === mockCurrentHotKeyScopeState.key) return { scope: TableHotkeyScope.Table };
      },
    }),
  },
...
jest.mock('recoil', () => ({
  ...
  useRecoilCallback: (callback: (_interface: CallbackInterface) => (newPosition: TableCellPosition) => void) => callback(mockCallbackInterface),
}));
jest.mock('@/object-record/job-table/hooks/internal/useRecordTableStates', () => ({
  useRecordTableStates: () => ({
    softFocusPositionState: mockSoftFocusPositionState,
    isSoftFocusActiveState: mockSoftFocusActiveState,
    isSoftFocusOnTableCellFamilyState: () => mockIsSoftFocusOnTableCellFamilyState,
    currentTableCellInEditModePositionState: mockCurrentTableCellInEditModePositionState,
    isTableCellInEditModeFamilyState: () => mockIsTableCellInEditModeFamilyState,
  }),
}));
jest.mock('@/object-record/job-table/hooks/useRecordTable', () => ({
  useRecordTable: () => ({
...
describe('useMoveSoftFocusToCurrentCellOnHover', () => {
  it('should work as expected', () => {
    const { result } = renderHook(
      () => {
        return {
          moveSoftFocusToCurrentCellOnHover: useMoveSoftFocusToCurrentCellOnHover(),
        };
      },
      { wrapper: Wrapper },
    );
    ...
    expect(mockCallbackInterface.set).toHaveBeenCalledWith(mockSoftFocusActiveState, true);
    expect(mockCallbackInterface.set).toHaveBeenCalledWith(mockIsSoftFocusOnTableCellFamilyState, false);
    expect(mockCallbackInterface.set).toHaveBeenCalledWith(mockSoftFocusPositionState, { column: 3, row: 2 });
    expect(mockCallbackInterface.set).toHaveBeenCalledWith(mockIsSoftFocusOnTableCellFamilyState, true);
    expect(mockCallbackInterface.set).toHaveBeenCalledWith(mockSoftFocusActiveState, true);
    expect(setHotkeyScope).toHaveBeenCalledWith(TableHotkeyScope.TableSoftFocus);
  });
});
