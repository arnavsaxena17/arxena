...
import { isRemoveSortingModalOpenState } from '@/object-record/job-table/states/isRemoveSortingModalOpenState';
import { ConfirmationModal } from '@/ui/layout/modal/components/ConfirmationModal';
import { useCombinedViewSorts } from '@/views/hooks/useCombinedViewSorts';
import { useGetCurrentView } from '@/views/hooks/useGetCurrentView';

export const RemoveSortingModal = ({ recordTableId }: { recordTableId: string }) => {
  const { currentViewWithCombinedFiltersAndSorts } = useGetCurrentView(recordTableId);

  const viewSorts = currentViewWithCombinedFiltersAndSorts?.viewSorts || [];
  const fieldMetadataIds = viewSorts.map(viewSort => viewSort.fieldMetadataId);
  const isRemoveSortingModalOpen = useRecoilState(isRemoveSortingModalOpenState);
