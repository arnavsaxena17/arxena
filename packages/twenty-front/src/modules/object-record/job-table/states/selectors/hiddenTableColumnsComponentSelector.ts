...
export const hiddenTableColumnsComponentSelector = createComponentReadOnlySelector({
  key: 'hiddenTableColumnsComponentSelector',
  get:
    ({ scopeId }) =>
    ({ get }) => {
      const tableColumns = get(tableColumnsComponentState({ scopeId }));
      const availableColumns = get(availableTableColumnsComponentState({ scopeId }));
      const tableColumnsByKey = mapArrayToObject(tableColumns, ({ fieldMetadataId }) => fieldMetadataId);

      const hiddenColumns = availableColumns
        .filter(({ fieldMetadataId }) => !tableColumnsByKey[fieldMetadataId]?.isVisible)
        .map(availableColumn => {
