...
export const useRecordTableMoveFocus = (recordTableId?: string) => {
  const { scopeId, softFocusPositionState, numberOfTableRowsState, numberOfTableColumnsSelector, selectedRowIdsSelector } = useRecordTableStates(recordTableId);
  ...
  const moveDown = useRecoilCallback(
    ({ snapshot }) =>
      () => {
        ...
        const numberOfTableRows = getSnapshotValue(snapshot, numberOfTableRowsState);
  ...
  const moveRight = useRecoilCallback(
    ({ snapshot }) =>
      () => {
        ...
        const isLastRowAndLastColumn = currentColumnNumber === numberOfTableColumns - 1 && currentRowNumber === numberOfTableRows - 1;

        const isLastColumnButNotLastRow = currentColumnNumber === numberOfTableColumns - 1 && currentRowNumber !== numberOfTableRows - 1;

        const isNotLastColumn = currentColumnNumber !== numberOfTableColumns - 1;
    ...
    [softFocusPositionState, numberOfTableColumnsSelector, numberOfTableRowsState, setSoftFocusPosition],
  );
  ...
  const moveLeft = useRecoilCallback(
    ({ snapshot }) =>
      () => {
        ...
        const isFirstRowAndFirstColumn = currentColumnNumber === 0 && currentRowNumber === 0;

        const isFirstColumnButNotFirstRow = currentColumnNumber === 0 && currentRowNumber > 0;
    ...
    [numberOfTableColumnsSelector, softFocusPositionState, setSoftFocusPosition],
  );
