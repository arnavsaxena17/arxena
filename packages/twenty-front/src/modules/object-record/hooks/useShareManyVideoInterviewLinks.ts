    ...
    try {
      const results = await Promise.all(
        candidateIds.map((candidateId) =>
          axios.post(
            `${REACT_APP_SERVER_BASE_URL}/video-interview-process/send-video-interview-to-candidate`,
            { candidateId },
            {
              headers: {
      ...
      if (successfulCandidates.length === candidateIds.length) {
        enqueueSuccessSnackBar({
          message: `Successfully shared video interview ${candidateIds.length === 1 ? 'link' : 'links'} for ${candidateIds.length} candidate${candidateIds.length === 1 ? '' : 's'}`,
