    ...
    try {
      ...
      if (data) {
        ...
        const consolidatedErrorMessage = [
          ...
          // Recruiter profile
          !data?.jobs?.edges?.[0]?.node?.recruiter?.workspaceMemberProfile?.edges?.[0]?.node?.name && 'Recruiter name is missing in workspace member profiles',
          !data?.jobs?.edges?.[0]?.node?.recruiter?.workspaceMemberProfile?.edges?.[0]?.node?.phoneNumber && 'Recruiter phone number is missing in workspace member profiles',
          !data?.jobs?.edges?.[0]?.node?.recruiter?.workspaceMemberProfile?.edges?.[0]?.node?.companyDescription && 'Recruiter company description is missing in workspace member profiles',
          !data?.jobs?.edges?.[0]?.node?.recruiter?.workspaceMemberProfile?.edges?.[0]?.node?.jobTitle && 'Recruiter job title is missing in workspace member profiles',
