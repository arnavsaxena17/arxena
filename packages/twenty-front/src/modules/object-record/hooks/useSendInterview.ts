...
//   const showNotification = useShowNotification();
  ...
  const sendVideoInterviewLink = async (candidateIds: string[]) => {
    setLoading(true);
    setError(null);
    ...
    const successfulCandidates = results.filter(
      ...
      if (successfulCandidates.length === candidateIds.length) {
        ...
        throw new Error(
          `Failed to send Video Interview Links for ${failedCount} candidate${failedCount === 1 ? '' : 's'}`
        );
      }
