...
import { type WorkflowSendLinkedinVoiceNoteAction } from '@/workflow/types/Workflow';
...
type WorkflowEditActionSendLinkedinVoiceNoteProps = {
  action: WorkflowSendLinkedinVoiceNoteAction;
  actionOptions:
    | {
        readonly: true;
      }
    | {
        readonly?: false;
        onActionUpdate: (action: WorkflowSendLinkedinVoiceNoteAction) => void;
      };
};

export const WorkflowEditActionSendLinkedinVoiceNote = ({
  action,
  actionOptions,
}: WorkflowEditActionSendLinkedinVoiceNoteProps) => {
  ...
  return (
    <>
      <WorkflowStepBody>
        ...
        <FormTextFieldInput
          label={t`Caption`}
          placeholder={t`Optional. Leave empty for voice-only`}
          multiline
          readonly={actionOptions.readonly}
        ...
        <WorkflowSendEmailAttachments
          label={t`Voice file (.m4a preferred)`}
