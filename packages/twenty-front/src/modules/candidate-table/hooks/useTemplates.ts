...
export const TEMPLATE_DISPLAY_NAMES: Record<SupportedTemplateName, string> = {
  ...
  share_video_interview_link_with_start_link:
    'Video Interview Link with Button',
...
export const useTemplates = () => {
  const templates = useAtomStateValue(templatesState);
  ...
  return {
    ...
    templatePreviews: Object.fromEntries(
      Object.entries(templates.templatePreviews).filter(([key]) =>
        SUPPORTED_TEMPLATES.includes(key as SupportedTemplateName),
      ),
    ),
    ...
    isValidTemplate: (
      templateName: string,
    ): templateName is SupportedTemplateName =>
