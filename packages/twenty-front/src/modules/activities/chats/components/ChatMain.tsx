...
export default function ChatMain({ initialCandidateId }: ChatMainProps) {
  ...
  const fetchData = async () => {
    try {
      ...
      const availablePeople: frontChatTypes.PersonNode[] = peopleResponse.data.filter(
  ...
  const updateUnreadMessagesStatus = async (selectedIndividual: string) => {
    ...
    const response = await axios.post(process.env.REACT_APP_SERVER_BASE_URL + '/arx-chat/update-whatsapp-delivery-status', { listOfMessagesIds: listOfMessagesIds }, { headers: { Authorization: `Bearer ${tokenPair?.accessToken?.token}` } });
  };
  ...
  return (
    <ChatContainer>
      ...
      <ChatWindowContainer>
      <ChatWindow selectedIndividual={selectedIndividual} individuals={individuals} onMessageSent={fetchData}/>
      </ChatWindowContainer>
    </ChatContainer>
  );
}
