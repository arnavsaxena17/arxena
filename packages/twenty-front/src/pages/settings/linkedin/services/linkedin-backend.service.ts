...
export class LinkedinBackendService {
  ...
  constructor(baseUrl?: string) {
    if (baseUrl) {
      ...
      console.log(
        'LinkedIn Backend Service initialized with baseUrl:',
        this.baseUrl,
      );
      console.log('REACT_APP_SERVER_BASE_URL:', REACT_APP_SERVER_BASE_URL);
    }
  }
  ...
  async connectWithCredentials(
    ...
    accessToken?: string,
  ): Promise<LinkedinSignupResponse> {
    try {
  ...
  async createHostedAuthLink(
    ...
    console.log(
      'typeof options.success_redirect_url:',
      typeof options.success_redirect_url,
    );
    console.log('options.success_redirect_url:', options.success_redirect_url);
    console.log(
      'typeof options.failure_redirect_url:',
      typeof options.failure_redirect_url,
    );
    ...
    console.log(
      'typeof options.reconnect_account:',
      typeof options.reconnect_account,
    );
    ...
    if (response && typeof response.hosted_link === 'string') {
      response.hosted_link = response.hosted_link.replace(
        'account.unipile.com',
        'auth.arxena.com',
      );
    }
  ...
  async solveCheckpoint(
    checkpointData: LinkedinCheckpointData,
    accessToken?: string,
  ): Promise<LinkedinSignupResponse> {
    try {
      const response = await this.makeRequest<any>(
  ...
  async getOwnProfile(
    ...
    const response = await this.makeRequest<{ profile: LinkedinProfileData }>(
  ...
  async reconnectFromStoredProfile(
    ...
    const clientGeo = await getOrFetchClientGeoSession();
    const baseBody =
      options?.user_agent?.trim() !== undefined &&
      options.user_agent.trim() !== ''
  ...
  async sendMessage(
    ...
    const response = await this.makeRequest<{ chat: any }>(
      '/message/send',
      'POST',
      messageData,
      accessToken,
  ...
  async getAccountRateLimits(
    ...
    reserved: Partial<Record<keyof LinkedinAccountRateLimits, number>>;
    nextSlotAt: Partial<Record<keyof LinkedinAccountRateLimits, string | null>>;
  }> {
    const response = await this.makeRequest<{
      limits: LinkedinAccountRateLimits;
      usage?: Partial<Record<keyof LinkedinAccountRateLimits, number>>;
      reserved?: Partial<Record<keyof LinkedinAccountRateLimits, number>>;
      nextSlotAt?: Partial<
        Record<keyof LinkedinAccountRateLimits, string | null>
      >;
...
export const getLinkedinService = (
  baseUrl?: string,
): LinkedinBackendService => {
