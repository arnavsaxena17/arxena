 ...
 *   OUTREACH_PROJECT_ID (preferred) — active Project UUID
 *   OUTREACH_SIMULATE_MODE = accept | ignore | reply | bootstrap | full (default full)
 */
...
const MODE = (process.env.OUTREACH_SIMULATE_MODE || 'full') as
  | 'accept'
  | 'ignore'
  | 'reply'
  | 'bootstrap'
...
const findProjects = async (projectId?: string) => {
  ...
  const data = await graphqlRequest<{
    ...
    `query FindOutreachProjects($filter: ProjectFilterInput) {
...
const findCandidates = async (projectId: string) => {
  const data = await graphqlRequest<{
    ...
    `query FindOutreachCandidates($filter: CandidateFilterInput!) {
...
type ResolvedOutreachProject = {
  id: string;
  name?: string;
  outreachWorkflowId?: string | null;
};

const resolveOutreachProject = async (): Promise<ResolvedOutreachProject> => {
  const projects = await findProjects(OUTREACH_PROJECT_ID || undefined);
  const project =
    projects.find((row) => row.id === OUTREACH_PROJECT_ID) ??
  ...
  if (!project) {
    throw new Error(
      OUTREACH_PROJECT_ID
        ? `No Project id=${OUTREACH_PROJECT_ID}. Open /outreach-home and create a project, or seed dashboard.`
        : 'Pass OUTREACH_PROJECT_ID. Seed dashboard first.',
    );
  }
...
const main = async () => {
  const project = await resolveOutreachProject();

  console.log(
    `Simulating GTM outreach projectId=${project.id} mode=${MODE}`,
  );
  console.log('Compressed delay env OUTREACH_DELAY_MS=', process.env.OUTREACH_DELAY_MS);
  ...
  if (candidates.length === 0) {
    throw new Error(
      `No candidates for projectId=${project.id}. Seed dashboard or enroll from /outreach-home?projectId=${project.id}.`,
    );
  }
  ...
  console.log(`Open /outreach-home?projectId=${project.id}`);
};
