...
// Seeds sample CRM rows for the native "Outreach" dashboard charts.
...
const DASHBOARD_TITLE = 'Outreach';
...
const OUTREACH_SEED_PROJECT_NAME = 'GTM Project · seed';
...
const OUTREACH_COMPANIES: CompanySeed[] = [
  {
    key: 'co-1',
    name: 'Northwind Labs',
  ...
  },
  {
    ...
    segment: 'Series B SaaS',
    icpFit: 'HIGH',
    outreachFunnelStage: 'MEETING_HELD',
    peopleTargeted: 3,
    peopleReached: 2,
    coverageBucket: 'ONE_TWO',
...
const OUTREACH_PEOPLE: PersonSeed[] = [
  {
    firstName: 'Maya',
    lastName: 'Chen',
    title: 'VP People',
  ...
  },
  {
    ...
    enrichStatus: 'FOUND',
    messagingChannel: MessagingChannel.LINKEDIN_INMAIL,
...
const COMPANY_FIELDS_TO_ENSURE: FieldToCreate[] = [
  ...
  { name: 'projectIds', label: 'GTM Project Key', type: 'ARRAY', icon: 'IconKey' },
  {
    name: 'peopleTargeted',
...
const OPPORTUNITY_FIELDS_TO_ENSURE: FieldToCreate[] = [
  ...
  //   defaultValue: false,
  // },
  // { name: 'projectIds', label: 'GTM Project Key', type: 'TEXT', icon: 'IconKey' },
];
...
const buildCompanyPayload = (
  ...
  const payload: Record<string, unknown> = {
    ...
    outreachFunnelStage: company.outreachFunnelStage,
...
const seedCompanies = async (
  ...
  for (const company of OUTREACH_COMPANIES) {
...
const seedPeople = async (
  ...
  for (const person of OUTREACH_PEOPLE) {
...
const seedProject = async (companyIdsByKey: Record<string, string>) => {
  const projectName = OUTREACH_SEED_PROJECT_NAME;
...
const seedCandidates = async ({
  ...
  for (const person of OUTREACH_PEOPLE) {
    ...
    const company = OUTREACH_COMPANIES.find(
  ...
  const tabs = [
    {
      ...
      widgets: [
        aggregateWidget({
          ...
          filter: {
            recordFilters: [
              {
                id: randomUUID(),
                fieldMetadataId: companyFields.projectIds,
                operand: 'CONTAINS',
                value: projectId,
              },
            ],
        ...
        aggregateWidget({
          ...
          filter: {
            recordFilters: [
              {
                id: randomUUID(),
                fieldMetadataId: companyFields.outreachFunnelStage,
                operand: 'IS',
        ...
        aggregateWidget({
          ...
          filter: {
            recordFilters: [
              {
                id: randomUUID(),
                fieldMetadataId: opportunityFields.sourcedFromOutreach,
                operand: 'IS',
                value: 'true',
              },
            ],
        ...
        barWidget({
          ...
          groupByFieldId: companyFields.outreachFunnelStage,
...
const main = async () => {
  ...
  const projectId = OUTREACH_PROJECT_ID || (await seedProject(companyIdsByKey));

  console.log('Tagging companies with Project.id as projectIds...');
  for (const companyId of Object.values(companyIdsByKey)) {
    await requestWithRetry(
      ...
      { id: companyId, data: { projectIds: projectId } },
    );
  }
  ...
  const attentionViewId = await createTableWidgetView({
    ...
    fieldIds: [
      companyFields.name,
      companyFields.outreachFunnelStage,
  ...
  const opportunitiesViewId = await createTableWidgetView({
    ...
    fieldIds: [
      ...
      opportunityFields.sourcedFromOutreach,
