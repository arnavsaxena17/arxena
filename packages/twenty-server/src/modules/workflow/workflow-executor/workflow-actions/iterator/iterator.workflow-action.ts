...
import { isDefined, StepStatus, WorkflowRunStepInfo } from 'twenty-shared';
...
export class IteratorWorkflowAction implements WorkflowActionInterface {
  ...
  async execute(input: WorkflowActionInput): Promise<WorkflowActionOutput> {
    ...
    const existingIteratorStepResult = stepInfos[iteratorStepId]
      ?.result as WorkflowIteratorResult | undefined;
  ...
  private async resetStepsInLoop({
    ...
    if (!hasProcessedAllItems) {
      const subStepsInfos = this.buildSubStepInfosReset({
        iteratorStepId,
        initialLoopStepIds,
        stepInfos,
        steps,
    ...
    const iteratorStepInfo = this.buildIteratorStepInfoReset({
      iteratorStepId,
  ...
  private buildSubStepInfosReset({
    iteratorStepId,
    initialLoopStepIds,
    stepInfos,
    steps,
