...
@RegisteredWorkspaceCommand('2.25.0', 1785600000063)
@Command({
  name: 'upgrade:2-25:sync-gtm-search-people-search-url',
  description:
    'Re-seed Search people so a pasted LinkedIn search URL (searchUrl) is an input',
})
export class SyncOutreachSearchPeopleSearchUrlCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing search-people searchUrl input for workspace ${workspaceId}`,
    );
