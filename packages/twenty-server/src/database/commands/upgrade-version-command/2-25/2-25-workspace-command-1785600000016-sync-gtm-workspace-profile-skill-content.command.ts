...
@RegisteredWorkspaceCommand('2.25.0', 1785600000016)
@Command({
  name: 'upgrade:2-25:sync-outreach-workspace-profile-skill-content',
  description:
    'Sync outreach-icp-onboarding skill content (workspace profile ICP + search blurbs) for existing workspaces',
})
export class SyncOutreachWorkspaceProfileSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing GTM workspace profile skill content for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `GTM workspace profile skill already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync GTM workspace profile skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync GTM workspace profile skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced GTM workspace profile skill content for workspace ${workspaceId}`,
    );
  }
}
