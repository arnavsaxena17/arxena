...
@RegisteredWorkspaceCommand('2.25.0', 1785600000048)
@Command({
  name: 'upgrade:2-25:ensure-gtm-detect-fake-profiles-logic-function',
  description:
    'Seed native GTM logic function detect-fake-profiles for existing workspaces',
})
export class EnsureOutreachDetectFakeProfilesLogicFunctionCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Ensuring GTM detect-fake-profiles logic function for workspace ${workspaceId}`,
    );
