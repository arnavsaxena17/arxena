...
@RegisteredWorkspaceCommand('2.25.0', 1785600000030)
@Command({
  name: 'upgrade:2-25:sync-gtm-outreach-workflows-skill-content',
  description:
...
export class SyncGtmOutreachWorkflowsSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing gtm-outreach-workflows skill content for workspace ${workspaceId}`,
    );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync gtm-outreach-workflows skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync gtm-outreach-workflows skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced gtm-outreach-workflows skill content for workspace ${workspaceId}`,
    );
  }
}
