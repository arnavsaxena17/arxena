...
@RegisteredWorkspaceCommand('2.25.0', 1785600000096)
@Command({
  name: 'upgrade:2-25:sync-outreach-sales-reply-closer',
  description:
    'Refresh GTM inbound sales reply agent, REPLIED graph (transcript, times, skip-send), and outreach skill',
})
export class SyncOutreachSalesReplyCloserCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing outreach sales reply closer for workspace ${workspaceId}`,
    );
    ...
    this.logger.log(
      `Outreach sales reply closer sync complete for workspace ${workspaceId}`,
    );
  }
