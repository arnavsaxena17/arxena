...
@RegisteredWorkspaceCommand('2.25.0', 1785600000103)
@Command({
  name: 'upgrade:2-25:ensure-gtm-visit-linkedin-profile-logic-function',
  description:
    'Seed native GTM logic function visit-linkedin-profile for existing workspaces',
})
export class EnsureOutreachVisitLinkedinProfileLogicFunctionCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Ensuring GTM visit-linkedin-profile logic function for workspace ${workspaceId}`,
    );
