...
@RegisteredWorkspaceCommand('2.25.0', 1785600000060)
@Command({
  name: 'upgrade:2-25:sync-gtm-search-people-hide-data-source-account-id',
  description:
    'Re-seed Search people so Data source and Account ID are hidden; Unipile is resolved from the workspace member LinkedIn profile, otherwise Harvest',
})
export class SyncOutreachSearchPeopleHideDataSourceAccountIdCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing search-people hidden data source/account ID for workspace ${workspaceId}`,
    );
