...
export class SyncIcpScopedRegenerateSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing GTM ICP scoped-regenerate skill content for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `GTM ICP scoped-regenerate skill already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync GTM ICP scoped-regenerate skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync GTM ICP scoped-regenerate skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced GTM ICP scoped-regenerate skill content for workspace ${workspaceId}`,
    );
  }
}
