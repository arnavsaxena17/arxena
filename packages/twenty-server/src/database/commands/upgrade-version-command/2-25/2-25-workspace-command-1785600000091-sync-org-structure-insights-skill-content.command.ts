...
@RegisteredWorkspaceCommand('2.25.0', 1785600000091)
@Command({
  name: 'upgrade:2-25:sync-org-structure-insights-skill-content',
  description:
    'Create or sync org-structure-insights Ask AI skill content for existing workspaces',
})
export class SyncOrgStructureInsightsSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing org-structure-insights skill for workspace ${workspaceId}`,
    );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync org-structure-insights skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync org-structure-insights skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced org-structure-insights skill content for workspace ${workspaceId}`,
    );
  }
}
