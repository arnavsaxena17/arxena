...
@RegisteredWorkspaceCommand('2.25.0', 1785600000095)
@Command({
  name: 'upgrade:2-25:ensure-gtm-fetch-user-comments-logic-function',
  description:
    'Seed native GTM logic function fetch-user-comments for existing workspaces',
})
export class EnsureOutreachFetchUserCommentsLogicFunctionCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Ensuring GTM fetch-user-comments logic function for workspace ${workspaceId}`,
    );
