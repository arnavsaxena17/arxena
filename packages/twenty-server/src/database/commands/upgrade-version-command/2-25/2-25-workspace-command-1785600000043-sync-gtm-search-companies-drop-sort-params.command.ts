...
@RegisteredWorkspaceCommand('2.25.0', 1785600000043)
@Command({
  name: 'upgrade:2-25:sync-gtm-search-companies-drop-sort-params',
  description:
    'Re-seed native GTM logic functions so Search companies no longer exposes sortBy/sortOrder inputs',
})
export class SyncOutreachSearchCompaniesDropSortParamsCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Dropping search-companies sort params for workspace ${workspaceId}`,
    );
