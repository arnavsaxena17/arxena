...
@RegisteredWorkspaceCommand('2.25.0', 1785600000026)
@Command({
  name: 'upgrade:2-25:ensure-gtm-fetch-logic-functions',
  description:
    'Seed missing native GTM logic functions (fetch-linkedin-messages, fetch-company-details) for existing workspaces',
})
export class EnsureOutreachFetchLogicFunctionsCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Ensuring GTM fetch logic functions for workspace ${workspaceId}`,
    );
