...
@Command({
  name: 'upgrade:2-25:resync-gtm-outreach-whatsapp-form-details',
  description:
...
export class ResyncGtmOutreachWhatsappFormDetailsCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Resyncing GTM outreach WhatsApp form details for workspace ${workspaceId}`,
    );
    ...
    this.logger.log(
      `Resync GTM outreach WhatsApp form details complete for workspace ${workspaceId}`,
    );
  }
}
