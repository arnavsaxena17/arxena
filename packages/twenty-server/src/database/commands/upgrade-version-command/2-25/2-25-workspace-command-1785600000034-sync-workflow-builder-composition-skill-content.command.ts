...
@Command({
  name: 'upgrade:2-25:sync-workflow-builder-composition-skill-content',
  description:
    'Sync workflow-building and gtm-outreach-workflows skill content (DELAY/event composition) for existing workspaces',
})
export class SyncWorkflowBuilderCompositionSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing workflow-building and gtm-outreach-workflows skill content for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `Workflow builder composition skills already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync workflow builder composition skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync workflow builder composition skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced workflow-building and gtm-outreach-workflows skill content for workspace ${workspaceId}`,
    );
  }
}
