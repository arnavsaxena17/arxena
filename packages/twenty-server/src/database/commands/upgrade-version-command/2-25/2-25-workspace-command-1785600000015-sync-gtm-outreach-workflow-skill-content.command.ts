...
@RegisteredWorkspaceCommand('2.25.0', 1785600000015)
@Command({
  name: 'upgrade:2-25:sync-gtm-outreach-workflow-skill-content',
  description:
...
export class SyncOutreachWorkflowSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing GTM outreach workflow skill content for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `GTM outreach workflow skills already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync GTM outreach workflow skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync GTM outreach workflow skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced GTM outreach workflow skill content for workspace ${workspaceId}`,
    );
  }
}
