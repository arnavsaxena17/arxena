...
@RegisteredWorkspaceCommand('2.25.0', 1785600000113)
@Command({
  name: 'upgrade:2-25:sync-workflow-spill-parse-skill-content',
  description:
    'Sync workflow-building and code-interpreter skill content (correct get_workflow_current_version spill parse shape) for existing workspaces',
})
export class SyncWorkflowSpillParseSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing workflow spill-parse skill content for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `Workflow spill-parse skills already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync workflow spill-parse skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync workflow spill-parse skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced workflow-building and code-interpreter spill-parse skill content for workspace ${workspaceId}`,
    );
  }
}
