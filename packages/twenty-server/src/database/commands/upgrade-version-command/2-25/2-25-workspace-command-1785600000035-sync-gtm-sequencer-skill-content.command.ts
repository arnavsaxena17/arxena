...
@RegisteredWorkspaceCommand('2.25.0', 1785600000035)
@Command({
  name: 'upgrade:2-25:sync-gtm-sequencer-skill-content',
  description:
...
export class SyncOutreachSequencerSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing GTM sequencer skill content for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `GTM sequencer skills already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync GTM sequencer skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync GTM sequencer skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced GTM sequencer skill content for workspace ${workspaceId}`,
    );
  }
}
