...
@RegisteredWorkspaceCommand('2.25.0', 1785600000069)
@Command({
  name: 'upgrade:2-25:sync-gtm-project-send-window-fields',
  description:
...
export class SyncGtmProjectSendWindowFieldsCommand extends ProvisionedWorkspaceCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing GTM Project send-window fields for workspace ${workspaceId}`,
    );
    ...
    this.logger.log(
      `Synced Project sendTimezone / sendWindowStart / sendWindowEnd for workspace ${workspaceId}`,
    );
  }
}
