...
@RegisteredWorkspaceCommand('2.25.0', 1785600000064)
@Command({
  name: 'upgrade:2-25:sync-gtm-search-people-profile-fields',
  description:
    'Re-seed Search people so job title, company, employment history, and education reach workflow output',
})
export class SyncOutreachSearchPeopleProfileFieldsCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing search-people profile fields for workspace ${workspaceId}`,
    );
