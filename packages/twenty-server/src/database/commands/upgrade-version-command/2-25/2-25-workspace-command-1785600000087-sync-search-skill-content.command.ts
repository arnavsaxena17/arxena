...
@RegisteredWorkspaceCommand('2.25.0', 1785600000087)
@Command({
  name: 'upgrade:2-25:sync-search-skill-content',
  description:
    'Sync unified search skill content (split source files + provider toggle markers) for existing workspaces',
})
export class SyncSearchSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing search skill content for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `Search skill already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync search skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync search skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(`Synced search skill content for workspace ${workspaceId}`);
  }
}
