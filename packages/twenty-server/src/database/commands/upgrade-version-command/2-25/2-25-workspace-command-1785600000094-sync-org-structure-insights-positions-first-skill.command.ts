...
const SKILL_UNIVERSAL_IDENTIFIERS_TO_SYNC = [
  STANDARD_SKILL['org-structure-insights'].universalIdentifier,
] as const;

@RegisteredWorkspaceCommand('2.25.0', 1785600000094)
@Command({
  name: 'upgrade:2-25:sync-org-structure-insights-positions-first-skill',
  description:
    'Sync org-structure-insights skill to positions-first Ask AI playbooks',
})
export class SyncOrgStructureInsightsPositionsFirstSkillCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing org-structure-insights positions-first skill for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `Org-structure-insights skill already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync org-structure-insights positions-first skill:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync org-structure-insights positions-first skill for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced org-structure-insights positions-first skill for workspace ${workspaceId}`,
    );
  }
}
