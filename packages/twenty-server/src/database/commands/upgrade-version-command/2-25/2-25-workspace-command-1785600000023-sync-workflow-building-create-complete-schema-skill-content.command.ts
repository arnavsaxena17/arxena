...
@RegisteredWorkspaceCommand('2.25.0', 1785600000023)
@Command({
  name: 'upgrade:2-25:sync-workflow-building-create-complete-schema-skill-content',
  description:
    'Sync workflow-building skill content (compact create_complete_workflow + GTM Workflow 1 example) for existing workspaces',
})
export class SyncWorkflowBuildingCreateCompleteSchemaSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing workflow-building create-complete schema skill content for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `Workflow-building create-complete schema skill already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync workflow-building create-complete schema skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync workflow-building create-complete schema skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced workflow-building create-complete schema skill content for workspace ${workspaceId}`,
    );
  }
}
