...
@RegisteredWorkspaceCommand('2.25.0', 1785600000022)
@Command({
  name: 'upgrade:2-25:sync-workflow-building-form-hitl-skill-content',
  description:
    'Sync workflow-building skill content (FORM WhatsApp Official human-in-the-loop) for existing workspaces',
})
export class SyncWorkflowBuildingFormHitlSkillContentCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Syncing workflow-building FORM HITL skill content for workspace ${workspaceId}`,
    );
    ...
    if (skillsToCreate.length === 0 && skillsToUpdate.length === 0) {
      this.logger.log(
        `Workflow-building FORM HITL skill already up to date for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to sync workflow-building FORM HITL skill content:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to sync workflow-building FORM HITL skill content for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Synced workflow-building FORM HITL skill content for workspace ${workspaceId}`,
    );
  }
}
