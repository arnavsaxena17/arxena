...
@RegisteredWorkspaceCommand('2.25.0', 1785600000129)
@Command({
  name: 'upgrade:2-25:unpin-bulk-force-stop-workflow-runs-command-menu-item',
  description:
    'Unpin Bulk Force Stop from the record page top bar (keep it in the actions drawer)',
})
export class UnpinBulkForceStopWorkflowRunsCommandMenuItemCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Unpinning Bulk Force Stop command for workspace ${workspaceId}`,
    );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to unpin Bulk Force Stop command:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to unpin Bulk Force Stop command for workspace ${workspaceId}`,
      );
    }
