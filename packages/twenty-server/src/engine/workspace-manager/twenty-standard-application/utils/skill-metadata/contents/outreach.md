...
3. Enroll: native `upload-profiles` / Candidates with `outreachSequenceStage=QUEUED` and `candidateFlags.startOutreach=false` — enroll alone does **not** fire the Sequencer.
4. Start: record action **Start Outreach** on Candidate or Person (or People tab) sets `candidateFlags.startOutreach=true` → Sequencer runs. **Stop Outreach** sets `candidateFlags.stopOutreach=true` / stage `STOPPED` and blocks reply re-entry.
