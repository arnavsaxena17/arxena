...
export const getOutreachAgentIds = (workspaceId: string) => ({
  linkedinMessage: v5(
...
const upsertHarvestProject = async ({
  ...
  const projectId = getOutreachHarvestProjectId(workspaceId);
  ...
  const hasOutreachRunKey = await tableHasColumn({
    entityManager,
    schemaName,
    tableName,
    columnName: 'projectIds',
  });
  ...
  if (hasOutreachRunKey) {
    columns.splice(2, 0, 'projectIds');
    values.splice(2, 0, projectId);
  }

  if (hasOutreachWorkflowId) {
    const insertAt = hasOutreachRunKey ? 3 : 2;
  ...
  const updateSet = [
    hasOutreachWorkflowId
      ...
      : null,
    hasOutreachRunKey ? `"projectIds" = EXCLUDED."projectIds"` : null,
...
export const prefillOutreachWorkflows = async ({
  entityManager,
  workspaceId,
  schemaName,
  applicationId,
