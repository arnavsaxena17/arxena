...
const FETCH_EMAIL_HANDLER = ENRICH_CONTACT_HANDLER;
const FETCH_PHONE_HANDLER = ENRICH_CONTACT_HANDLER;
...
const DETECT_FAKE_PROFILES_HANDLER = `// Native GTM action: OutreachFakeProfileDetectorService.
// Workflow/Test/executeOneLogicFunction run the server executor, not this sandbox.
export const main = async (params: {
...
const FILTER_PROFILES_HANDLER = `// Native GTM action: OutreachFilterProfilesService.
// Workflow/Test/executeOneLogicFunction run the server executor, not this sandbox.
export const main = async (params: {
  profiles?: Array<Record<string, unknown>>;
  prompt?: string;
...
const NATIVE_HANDLERS: Record<string, string> = {
  ...
  [OUTREACH_UPLOAD_PROFILES_LOGIC_FUNCTION_NAME]: UPLOAD_PROFILES_HANDLER,
  [OUTREACH_UPSERT_COMPANIES_LOGIC_FUNCTION_NAME]: UPSERT_COMPANIES_HANDLER,
