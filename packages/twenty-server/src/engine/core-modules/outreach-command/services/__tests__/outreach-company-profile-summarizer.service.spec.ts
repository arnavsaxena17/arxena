...
describe('OutreachCompanyProfileSummarizerService', () => {
  const enrichment: OutreachCollectedCompanyEnrichment = {
  ...
  it('returns null when AI model registry is unavailable', async () => {
    const summarizer = new OutreachCompanyProfileSummarizerService(undefined);
  ...
  it('uses generateObject with the resolved model and returns the object', async () => {
    ...
    const summarizer = new OutreachCompanyProfileSummarizerService(
