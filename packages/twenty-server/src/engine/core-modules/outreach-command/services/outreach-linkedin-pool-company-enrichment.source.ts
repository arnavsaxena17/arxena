...
import type {
  OutreachLinkedInCompanyProfile,
  OutreachLinkedInCompanySearchHit,
...
@Injectable()
export class OutreachLinkedInPoolCompanyEnrichmentSource
  implements OutreachCompanyEnrichmentSource
{
  readonly sourceId = 'linkedin_unipile_pool' as const;
  private readonly logger = new Logger(
    OutreachLinkedInPoolCompanyEnrichmentSource.name,
  );
  ...
  async enrich(
    ...
    let linkedInCompanyProfile: OutreachLinkedInCompanyProfile | null =
      profileFromIndex;
  ...
  private async fetchCompanySearchHit(input: {
    ...
    accountId: string;
  }): Promise<OutreachLinkedInCompanySearchHit | null> {
  ...
  ): OutreachLinkedInCompanySearchHit | null {
    if (items.length === 0) {
      return null;
    }
  ...
  private async fetchCompanyProfile(
    ...
    accountId: string,
  ): Promise<OutreachLinkedInCompanyProfile | null> {
