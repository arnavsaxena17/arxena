...
import { appendProjectId, projectIdsHasProject } from 'src/engine/core-modules/outreach-command/utils/project-ids.util';
    ...
    return this.globalWorkspaceOrmManager.executeInWorkspaceContext(
      async () => {
        ...
        for (const hit of hits) {
          ...
          const nextOutreachRunKey = appendProjectId(match?.projectIds, projectId);
          const patch = pickWritable(
            {
              ...
              !isNonEmptyString(match?.linkedinId)
                ? { linkedinId }
                : {}),
              ...(!projectIdsHasProject(match?.projectIds, projectId)
                ? { projectIds: nextOutreachRunKey }
                : {}),
              ...(!isNonEmptyString(match?.outreachFunnelStage)
          ...
          const record = pickWritable(
            {
              ...
              projectIds: appendProjectId(null, projectId),
              outreachFunnelStage: 'ADDED',
