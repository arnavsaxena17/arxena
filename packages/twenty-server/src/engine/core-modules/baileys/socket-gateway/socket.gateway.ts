...
export class SocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async fileUpload(@MessageBody() message: MessageDto) {
    try {
      ...
      if (fileData) {
        new BaileysBot("sendWhatsappMessageHandleFileUpload").handleFileUpload(
          fileData,
          userDirectory
        );
      }
  ...
  async sendWhatsappMessage(@MessageBody() body: MessageDto) {
    try {
      ...
      if (Boolean(body.fileData?.fileName)) {
        ...
        if (body.fileData) {
          body.fileData = await new BaileysBot(
            "sendWhatsappMessagesendWhatsappMessagehandleFileUpload"
          ).handleFileUpload(body.fileData, userDirectory);
        }
  ...
  handleMessage(
    ...
    this.server.emit(
      "message",
      "server reply:" + authToken + ", this is data" + data
  ...
  handleConnection(client: Socket) {
    ...
    (async () => {
      try {
        this.baileys = await new BaileysBot(
          "sendWhatsappMessagehandleConnection"
        ).initApp(this, "started from handleConnection");
      } catch (error) {
        this.handleError(error);
      }
    })();
    ...
    try {
      ...
      if (!this.baileys) {
        ...
        (async () => {
          try {
            this.baileys = await new BaileysBot(
              "sendMessageServiceNotInitilaised"
            ).initApp(this, "because service is not initilised");
          } catch (error) {
            this.handleError(error);
          }
        })();
