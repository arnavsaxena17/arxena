...
export class BaileysBot {
  ...
  async initApp(arxSocket: SocketGateway, source: string) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      socket.ev.on("connection.update", async (updatedConnection: any) => {
        await this.handleBaileysConnection(
          updatedConnection,
          arxSocket,
      ...
      socket.ev.on(
        "messages.upsert",
        async (message: any) =>
          await this.handleMessage(message, arxSocket, socket)
      );
  ...
  async processBaileysMessages(m: any, arxSocket: SocketGateway, socket: any) {
    ...
    if (m.messages[0].key.fromMe === false) {
      ...
      let phoneNumberTo = "";
      try {
        phoneNumberTo = JSON.parse(
          JSON.stringify(arxSocket, this.getCircularReplacer())
        )?.baileys?.user?.id?.replace(/:(.*)/, "");
      } catch {
        phoneNumberTo = "";
      }
      console.log("Phone Number TO captured:", phoneNumberTo);
      const baileysWhatsappIncomingObj = {
        phoneNumberFrom: m?.messages[0]?.key?.remoteJid?.replace(
          "@s.whatsapp.net",
          ""
        ),
        message:
      ...
      console.log(
        "FInal baileysWhatsappIncomingObj:",
        baileysWhatsappIncomingObj
      );
      new IncomingWhatsappMessages().receiveIncomingMessagesFromBaileys(
        baileysWhatsappIncomingObj
      );
    } else {
      let phoneNumberFrom = "";
      try {
        phoneNumberFrom = JSON.parse(
          JSON.stringify(arxSocket, this.getCircularReplacer())
        )?.baileys?.user?.id?.replace(/:(.*)/, "");
      } catch {
        phoneNumberFrom = "";
      ...
      const baileysWhatsappOutgoingObj = {
        ...
        phoneNumberTo: m?.messages[0]?.key?.remoteJid?.replace(
          "@s.whatsapp.net",
          ""
        ),
        message:
      ...
      const candidateProfileData =
        await new FetchAndUpdateCandidatesChatsWhatsapps().getCandidateDetailsByPhoneNumber(
          baileysWhatsappOutgoingObj.phoneNumberTo
        );
      console.log("This is the candidateProfileData", candidateProfileData);
      if (
        candidateProfileData &&
        candidateProfileData != allDataObjects.emptyCandidateProfileObj
      ) {
        ...
        const userMessage: allDataObjects.candidateChatMessageType = {
          ...
          messageObj:
            candidateProfileData?.whatsappMessages?.edges[
              candidateProfileData?.whatsappMessages?.edges?.length
            ]?.node?.messageObj || {},
  ...
  async handleBaileysConnection(
    updatedConnection: any,
    arxSocket: SocketGateway,
    handleConnectionsource: string
  ) {
    // console.log("Value of arxSocker!!!", arxSocket);
    console.log(
      "handleBaileysConnection being calledW!!!",
      updatedConnection,
      "by the source of",
      this.source,
      "handleConnectionsource::",
      handleConnectionsource
    );
    ...
    console.log(
      "handleBaileysConnection being receivedPendingNotifications!!!",
      receivedPendingNotifications
    );
    ...
    if (connection === "close") {
      ...
      if (shouldReconnect) {
        if (!isConnectionRefreshed) {
          console.log(
            "its saying to init app again because it says should reconnec tand that connections is not refreshed"
          );
          this.initApp(
            arxSocket,
            "because should reconnect and connection is not refreshed"
          );
        } else {
          console.log(
            "Tried to do an init app and not sure if it has workedd "
          );
        }
      }
      if (isLoggedOut) {
        ...
        this.initApp(
          arxSocket,
          "is loggedout and hence deleting creds and init app again"
        );
      }
