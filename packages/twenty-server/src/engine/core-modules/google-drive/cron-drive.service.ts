...
export class CronDriveService {
  ...
  async getWorkspaceRecordingsFolder(
    workspaceId: string,
    dataSourceSchema: string,
  ): Promise<string> {
    const baseDir = process.env.GOOGLE_DRIVE_DATA_DIR || './google-drive-data';
  ...
  private async processWorkspaceFiles(
    ...
    const smsFolder = await this.getWorkspaceSMSFolder(
      workspaceId,
      dataSourceSchema,
    );
    const recordingsFolder = await this.getWorkspaceRecordingsFolder(
      workspaceId,
      dataSourceSchema,
    );
