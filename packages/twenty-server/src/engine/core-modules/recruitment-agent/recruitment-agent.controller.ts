...
export class RecruitmentAgentController {
  ...
  async create(@Req() request: Request): Promise<string> {
    ...
    const userMessageBody: allDataObjects.chatMessageType | null =
      request?.body as allDataObjects.chatMessageType | null; // Type assertion
    ...
    if (userMessageBody !== null) {
      ...
      let chatResponseMessagesResult = await new LLMChatAgent().runChatAgent(
        userMessage
      );
...
export class UpdateChatEndpoint {
  ...
  async create(@Req() request: Request): Promise<object> {
    ...
    if (userMessageBody !== null) {
      ...
      const updateStatus =
        await new CandidateEngagement().updateAndSendWhatsappMessageAndCandidateEngagementStatusInTable(
          userMessage
        );
