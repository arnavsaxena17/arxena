...
export class CandidateSearchChatController {
  ...
  async processMessageStream(
    ...
    try {
      this.logger.log(
        `Processing streaming chat message for assistantThreadId: ${body.assistantThreadId}`,
      );
      ...
      const sendEvent = (event: string, data: any) => {
        ...
        if (isAborted) {
          this.logger.log(
            `Skipping event ${event} - request aborted (isAborted flag)`,
          );
          return false;
        }
        ...
        if (res.closed || res.destroyed) {
          this.logger.log(
            `Skipping event ${event} - response closed or destroyed`,
          );
          isAborted = true;
          return false;
        }
        ...
        if (socket && (socket.destroyed || socket.readyState === 'closed')) {
          this.logger.log(
            `Skipping event ${event} - socket destroyed or closed`,
          );
          isAborted = true;
          return false;
        }
        ...
        if (data?.chatMessage && typeof data.chatMessage === 'string') {
          ...
          this.logger.log(
            `Captured chatMessage from ${event} event: ${data.chatMessage.substring(0, 100)}...`,
          );
        } else if (
          data?.message &&
          typeof data.message === 'string' &&
          (event === 'clarification' || event === 'message')
        ) {
          // Capture message field for clarification and message events
          accumulatedChatMessages.push(data.message);
          this.logger.log(
            `Captured message from ${event} event: ${data.message.substring(0, 100)}...`,
          );
        }
      ...
      if (finalAssistantMessage) {
        try {
          ...
          this.logger.log(
            `Saved assistant message to chat history for assistantThreadId: ${body.assistantThreadId}`,
          );
        } catch (error) {
          this.logger.error(
            `Failed to save assistant message to chat history: ${error.message}`,
          );
        }
      } else {
        this.logger.warn(
      ...
      if (accumulatedChatMessages.length > 0 && apiToken) {
        try {
          ...
          this.logger.log(
            `Saved accumulated assistant message to chat history after error`,
          );
        } catch (saveError) {
          this.logger.error(
            `Failed to save accumulated assistant message after error: ${saveError.message}`,
          );
        }
      }
      ...
      if (!isAborted && !res.closed && !res.destroyed) {
        try {
          ...
          res.write(
            `data: ${JSON.stringify({ error: errorMessage, chatMessage: `Sorry, I encountered an error: ${errorMessage}` })}\n\n`,
          );
          ...
          if (apiToken) {
            try {
              ...
              this.logger.error(
                `Failed to save error message to chat history: ${saveError.message}`,
              );
            }
          }
        } catch (writeError) {
