...
export class QueryCleanupPrompts {
  ...
  static getSystemPrompt(): string {
    ...
    If the query indicates positions and a requirement of company types, Your result should most likely be in the form xyz positions at abc types of companies. 
    Mention the exact type or category or segment of companies indicated in the requirement in a single phrase (e.g. Listed specialty chemicals companies based in India, Product based companies based in US, Law firms or corporate environments which are family run businesses, Startups). Mention the company/ industry segment + geography.
    If the query indicates a type of role without an indication of company type, your response should be in the form xyz positions without a company type.
    ...
    Do not mention company or job or industry specifications that may not be written in candidate profiles but can be inferred from the job title or company names by recruiters. Ignore jargon that is not commonly written in candidate profiles.
    Ignore any specific company or job description specifications outside of the company segement or job title.
    Your task is to rewrite the client search query into a clean search query that focuses on what candidates actually write in their profiles.`;
