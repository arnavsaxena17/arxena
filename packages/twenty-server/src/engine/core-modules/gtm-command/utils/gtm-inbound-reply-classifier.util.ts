export const GTM_INBOUND_INTENTS = [
  'unsubscribe',
  'not_now',
  'interested',
  'times_proposed',
...
export type GtmInboundIntent = (typeof GTM_INBOUND_INTENTS)[number];

export type GtmClassifiedOutreachStage =
  | 'STOPPED'
  | 'DEFERRED'
  | 'NEGOTIATING'
  | 'REPLIED'
...
export type GtmInboundReplyClassification = {
  intent: GtmInboundIntent;
  stage: GtmClassifiedOutreachStage;
  confidence: number;
  reasoning: string;
  extractedTimeHint: string;
};

export const GTM_INBOUND_INTENT_TO_STAGE: Record<
  GtmInboundIntent,
  GtmClassifiedOutreachStage
> = {
  unsubscribe: 'STOPPED',
  not_now: 'DEFERRED',
...
export const classificationFromIntent = ({
  ...
  const normalized = GTM_INBOUND_INTENTS.includes(intent as GtmInboundIntent)
    ? (intent as GtmInboundIntent)
    : 'question';

  return {
    intent: normalized,
    stage: GTM_INBOUND_INTENT_TO_STAGE[normalized],
