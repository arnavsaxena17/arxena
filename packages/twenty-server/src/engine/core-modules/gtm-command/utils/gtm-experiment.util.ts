...
export type GtmExperimentVariant = 'A' | 'B';

export type GtmExperimentWorkflowBinding = {
  workflowId: string;
  versionA?: string;
  versionB?: string;
};

export type GtmExperimentConfig = {
  status: 'running' | 'paused' | 'completed';
  split: number;
  name?: string;
  workflows?: {
    companySearch?: GtmExperimentWorkflowBinding;
    perCandidate?: GtmExperimentWorkflowBinding;
    candidateUpdated?: GtmExperimentWorkflowBinding;
  };
};

export const parseGtmExperimentConfig = (
  raw: string | null | undefined,
): GtmExperimentConfig | null => {
  if (!raw || raw.trim().length === 0) {
    return null;
  }

  try {
    const parsed = JSON.parse(raw) as GtmExperimentConfig;
...
export const stringifyGtmExperimentConfig = (
  config: GtmExperimentConfig,
 ...
 */
export const assignGtmExperimentVariant = ({
  seed,
  split = 0.5,
}: {
  seed: string;
  split?: number;
}): GtmExperimentVariant => {
...
export const SEQUENCE_DELAY_PENDING_REASON = 'gtm_sequence_delay';

export type GtmOutboundMessageKind =
  | 'CONNECT_NOTE'
  | 'OPENER'
  | 'FU1'
  | 'FU2'
...
export const resolveGtmOutboundMessageKind = ({
  materializeEvent,
  messagingChannel,
