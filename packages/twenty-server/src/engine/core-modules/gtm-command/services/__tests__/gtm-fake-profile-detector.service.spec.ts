...
describe('GtmFakeProfileDetectorService', () => {
  beforeEach(() => {
    generateObjectMock.mockReset();
  });

  it('returns an error when no profiles are provided', async () => {
    const detector = new GtmFakeProfileDetectorService(undefined);
  ...
  it('assesses a profile with hy3:free and returns fake hits', async () => {
    ...
    const detector = new GtmFakeProfileDetectorService(aiModelRegistryService);
