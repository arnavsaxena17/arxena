...
type GtmWorkspaceProfileRecord = ObjectLiteral & {
  id: string;
  name?: string | null;
...
@Injectable()
export class GtmWorkspaceProfileProvisioningService {
  private readonly logger = new Logger(
    GtmWorkspaceProfileProvisioningService.name,
  );
  ...
  async enqueueBootstrap(
    ...
    await this.messageQueueService.add<GtmWorkspaceProfileBootstrapJobData>(
      GTM_WORKSPACE_PROFILE_BOOTSTRAP_JOB_NAME,
      data,
      { retryLimit: 2 },
    );
