...
export class EnrichmentProcessorService {
  ...
  async processEnrichments(
    ...
    const totalOperations = candidates.length * enrichments.length;
    let currentOperation = 0;

    console.log(`Processing ${enrichments.length} enrichments for ${candidates.length} candidates with parallel requests`);
    ...
    for (const enrichment of enrichments) {
      for (const candidate of candidates) {
        allTasks.push({
          candidate,
          enrichment,
          taskId: `${candidate.id}-${enrichment.modelName}`
        });
      }
    }
    ...
    const taskPromises = allTasks.map(async (task) => {
      ...
      try {
        ...
        return {
          ...
          enrichmentName: task.enrichment.modelName,
          data: enrichmentData
        };
      } catch (error) {
  ...
  private async processSingleEnrichment(
    candidate: CandidateData,
    enrichment: EnrichmentConfig
    ...
    try {
      ...
      if (!userInput.trim()) {
        console.warn(`No input data for candidate ${candidate.id} with enrichment ${enrichment.modelName}`);
        return {};
      }
      ...
      const response = await this.getOpenAIResponse(
        enrichment.prompt,
        userInput,
        enrichment.selectedModel,
        enrichment.fields
      );

      return response;
    } catch (error) {
      console.error(`Error processing enrichment for candidate ${candidate.id}:`, error);
      return {};
    }
  }
  ...
  private async getOpenAIResponse(
    ...
    userInput: string,
    model: string,
    expectedFields: EnrichmentField[]
  ): Promise<Record<string, any>> {
  ...
  async computeTokensForEnrichment(
    candidates: CandidateData[],
    enrichments: EnrichmentConfig[]
  ): Promise<{
    ...
    for (const enrichment of enrichments) {
      for (const candidate of candidates) {
        ...
        const inputTokens = Math.ceil((enrichment.prompt.length + userInput.length) / 4);
        const outputTokens = Math.ceil(enrichment.fields.length * 50 / 4); // Estimate 50 chars per field
