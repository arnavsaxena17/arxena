...
export class CandidateService {
  ...
  async getCandidateFieldsByJobId(
    ...
    try {
      ...
      console.log('This is the candidate field values for jobId:', response.data.data?.jobs?.edges[0]?.node?.candidates?.edges[0]?.node?.candidateFieldValues?.edges.map((edge: any) => edge?.node?.candidateFields?.name));
