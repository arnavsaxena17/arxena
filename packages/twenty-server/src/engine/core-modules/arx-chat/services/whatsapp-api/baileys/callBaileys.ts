...
export async function sendWhatsappTextMessageViaBaileys(
  sendTextMessageObj: allDataObjects.ChatRequestBody
) {
  console.log(
    "This is the ssendTextMessageObj for baileys to be sent ::",
    sendTextMessageObj
  );
  ...
  try {
    ...
    if (response.status !== 200 || 201) {
      ...
      console.log(
        "Trying a second time because not worked the first time",
        response.status
      );
      if (response.status !== 200 || 201) {
        const response = await axios.post(sendMessageUrl, data);
        if (response.status !== 200 || 201) {
          console.log(
            "Even third time not worked. SO FUCKEDUP. FIND ANOTHER WAY",
            response.status
          );
        } else {
...
export async function sendAttachmentMessageViaBaileys(
  sendTextMessageObj: allDataObjects.BaileysAttachmentObject
) {
