  ...
  async getCandidateIdsWithVideoInterviewCompleted(apiToken: string): Promise<string[]> {
    ...
    if (allCandidates.length > 0) {
      ...
      const candidateIdsWithVideoInterviewCompleted = allCandidates
        .filter(candidate => candidate?.videoInterview?.edges[0]?.node?.interviewCompleted && new Date(candidate?.videoInterview?.edges[0]?.node?.updatedAt).getTime() < new Date(sixHoursAgo).getTime())
        .map(candidate => candidate.id);
      console.log('Number of candidates for which video interview completed::', candidateIdsWithVideoInterviewCompleted.length);
