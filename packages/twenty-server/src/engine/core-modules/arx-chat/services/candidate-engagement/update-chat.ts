...
>>>>>>> origin/dev_ninad
  ...
  async createAndUpdateWhatsappMessage(
    ...
    const createNewWhatsappMessageUpdateVariables = {
      input: {
        ...
        phoneTo: userMessage?.phoneNumberTo,
        jobsId:
          candidateProfileObj[0]?.node.jobs?.id || candidateProfileObj.jobs?.id,
        recruiterId:
          candidateProfileObj[0]?.node?.jobs?.recruiterId ||
          candidateProfileObj?.jobs?.recruiterId,
        name: userMessage?.messageType,
        messageObj: userMessage?.messageObj,
        whatsappDeliveryStatus: userMessage.whatsappDeliveryStatus,
        whatsappMessageId: userMessage?.whatsappMessageId,
    ...
    try {
      const response = await axiosRequest(graphqlQueryObj);
      console.log(
        "This is the response from the axios request::",
        response.data
      );
      return response.data;
    } catch (error) {
  ...
  async updateCandidateAnswer(
    ...
    console.log(
      "Updating candidate's status",
      candidateProfileObj,
      JSON.stringify(AnswerMessageObj)
    );
    const updateCandidateObjectVariables = {
      input: {
        ...AnswerMessageObj,
  ...
  async scheduleCandidateInterview(
    ...
    console.log(
      "Updating candidate's status",
      candidateProfileObj,
      JSON.stringify(scheduleInterviewObj)
    );
    const candidateEngagementStatus =
