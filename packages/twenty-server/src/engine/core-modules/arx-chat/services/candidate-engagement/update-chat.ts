...
export class FetchAndUpdateCandidatesChatsWhatsapps {
  ...
  async getCandidateInformation(userMessage: allDataObjects.chatMessageType) {
    ...
    try {
      ...
      const response = await axiosRequest(graphqlQueryObj);
      console.log(
        "This is the response from getCandidate Information",
        response.data.data
      );
      ...
      if (activeJobCandidateObj) {
        const personWithActiveJob = response?.data?.data?.people?.edges?.find(
          (person: { node: { candidates: { edges: any[] } } }) =>
        ...
        const candidateProfileObj: allDataObjects.CandidateNode = {
          ...
          jobs: {
            ...
            companies: {
              ...
              descriptionOneliner:
                activeJobCandidateObj?.node?.jobs?.companies
                  ?.descriptionOneliner,
            },
            jobLocation: activeJobCandidateObj?.node?.jobs?.jobLocation,
            whatsappMessages:
              activeJobCandidateObj?.node?.jobs?.whatsappMessages,
          },
  ...
  async getCandidateInformationToSendMessageTo(
    userMessage: allDataObjects.chatMessageType
  ) {
    // Get the candidate information from the user message
    // console.log("This is the userMessage Content", userMessage.messages[0].content);
    // console.log("This is the candidate userMessage", userMessage);
    ...
    const graphVariables = {
      filter: {
        phone: {
          ilike: "%" + userMessage.phoneNumberTo + "%",
        },
      },
      orderBy: {
        position: "AscNullsFirst",
    ...
    try {
      ...
      console.log("This is the activeJobCandidateObj", candidateDataObjs);
      //   if (activeJobCandidateObj) {
      const personWithActiveJob = response?.data?.data?.people?.edges?.find(
        (person: { node: { candidates: { edges: any[] } } }) =>
          person?.node?.candidates?.edges?.some(
      ...
      const candidateProfileObj: allDataObjects.CandidateNode = {
        ...
        jobs: {
          ...
          companies: {
            ...
            descriptionOneliner:
              candidateDataObjs?.node?.jobs?.companies?.descriptionOneliner,
          },
  ...
  async fetchQuestionsByJobId(jobId: string): Promise<{
    questionIdArray: {
      questionId: string;
      question: string;
    }[];
  ...
  async createAndUpdateWhatsappMessage(
    candidateProfileObj: allDataObjects.CandidateNode,
    userMessage: allDataObjects.candidateChatMessageType
  ) {
    console.log(
      "This is the candidate profile object",
      JSON.stringify(candidateProfileObj)
    );
    // console.log("This is the user message for updateWhtsappMessage in createAnd UpdateWhatsappMessage", userMessage);
    console.log(
      "This is the user messageObj for updateWhtsappMessage",
      userMessage?.messageObj
    );
    console.log(
      "This is the number of messages in  updateWhtsappMessage",
      userMessage?.messageObj.length
    );
    // console.log("This is the message being published ", userMessage?.messages[0]?.text);
    console.log(
      "This is the message being published ",
    ...
    const createNewWhatsappMessageUpdateVariables = {
      input: {
        ...
        message:
          userMessage?.messages[0]?.content || userMessage?.messages[0]?.text,
    ...
    console.log(
      "These are the graphvsariables:",
      JSON.stringify(createNewWhatsappMessageUpdateVariables)
    );
    const graphqlQueryObj = JSON.stringify({
    ...
    try {
      const response = await axiosRequest(graphqlQueryObj);
      console.log(
        "This is the response from the axios request in createAndUpdateWhatsappMessage::",
        response.data
      );
  ...
  async setCandidateEngagementStatusToFalse(
    ...
    console.log(
      "This is the value of updatecandidateobject variables::0",
  ...
  async updateCandidateAnswer(
    candidateProfileObj: allDataObjects.CandidateNode,
    AnswerMessageObj: allDataObjects.AnswerMessageObj
  ) {
  ...
  async scheduleCandidateInterview(
    candidateProfileObj: allDataObjects.CandidateNode,
    scheduleInterviewObj: allDataObjects.candidateChatMessageType
  ) {
    // console.log("Updating candidate's status", candidateProfileObj, JSON.stringify(scheduleInterviewObj));
    const candidateEngagementStatus =
      scheduleInterviewObj.messageType !== "botMessage";
    const updateCandidateObjectVariables = {
      idToUpdate: candidateProfileObj?.id,
      input: {
  ...
  async removeChatsByPhoneNumber(phoneNumberFrom: string) {
    const personObj: allDataObjects.PersonNode =
      await this.getPersonDetailsByPhoneNumber(phoneNumberFrom);
