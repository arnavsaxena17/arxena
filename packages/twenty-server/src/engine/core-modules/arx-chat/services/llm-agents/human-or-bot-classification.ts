...
export class HumanLikeLLM {
  ...
  async checkIfResponseMessageSoundsHumanLike(
    responseMessage: { content: string | null },
    apiToken,
  ) {
    let responseMessageType = 'seemsHumanMessage';

    try {
      console.log(
        'Going to check if response message sounds human like or not',
      );
      console.log(
        'the response message is actually:',
      ...
      if (responseMessage?.content != null && responseMessage?.content != '') {
        const checkBotTypeMessages: ChatCompletionMessageParam[] = [
          {
            role: 'system',
            content: `You are an expert at classifying sentences from a conversation into whether they sound human or bot like. 
        ...
        if (LLMMessage === '') {
          ...
          if (isLikeBot === 'no' || isLikeBot === 'No') {
            ...
            console.log(
              'THe message does sound like a bot:',
              responseMessage.content,
            );
        ...
        console.log(
          'REsponse message is either null or is empty string',
          responseMessage?.content,
        );
        responseMessageType = 'seemsHumanMessage';
      }
      ...
      console.log(
        'responseMessage Type to check if HUman Message has been created is an error:',
        responseMessageType,
      );
      responseMessageType = 'seemsHumanMessage';
    }
