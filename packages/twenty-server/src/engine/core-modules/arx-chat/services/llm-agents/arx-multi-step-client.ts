...
export class OpenAIArxMultiStepClient {
  ...
  async createCompletion(mostRecentMessageArr: allDataObjects.ChatHistoryItem[], personNode: allDataObjects.PersonNode, chatControl:allDataObjects.chatControls, isChatEnabled: boolean = true) {
    ...
    mostRecentMessageArr = await this.runCandidateFacingAgentsAlongWithToolCalls(mostRecentMessageArr, personNode, stage,  chatControl, isChatEnabled);
  ...
  async runCandidateFacingAgentsAlongWithToolCalls(mostRecentMessageArr: allDataObjects.ChatHistoryItem[], personNode: allDataObjects.PersonNode, stage: string, chatControl:allDataObjects.chatControls,  isChatEnabled: boolean = true, ) {
    try{
      ...
      console.log('BOT_MESSAGE in runCandidateFacingAgentsAlongWithToolCalls_stage1 :', "at::", new Date().toString(), ' ::: ' ,JSON.stringify(responseMessage), '  Stage:::', stage);
      if (responseMessage){
      ...
      if (responseMessage?.tool_calls && isChatEnabled) {
        mostRecentMessageArr = await this.addResponseAndToolCallsToMessageHistory(responseMessage, mostRecentMessageArr, stage, chatControl);
      }
  ...
  async addResponseAndToolCallsToMessageHistory(responseMessage: ChatCompletionMessage, mostRecentMessageArr: allDataObjects.ChatHistoryItem[], stage: string, chatControl:allDataObjects.chatControls) {
    ...
    if (toolCalls) {
      ...
      if (response?.choices[0]?.message?.tool_calls) {
        ...
        mostRecentMessageArr = await this.addResponseAndToolCallsToMessageHistory(response.choices[0].message, mostRecentMessageArr, stage, chatControl);
      }
