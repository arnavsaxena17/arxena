...
export class OpenAIArxMultiStepClient {
  ...
  async runCandidateFacingAgentsAlongWithToolCalls(mostRecentMessageArr: allDataObjects.ChatHistoryItem[], personNode: allDataObjects.PersonNode, stage: string, isChatEnabled: boolean = true) {
    ...
    console.log('BOT_MESSAGE FROM runCandidateFacingAgentsAlongWithToolCalls STAGE1 :', responseMessage.content);
  ...
  async addResponseAndToolCallsToMessageHistory(responseMessage: ChatCompletionMessage, stage:string, messages: allDataObjects.ChatHistoryItem[]) {
    ...
    if (toolCalls) {
      ...
      console.log('BOT_MESSAGE FROM addResponseAndToolCallsToMessageHistory STAGE2 :', response.choices[0].message.content);
      messages.push(response.choices[0].message);
      if (response.choices[0].message.tool_calls) {
        messages = await this.addResponseAndToolCallsToMessageHistory(response.choices[0].message, stage, messages);
      }
