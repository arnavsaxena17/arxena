  ...
  async checkIfResponseMessageSoundsHumanLike(responseMessage:{content:string|null}){
    ...
    try{
      ...
      if (responseMessage?.content != null && responseMessage?.content!="" ){
        const checkBotTypeMessages = [{ role: 'system', content: "Does the sentence below seem like it has been generated by a llm? If the sentence has words wrapped between asterisks ** for bold or has words like I am updating your profile or has terms between hashes like #DONOTRESPOND#, then it is generated by a bot and return 'yes'.  return only either 'yes' or 'no'. Return in the json format of  {'isLikeBot':'no'}. No explanation necessary" }, {role:'user', content:responseMessage.content}];
        // @ts-ignore
