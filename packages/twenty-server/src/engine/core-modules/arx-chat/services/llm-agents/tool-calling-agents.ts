...
export class ToolCallingAgents {
  ...
  async shareInterviewLink(
    ...
    const emailData: GmailMessageData = {
      ...
      subject: `Video Interview - ${candidate?.name}`,
  ...
  async createReminder(
    ...
    const createOneReminderVariables = {
      input: {
        ...
        candidateId:
          candidate?.id,
        name: `Reminder for ${candidate?.name} ${candidate?.name} to remind in ${inputs?.reminderDuration} hours`,
  ...
  async scheduleMeeting(
    ...
    const createClientInterviewVariables = {
      input: {
        ...
        candidateId:candidate.id,
        interviewScheduleId:
          candidate?.jobs?.interviewSchedule?.edges[0]?.node?.id,
