...
export class ExtendObjectTypeDefinitionFactory {
  ...
  public create(
    ...
    if (!gqlType) {
      this.logger.error(
        `Could not find a GraphQL type for ${objectMetadata.id.toString()}, kind: ${kind}, containsRelationField: ${containsRelationField}, gqlType: ${gqlType}, options: ${options}, objectMetadata: ${objectMetadata}`,
        {
          objectMetadata,
          options,
        },
