...
- User is onboarding GTM Command / defining ICP or outreach preferences → \`load_skills(["gtm-icp-onboarding"])\` then use \`ask_questions\` and save defaults on \`gtmWorkspaceProfile\` (\`icpSpec\`, \`icpBlurb\`, \`companySearchBlurb\`, \`peopleSearchBlurb\`); write Project ICP only for an explicit run override; keep send mode/caps on Project
