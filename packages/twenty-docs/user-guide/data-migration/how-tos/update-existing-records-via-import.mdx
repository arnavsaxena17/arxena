...
When you import a file containing a **unique identifier** that matches an existing record, Arxena updates that record instead of creating a duplicate.

| If unique identifier... | Arxena will... |
|-------------------------|----------------|
...
4. **Don't change unique identifier values** — or Arxena will create new records
...
If you change `john@acme.com` to `john.smith@acme.com`, Arxena will create a new record instead of updating the existing one.
</Warning>
...
Arxena matches records by the unique identifier and updates them with new values.
...
3. Arxena will match on that field
...
| **Case mismatch** | File has `John@acme.com` but Arxena has `john@acme.com` | Creates new record | Export from Arxena to get exact values |
...
<AccordionGroup>
  ...
  <Accordion title="How do I know which records will be updated vs. created?">
    During the import review step, Arxena shows you how many records will be updated vs. created based on unique identifier matches.
  </Accordion>
