...
description: Step-by-step guide to migrate your data from any CRM to Arxena.
---

## Overview

This guide walks you through migrating your data from any CRM to Arxena. The process involves auditing your data, preparing your Arxena workspace, exporting from your current system, and importing into Arxena.
...
Create a mapping document between your current CRM and Arxena:
...
## Step 3: Set Up Your Arxena Workspace

Before importing data, prepare your Arxena workspace:
...
If your data includes user references (Account Owner, Assignee, etc.), those users must exist in Arxena before import. Otherwise, those relations cannot be mapped.
</Warning>
...
Open each exported CSV in a spreadsheet application and prepare it for Arxena.

### Remove Duplicates

1. Sort by the unique field (email for People, domain for Companies)
2. Remove or merge duplicate rows
3. Verify no duplicates exist in Arxena already
...
Use `https://domain.com` (not `domain.com` or `www.domain.com`). This matches Arxena's format and prevents duplicates when you connect email/calendar sync.
</Warning>
...
1. Navigate to the object in Arxena
...
- Arxena will start syncing emails to contact records
...
<AccordionGroup>
  <Accordion title="Can I migrate workflows automatically?">
    Not currently. Workflows must be recreated manually in Arxena.
  </Accordion>
...
<Tip>
  ...
  - **Done for you:** [find a certified Arxena partner](https://arxena.com/partners/list?categories=SOLUTIONING&ref=docs-migrate-crm) to move your data, pipelines, and users end-to-end.
  - **Onboarding pack:** a 4-hour guided setup with our team — [contact@arxena.com](mailto:contact@arxena.com).
</Tip>
