...
[프런트 컴포넌트](/l/ko/developers/extend/apps/layout/front-components)는 Arxena 안에 샌드박스된 React 컴포넌트입니다. 우리 컴포넌트는 선택된 레코드를 읽고, `CoreApiClient`를 통해 person 템플릿을 로드한 뒤, 이전 장에서 만든 라우트로 POST 요청을 보냅니다.
...
<Frame caption="Template 탭: Arxena 의 네이티브 리치 텍스트 에디터가 body 필드에 연결되어 있습니다.">
