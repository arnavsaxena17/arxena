...
프런트 컴포넌트는 Arxena 내에서 세 위치에 렌더링될 수 있습니다:
...
> **Arxena Cloud에서 HTTP로 트리거되는 로직 함수는 작업공간별 전용 도메인에서 제공됩니다**: `https://\<your-workspace-subdomain>.witharxena.com\<path>`. 외부 호출자의 경우, 함수의 **HTTP trigger** 설정 또는 애플리케이션의 **Settings** 탭에서 정확한 URL을 복사하세요.
...
### Arxena REST API 호출하기

앱 HTTP 라우트를 호출하거나 프론트 컴포넌트에서 Arxena 레코드를 읽고 쓰려면 `twenty-client-sdk/rest`의 `RestApiClient`를 사용하세요. 이 클라이언트는 `/s/...` 경로를 작업공간의 함수 base URL로 전송하고, `/rest/...`를 포함한 그 외 모든 경로는 `TWENTY_API_URL`로 전송합니다.
...
| 변수                        | 설명                       |
| ------------------------- | ------------------------ |
| `TWENTY_API_URL`          | Arxena 코어 API의 기본 URL    |
...
이 URL이 항상 Arxena 서버 자체를 가리키는 것은 아니기 때문에 이 변수가 존재합니다. Arxena Cloud에서는 앱 라우트가 작업공간별 전용 도메인(`https://\<your-workspace-subdomain>.witharxena.com`, 또는 구성된 경우 애플리케이션의 기본 public 도메인)에서 제공되므로, 앱이 작성한 응답이 Arxena 앱 origin이 아니라 분리된 origin에서 실행됩니다. 셀프 호스팅 및 로컬 인스턴스에서는 앱 라우트가 서버 자체의 `/s` 접두사 아래에서 제공되며, 이 변수가 전혀 설정되지 않을 수 있습니다. base URL은 작업공간과 인스턴스마다 다르므로 코드에서 이를 하드코딩할 수 없으며, 서버가 런타임에 올바른 값을 주입합니다.
...
* **Arxena UI 컴포넌트** — Twenty의 자체 컴포넌트 라이브러리입니다. 아래의 [Arxena UI 컴포넌트 사용하기](#using-twenty-ui-components)를 참조하세요.
...
앱에 패키지를 추가하되, Arxena 인스턴스에서 제공되는 버전에 고정(pinned)하여 사용하세요:
...
Arxena UI 컴포넌트는 워크스페이스의 라이트 및 다크 테마와 자동으로 일치합니다. 렌더러가 호스트에서 활성 색 구성표를 적용하고, 컴포넌트는 그에 맞춰 자신의 색상을 결정합니다.
