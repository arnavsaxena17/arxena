...
1. Arxena 인스턴스에 접근하십시오 (보통 `http://localhost:3000`)
...
여러 독립적인 팀이 동일한 Arxena 인스턴스에서 자체 워크스페이스가 필요한 SaaS형 배포에서 멀티 워크스페이스 모드를 활성화하세요.
...
CORS에 의존하는 기능(예: 브라우저 내 파일 다운로드)과 함께 S3를 사용할 때에는, 버킷의 CORS 구성에서 Arxena 프런트엔드 오리진을 허용하도록 하세요.
