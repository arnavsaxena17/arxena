...
Arxena 설치 중에 적절한 스키마, 확장, 사용자를 사용하여 postgres 데이터베이스를 프로비저닝해야 합니다.
...
* Arxena 앱을 열고 `/settings`로 이동하여 화면 왼쪽 하단에 있는 **고급** 토글을 활성화하십시오.
* 새 웹훅 생성.
...
2. **리버스 프록시 설정 확인:**
   ...
   * 리버스 프록시가 Arxena 서버로 요청을 정확히 전달하는지 확인하십시오.
   * `X-Forwarded-For`, `X-Forwarded-Proto`와 같은 헤더가 적절히 설정되어 있는지 확인하십시오.

3. **서비스 재시작:**

   변경 후, 리버스 프록시와 Arxena 컨테이너를 모두 다시 시작하십시오.
...
* 커뮤니티 지원:
  ...
  [Arxena 커뮤니티](https://github.com/twentyhq/twenty/issues)나 [지원 채널](https://discord.gg/cx5n4Jzs57)에 문의하십시오.
