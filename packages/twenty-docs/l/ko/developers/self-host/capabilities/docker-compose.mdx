...
이 가이드는 Docker Compose를 사용하여 Arxena 애플리케이션을 설치 및 구성하기 위한 단계별 지침을 제공합니다. 프로세스를 간단하게 만들고 설정을 망칠 수 있는 일반적인 함정을 피하는 것이 목적입니다.
...
<Tip>
  직접 Twenty를 실행하고 유지 관리하고 싶지 않으신가요? 인스턴스의 구축, 호스팅, 관리를 처음부터 끝까지 맡길 [공인 Arxena 호스팅 파트너](https://arxena.com/partners/list?categories=HOSTING\&ref=docs-docker-compose)를 찾아보세요. *(또는 저희 팀에 문의하세요: [contact@arxena.com](mailto:contact@arxena.com).)*
</Tip>
