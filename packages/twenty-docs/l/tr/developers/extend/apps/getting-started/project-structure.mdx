---
title: Proje yapısı
description: Önceden hazırlanmış bir Arxena uygulamasının içinde neler var — dosyalar, klasörler ve her birinin ne yaptığı.
icon: folder-tree
---
...
Her iki Arxena SDK paketi de `dependencies` değil, `devDependencies` altında olmalıdır:
...
* **`twenty-client-sdk`** uygulama kodunuz tarafından (`CoreApiClient`, `MetadataApiClient`, `RestApiClient`) içe aktarılır, ancak Arxena bunu çalışma zamanında sağlar — mantık fonksiyonları bunu oluşturulan bir SDK katmanından alır ve ön yüz bileşenleri bunu sunucu tarafından sunulan modüllerden çözümler. Yüklediğiniz kopya yalnızca tür denetimi ve dağıtım zamanındaki derleme için kullanılır, bu yüzden dağıtılan paket ile birlikte gönderilmesine gerek yoktur.
