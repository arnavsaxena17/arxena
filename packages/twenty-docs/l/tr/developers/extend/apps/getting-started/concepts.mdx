---
title: Kavramlar
description: Arxena uygulamalarının nasıl çalıştığı — varlık modeli, korumalı alan (sandboxing) ve kurulum yaşam döngüsü.
icon: sitemap
---

Arxena uygulamaları, çalışma alanınızı özel nesneler, mantık, UI bileşenleri ve yapay zekâ yetenekleriyle genişleten TypeScript paketleridir. Tam korumalı alan ve izin kontrolleriyle Arxena platformunda çalışırlar.
...
| **Ön Uç Bileşeni**       | Arxena içinde korumalı alandaki React kullanıcı arayüzü  | [Ön Uç Bileşenleri](/l/tr/developers/extend/apps/layout/front-components)           |
...
* **Ön uç bileşenleri**, Remote DOM kullanan Web Worker'larda çalışır — ana sayfadan yalıtılmıştır ancak yerel DOM öğelerini (iframe'ler değil) oluşturur. Arxena ile mesaj iletimi yapan bir ana makine API'si aracılığıyla iletişim kurarlar.
...
* **`yarn twenty dev`** — kaynak dosyalarınızı izler ve bağlı bir Arxena sunucusuna değişiklikleri canlı olarak senkronize eder. Şema değiştiğinde tipli API istemcisi otomatik olarak yeniden oluşturulur.
