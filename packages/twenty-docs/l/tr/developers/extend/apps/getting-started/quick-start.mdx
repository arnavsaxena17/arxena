...
icon: rocket
description: İlk Arxena uygulamanızı dakikalar içinde oluşturun.
---
...
* **Docker** — [Buradan indirin](https://www.docker.com/products/docker-desktop/). Yerel bir Arxena sunucusunu çalıştırmak için gereklidir. Zaten başka bir yerde Arxena çalıştırıyorsanız atlayın.

Bir Arxena uygulaması oluşturmanın üç aşaması vardır. İskelet oluşturucu bunları tek bir sorunsuz akış komutuna indirger, ancak her aşama ayrı bir kavramdır — bir şeyler başarısız olduğunda hangi aşamada olduğunuzu bilmek neyi düzeltmeniz gerektiğini söyler.
...
| **2. Bir sunucu çalıştırma** | Eşitleme yapacağınız bir Arxena sunucusu başlatın | Docker + `yarn twenty docker:start` | Çalışan bir Arxena örneği          |
...
## Aşama 2 — Yerel bir Arxena sunucusu çalıştırın

Uygulamanızın eşitleme yapacağı bir Arxena sunucusuna ihtiyacı vardır. Sunucu, Docker içinde yerel olarak çalışan tam bir Arxena örneğidir — UI, GraphQL API, PostgreSQL. Yerel kodunuz tanımlarını bu sunucuya yükler; bu da onların arayüzde görünmesini sağlar.
...
Bunun yerine mevcut bir Arxena sunucusuna bağlanmak için `--url \<your-server-url>` parametresini iletin. Uzak sunucular OAuth ile kimlik doğrular: oturum açabilmeniz ve **Authorize**'a tıklayabilmeniz için bir tarayıcı açılır; bu, CLI'ye çalışma alanınıza erişim sağlar. (Yerelde de `--authentication-method oauth` ile OAuth kullanmayı tercih edebilirsiniz — `tim@apple.dev` / `tim@apple.dev` ile oturum açın.)

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Arxena oturum açma ekranı" />
</div>

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Arxena CLI yetkilendirme ekranı" />
</div>
...
**Bu aşamadan sonra:** [http://localhost:2020](http://localhost:2020) adresinde çalışan bir Arxena sunucunuz ve ona eşitleme yapmak için yetkilendirilmiş bir CLI'ınız olur.
...
| **Ön uç bileşenleri**     | Arxena'nin arayüzünde görüntülenen React bileşenleri (yan panel, widget'lar, komut menüsü)                |
