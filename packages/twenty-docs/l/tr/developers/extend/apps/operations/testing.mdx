---
title: Test
description: Vitest kurulumu, gerçek bir Arxena sunucusunda entegrasyon testleri, tip denetimi ve GitHub Actions ile CI.
icon: flask
---

SDK, test kodundan uygulamanızı derlemenize, dağıtmanıza, yüklemenize ve kaldırmanıza olanak tanıyan programatik API'ler sağlar. Tiplenmiş API istemcileriyle birlikte [Vitest](https://vitest.dev/) kullanarak, uygulamanızın gerçek bir Arxena sunucusunda uçtan uca çalıştığını doğrulayan entegrasyon testleri yazabilirsiniz.
...
Yerel Arxena sunucunuzun çalıştığından emin olun, ardından:
