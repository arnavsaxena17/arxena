...
title: Yayımlama
icon: upload
description: Arxena uygulamanızı pazaryerine sunun ya da dahili olarak dağıtın.
---
...
* **Bir tar arşivi dağıtın** — uygulamanızı dahili veya özel kullanım için doğrudan belirli bir Arxena sunucusuna yükleyin.
* **npm’ye yayımlama** — uygulamanızı Arxena pazaryerinde listeleyin; böylece herhangi bir çalışma alanı keşfedip yükleyebilir.
...
Genel kullanıma açık olmasını istemediğiniz uygulamalar — sahipli araçlar, yalnızca kurumsal entegrasyonlar veya deneysel derlemeler — için bir tarball’ı doğrudan bir Arxena sunucusuna dağıtabilirsiniz.
...
Uygulamanız belirli bir Arxena sunucu sürümünde sunulan bir özelliği kullanıyorsa (örneğin, v2.3.0'da eklenen OAuth sağlayıcıları), uygulamanızın gerektirdiği en düşük sunucu sürümünü `package.json` içindeki `engines.twenty` alanını kullanarak belirtmelisiniz:
...
2. `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` bileşik eylemini kullanarak yalıtılmış bir Arxena test örneği başlatır (CI'daki `yarn twenty docker:start --test` eşdeğeridir).
...
* `TWENTY_VERSION` (ortam, varsayılanı `latest`) — CI’da kullanılan Arxena sunucu sürümünü `ci.yml` içinde bunu düzenleyerek sabitleyin.
...
`main` dalına yapılan her itmede uygulamanızı yapılandırılmış bir Arxena sunucusuna dağıtır ve isteğe bağlı olarak `deploy` etiketi uygulandığında bir çekme isteğinden de dağıtım yapar.
...
| Ayar                    | Nerede                                                       | Amaç                                                                                                |
| ----------------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `cd.yml` içinde `env` (varsayılan: `http://localhost:3000`)  | Dağıtımın yapılacağı Arxena sunucusu. İlk kullanımdan önce bunu gerçek sunucu URL'nizle değiştirin. |
...
npm’ye yayımlamak, uygulamanızın Arxena pazaryerinde keşfedilebilir olmasını sağlar. Herhangi bir Arxena çalışma alanı, pazaryeri uygulamalarına doğrudan arayüzden göz atabilir, yükleyebilir ve güncelleyebilir.
...
Arxena sunucusu pazar yeri kataloğunu npm kayıt defterinden **her saat** eşitler.
...
<Note>
**npm provenance**, npm listenize bir güven rozeti ekler ve kullanıcıların paketin herkese açık bir CI ardışık düzenindeki belirli bir commit’ten oluşturulduğunu doğrulamasını sağlar. Ayrıca Arxena pazaryerinde uygulamanızın sahipliğini talep etmenizi de sağlar. Ayrıntılar için [npm provenance belgelerine](https://docs.npmjs.com/generating-provenance-statements) bakın.
</Note>
...
Arxena içinde **Ayarlar > Uygulamalar** sayfasına gidin; burada hem pazar yerindeki hem de tarball ile dağıtılmış uygulamalar görüntülenip yüklenebilir.
