...
Bir [ön uç bileşeni](/l/tr/developers/extend/apps/layout/front-components), Arxena içinde izole edilmiş bir React bileşenidir. Bizimki seçili kaydı okur, kişi şablonlarını `CoreApiClient` aracılığıyla yükler ve bir önceki bölümdeki rotaya POST isteği gönderir.
...
`RICH_TEXT` alanı olduğu için, Arxena bunun için zaten tam özellikli bir zengin metin editörü sunar — standart Note ve Task nesnelerinin kullandığı editörün aynısı. Biz sadece onu şablon kayıt sayfasında görünür hale getiriyoruz.
...
<Frame caption="Template sekmesi: Arxena'nin yerel zengin metin editörü, body alanına bağlı.">
