...
* `serverVariables` kullanın (`applicationVariables` değil) — OAuth kimlik bilgileri sunucu genelidir ve her Arxena sunucusu için bir OAuth uygulaması vardır.
...
4. Yüklü uygulamayı Arxena'de bir sunucu yöneticisi olarak açın → karşılık gelen `serverVariables` üzerinde değerleri ayarlayın.
