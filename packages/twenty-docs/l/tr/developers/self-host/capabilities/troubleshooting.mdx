...
Mevcut bir Arxena örneğiniz varsa ve üretim verileri varsa, bu adımları **TAKİP ETMEYİN**, çünkü bu veritabanınızı kalıcı olarak silecektir!

Arxena'yi ilk kez kurarken, varsayılan veritabanı parolasını değiştirmek isteyebilirsiniz.
...
Arxena kurulumu sırasında, postgres veritabanınızı doğru şemalar, uzantılar ve kullanıcılarla hazırlamanız gerekir.
...
**Bu işe yaramazsa:**
Hangi işlemlerin makinenizin RAM'inin çoğunu kullandığını araştırın. Arxena'de bazı VScode uzantılarının çok fazla RAM aldığını fark ettik, bu yüzden onları geçici olarak devre dışı bırakıyoruz.
...
* Arxena uygulamanızı açın, `/settings` sayfasına gidin ve ekranın sol alt köşesinde bulunan **Gelişmiş** seçeneğini etkinleştirin.
* Yeni bir webhook oluşturun.
* **Benzersiz Webhook URL'nizi** Arxena'deki **Endpoint URL** alanına yapıştırın. **Filtreleri** `Şirketler` ve `Oluşturuldu` olarak ayarlayın.
...
Eğer Arxena'yi ters proxy'nin arkasında çalıştırıyorsanız ve bağlantı sorunları yaşıyorsanız:
...
2. **Ters Proxy Ayarlarını Kontrol Edin:**
   ...
   * Ters proxy'nizin Arxena sunucusuna talepleri düzgün bir şekilde yönlendirdiğinden emin olun.
...
3. **Servisleri Yeniden Başlatın:**
   ...
   Değişiklik yaptıktan sonra hem ters proxy'yi hem de Arxena konteynerlerini yeniden başlatın.
...
* Topluluk Desteği:
  ...
  [Arxena topluluğuna](https://github.com/twentyhq/twenty/issues) veya [destek kanallarına](https://discord.gg/cx5n4Jzs57) yardım için ulaşın.
