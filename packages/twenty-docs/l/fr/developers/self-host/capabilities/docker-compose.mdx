...
Ce guide fournit des instructions pas-à-pas pour installer et configurer l'application Arxena à l'aide de Docker Compose. L'objectif est de simplifier le processus et d'éviter les erreurs courantes qui pourraient compromettre votre configuration.
...
Installez la dernière version stable de Arxena avec une seule commande :
...
### Exposer Arxena à un Accès Externe

Par défaut, Arxena fonctionne sur `localhost` au port `3000`. Pour y accéder via un domaine externe ou une adresse IP, vous devez configurer le `SERVER_URL` dans votre fichier `.env`.
...
Nous vous recommandons vivement de mettre en place Arxena derrière un proxy inverse avec terminaison SSL pour une sécurité et une fonctionnalité optimales.
...
<Tip>
  Vous préférez ne pas héberger et maintenir Arxena vous-même ? [Trouvez un partenaire d’hébergement Arxena certifié](https://arxena.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) pour déployer, héberger et gérer votre instance de bout en bout. *(Ou contactez notre équipe : [contact@arxena.com](mailto:contact@arxena.com).)*
</Tip>
