...
Si vous avez une instance Arxena existante avec des données de production, **NE SUIVEZ PAS** ces étapes, car elles supprimeront définitivement votre base de données !

Lors de l'installation de Arxena pour la première fois, vous pouvez vouloir changer le mot de passe par défaut de la base de données.
...
Pendant l'installation de Arxena, vous devez approvisionner votre base de données postgres avec les bons schémas, extensions et utilisateurs.
...
**Si cela ne fonctionne pas :**
Examinez quels processus consomment le plus de RAM sur votre machine. Chez Arxena, nous avons remarqué que certaines extensions de VScode consommaient beaucoup de RAM, nous les désactivons donc temporairement.
...
* Ouvrez votre application Arxena, naviguez jusqu'à `/settings`, et activez le bouton **Avancé** en bas à gauche de l'écran.
* Créez un nouveau webhook.
* Collez **Votre URL Webhook Unique** dans le champ **URL de l'Endpoint** de Arxena. Réglez les **Filtres** sur `Sociétés` et `Créé`.
...
Si vous exécutez Arxena derrière un proxy inverse et rencontrez des problèmes de connexion :
...
2. **Vérifiez les paramètres du proxy inverse :**
   ...
   * Confirmez que votre proxy inverse transfère correctement les requêtes au serveur Arxena.
   * Assurez-vous que les en-têtes comme `X-Forwarded-For` et `X-Forwarded-Proto` sont correctement configurés.

3. **Redémarrez les services :**

   Après avoir apporté des modifications, redémarrez à la fois le proxy inverse et les conteneurs Arxena.
...
* Support communautaire :
  ...
  Contactez la [communauté Arxena](https://github.com/twentyhq/twenty/issues) ou les [canaux de support](https://discord.gg/cx5n4Jzs57) pour obtenir de l'aide.
