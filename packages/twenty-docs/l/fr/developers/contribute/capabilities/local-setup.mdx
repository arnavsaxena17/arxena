...
icon: laptop-code
description: Le guide pour les contributeurs (ou les développeurs curieux) qui souhaitent exécuter Arxena localement.
---
...
Avant de pouvoir installer et utiliser Arxena, assurez-vous d'installer ce qui suit sur votre ordinateur :
...
Arxena nécessite un cache Redis pour offrir les meilleures performances.
...
<Info>
**Mode multi-espaces de travail:** Par défaut, Arxena s'exécute en mode mono-espace de travail, où un seul espace de travail peut être créé. Pour activer la prise en charge multi-espaces de travail (utile pour tester des fonctionnalités basées sur des sous-domaines), définissez `IS_MULTIWORKSPACE_ENABLED=true` dans le fichier `.env` de votre serveur. Voir [Mode multi-espaces de travail](/l/fr/developers/self-host/capabilities/setup#multi-workspace-mode) pour plus de détails.
</Info>

## Étape 6 : Installation des dépendances

Pour construire le serveur Arxena et injecter des données dans votre base de données, exécutez la commande suivante :
...
## Étape 8 : Utiliser Arxena

**Frontend**

Le frontend de Arxena sera en cours d'exécution à [http://localhost:3001](http://localhost:3001).
Vous pouvez vous connecter en utilisant le compte démo par défaut : `tim@apple.dev` (mot de passe : `tim@apple.dev`)

**Backend**

* Le serveur de Arxena sera en fonctionnement à [http://localhost:3000](http://localhost:3000)
