...
description: Ce qui se trouve à l’intérieur d’une application Arxena générée par un modèle — fichiers, dossiers et le rôle de chacun.
icon: folder-tree
---
...
Les deux packages du SDK Arxena doivent être placés dans `devDependencies`, et non dans `dependencies` :
