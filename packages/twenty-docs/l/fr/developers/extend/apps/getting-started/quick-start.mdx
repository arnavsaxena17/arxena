...
icon: rocket
description: Créez votre première application Arxena en quelques minutes.
---
...
* **Docker** — [Télécharger](https://www.docker.com/products/docker-desktop/). Nécessaire pour exécuter un serveur Arxena local. Ignorez si vous avez déjà Arxena en cours d’exécution ailleurs.

La création d’une application Arxena comporte trois phases. Le générateur les regroupe en une seule commande pour le parcours idéal, mais chaque phase est un concept distinct — en cas d’échec, savoir dans quelle phase vous vous trouvez indique ce qu’il faut corriger.
...
| **2. Exécuter un serveur** | Démarrer un serveur Arxena vers lequel se synchroniser | Docker + `yarn twenty docker:start` | Une instance Arxena en cours d’exécution                    |
...
## Phase 2 — Exécuter un serveur Arxena local

Votre application a besoin d’un serveur Arxena vers lequel se synchroniser. Le serveur est une instance Arxena complète — interface utilisateur, API GraphQL, PostgreSQL — exécutée localement dans Docker. Votre code local envoie ses définitions à ce serveur, qui les fait apparaître dans l’interface utilisateur.
...
**Après cette phase :** vous disposez d’un serveur Arxena en cours d’exécution sur [http://localhost:2020](http://localhost:2020) avec votre CLI autorisée à s’y synchroniser.
...
| **Composants d’interface utilisateur (UI)** | Composants React qui s’affichent dans l’interface de Arxena (panneau latéral, widgets, menu de commandes)               |
