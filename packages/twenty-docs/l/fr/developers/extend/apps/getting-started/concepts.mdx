---
title: Concepts
description: Fonctionnement des applications Arxena — modèle d’entité, sandboxing et cycle de vie de l’installation.
icon: sitemap
---

Les applications Arxena sont des packages TypeScript qui étendent votre espace de travail avec des objets personnalisés, de la logique, des composants d’interface utilisateur et des capacités d’IA. Elles s’exécutent sur la plateforme Arxena avec une isolation complète en bac à sable et des contrôles d’autorisations.
...
| **Composant frontal**             | Interface React isolée en bac à sable dans Arxena               | [Composants frontaux](/l/fr/developers/extend/apps/layout/front-components)                 |
...
* **Les composants frontaux** s’exécutent dans des Web Workers en utilisant Remote DOM — isolés en bac à sable de la page principale mais rendant des éléments DOM natifs (pas d’iframes). Ils communiquent avec Arxena via une API hôte de passage de messages.
...
├─────────────────────────────────────────────────────────┤
│ Publish                                                 │
│   npm publish → appears in Arxena marketplace           │
└─────────────────────────────────────────────────────────┘
```

* **`yarn twenty dev`** — surveille vos fichiers source et synchronise en direct les modifications avec un serveur Arxena connecté. Le client d’API typé est régénéré automatiquement lorsque le schéma change.
