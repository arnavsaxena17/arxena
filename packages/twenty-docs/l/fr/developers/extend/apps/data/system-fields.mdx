...
Chaque objet dans Arxena est livré avec un ensemble de **champs système** que vous ne déclarez jamais vous-même. Ils sont créés automatiquement par le serveur lorsque l'objet est provisionné :
...
<Note>
  ...
  à partir de la version 2.23 et nécessite un serveur Arxena en version 2.23 ou ultérieure.
</Note>
...
## Objets standard de Arxena

Pour un objet Arxena **standard** (Person, Company, Opportunity, …), vous n'avez rien à dériver : les identifiants des champs système sont des constantes pré-calculées que vous pouvez importer directement.
