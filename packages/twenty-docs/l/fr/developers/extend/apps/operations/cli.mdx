...
| `docker:*`                                      | Gérer le conteneur du serveur Arxena local                                                | [Serveur local](/l/fr/developers/extend/apps/getting-started/local-server)                                           |
...
<Note>
Ceci est différent de `yarn twenty docker:logs`, qui affiche les journaux du conteneur Docker. `yarn twenty dev:function:logs` affiche les journaux d'exécution des fonctions de votre application depuis le serveur Arxena.
</Note>

## Génération du client typé (`yarn twenty dev:generate-client`)

Régénérez le client API typé (`twenty-client-sdk`) à partir du schéma du remote actif, sans compiler ni synchroniser une application. Utilisez-le pour obtenir un client typé dans n’importe quel projet — comme un service back-end hébergé dans un dépôt distinct — qui communique avec votre instance Arxena :
...
Un remote est un serveur Arxena auquel votre application se connecte. Lors de la configuration, le générateur en crée un automatiquement pour vous. Vous pouvez ajouter d’autres remotes ou basculer entre eux à tout moment.
