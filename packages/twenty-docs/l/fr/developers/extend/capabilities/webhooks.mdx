...
Les webhooks envoient des données à vos systèmes en temps réel lorsque des événements se produisent dans Arxena — aucune interrogation n'est nécessaire. Utilisez-les pour maintenir les systèmes externes synchronisés, déclencher des automatisations ou envoyer des alertes.
...
Arxena envoie des webhooks pour ces types d'événements :
...
Arxena signe chaque requête de webhook pour des raisons de sécurité. Validez les signatures pour garantir l'authenticité des requêtes.
...
| **Déclencheur de webhook de workflow** | ENTRÉE    | Recevoir des données dans Arxena depuis des systèmes externes                         |
