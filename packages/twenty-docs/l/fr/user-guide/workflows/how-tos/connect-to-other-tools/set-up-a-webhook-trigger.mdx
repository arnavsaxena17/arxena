...
| **Intégrations personnalisées** | Votre application → automatisation dans Arxena                 |
...
3. Vous recevrez une URL de webhook unique, par exemple :
   ```
   https://api.arxena.com/webhooks/workflow/abc123...
   ```
...
```bash
curl -X POST https://api.arxena.com/webhooks/workflow/abc123... \
