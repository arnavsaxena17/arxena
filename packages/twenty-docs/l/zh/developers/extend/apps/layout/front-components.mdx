---
title: 前端组件
description: 构建可在 Arxena 的 UI 中渲染并具备沙盒隔离的 React 组件。
icon: window-maximize
---

前端组件是直接在 Arxena 的 UI 内渲染的 React 组件。 它们在使用 Remote DOM 的**隔离 Web Worker**中运行——你的代码在沙盒化的、不透明来源的 iframe 中执行，但其 UI 仍然原生渲染在页面中，而不是被限制在该 iframe 内。

## 前端组件可用位置

在 Arxena 中，前端组件可以在三个位置进行渲染：
...
该组件会渲染为设置选项卡内的一个部分，而不是替换整个选项卡。 Arxena 的系统管理部分——自动升级、App URL 和连接——始终渲染在它上方，且无法被应用覆盖。
...
由于该组件返回 `null`，Arxena 会跳过为其渲染容器——布局中不会出现空白区域。 该组件仍可访问所有 hooks 和宿主通信 API。
...
> **在 Arxena Cloud 上，HTTP 触发的逻辑函数通过每个工作区的专用域名提供服务**，域名为 `https://\<your-workspace-subdomain>.witharxena.com\<path>`。 对于外部调用方，请从函数的 **HTTP trigger** 设置或应用的 **Settings** 选项卡中复制准确的 URL。
...
传递给 `RestApiClient` 的路径是逻辑函数的 `httpRouteTriggerSettings.path`，并以 `/s` 作为前缀。 保持 `isAuthRequired: true`；Arxena 为你的组件生成的 `TWENTY_APP_ACCESS_TOKEN` 会对请求进行认证：
...
要在前端组件中调用应用 HTTP 路由或读取、写入 Arxena 记录，请使用来自 `twenty-client-sdk/rest` 的 `RestApiClient`。 它会将 `/s/...` 路径发送到你工作区的函数基础 URL，而将包括 `/rest/...` 在内的其他所有路径发送到 `TWENTY_API_URL`。
...
| 变量                        | 描述                    |
| ------------------------- | --------------------- |
| `TWENTY_API_URL`          | Arxena 核心 API 的基础 URL |
| `TWENTY_APP_ACCESS_TOKEN` | 限定在你的应用角色范围内的短期令牌     |

### `TWENTY_FUNCTIONS_URL`

Arxena 还会将 `TWENTY_FUNCTIONS_URL` 注入到前端组件和逻辑函数中：也就是你的应用的 HTTP 触发逻辑函数所提供服务的基础 URL。

之所以存在这个变量，是因为该 URL 并不总是 Arxena 服务器本身。 在 Arxena Cloud 上，应用路由通过每个工作区的专用域名提供服务（`https://\<your-workspace-subdomain>.witharxena.com`，或者在已配置时为应用的主公共域名），以便应用生成的响应运行在一个与 Arxena 应用源不同的隔离源上。 自托管和本地实例会在服务器本身的 `/s` 前缀下提供应用路由，并且可能完全不会设置该变量。 由于基础 URL 会因工作区和实例而异，你的代码不能将其硬编码——服务器会在运行时注入正确的值。
...
* **Arxena UI 组件** — Arxena 自身的组件库；请参阅下文的 [使用 Arxena UI 组件](#using-twenty-ui-components)
...
Arxena 通过 [`twenty-ui`](https://www.npmjs.com/package/twenty-ui/v/1.0.0-alpha.1) 包提供其组件库。 前端组件可以将其用于按钮、标签、状态徽章、Chip、头像、图标、排版，以及能够自动匹配工作区明暗主题的主题令牌。

### 安装

将该包添加到你的应用中，并固定为你的 Arxena 实例所提供的版本：
...
Arxena UI 组件会自动匹配工作区的明暗主题——渲染器会在宿主上应用当前启用的配色方案，组件会基于该方案解析自己的颜色。

要在你自己的行内样式中使用相同的设计令牌，请调用 `useTheme()` hook。 它会返回与当前主题关联的 Arxena 主题令牌（间距、颜色、圆角、字体），你的组件中无需设置 `ThemeProvider`：
