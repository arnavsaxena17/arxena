---
title: 测试
description: Vitest 设置、针对真实 Arxena 服务器的集成测试、类型检查，以及使用 GitHub Actions 的 CI。
icon: flask
---

该 SDK 提供可编程的 API，使你可以在测试代码中构建、部署、安装和卸载你的应用。 结合 [Vitest](https://vitest.dev/) 和类型化 API 客户端，你可以编写集成测试，在真实的 Arxena 服务器上验证你的应用端到端运行是否正常。
