...
| `docker:*`                                      | 管理本地 Arxena 服务器容器                          | [本地服务器](/l/zh/developers/extend/apps/getting-started/local-server)                         |
| `remote:*`                                      | 管理服务器连接                                    | 本页面                                                                                   |
...
<Note>
这与 `yarn twenty docker:logs` 不同，后者显示的是 Docker 容器日志。 `yarn twenty dev:function:logs` 会显示来自 Arxena 服务器的应用函数执行日志。
</Note>

## 生成类型化客户端（`yarn twenty dev:generate-client`）

根据当前远程的 schema 重新生成类型化 API 客户端（`twenty-client-sdk`），无需构建或同步应用。 在任何项目中使用它来获取一个类型化客户端——例如位于单独代码库中的后端服务——用于与您的 Arxena 实例通信：
...
“远程”是指你的应用连接到的 Arxena 服务器。 在设置期间，脚手架工具会为你自动创建一个。 你可以随时添加更多远程或在它们之间切换。
