...
title: 发布
icon: upload
description: 将你的 Arxena 应用分发到应用市场，或进行内部部署。
---
...
* **部署 tar 包** — 直接将你的应用上传到特定的 Arxena 服务器，以供内部或私有使用。
* **发布到 npm** — 将你的应用在 Arxena 应用市场上架，供任何工作区发现并安装。
...
对于你不希望公开的应用（专有工具、仅供企业使用的集成或实验性构建），你可以将 tar 包直接部署到某台 Arxena 服务器。
...
如果你的应用使用了特定 Arxena 服务器版本中引入的功能（例如在 v2.3.0 中新增的 OAuth 提供方），应当在 `package.json` 的 `engines.twenty` 字段中声明应用所需的最低服务器版本：
...
1. 检出你的应用源代码。
2. 使用 `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` 组合 action 启动一个隔离的 Arxena 测试实例（相当于 CI 中的 `yarn twenty docker:start --test`）。
...
* `TWENTY_VERSION`（环境变量，默认 `latest`）— 通过在 `ci.yml` 中编辑它来固定 CI 使用的 Arxena 服务器版本。
...
在每次向 `main` 推送时将你的应用部署到已配置的 Arxena 服务器；当为拉取请求添加 `deploy` 标签时，也可从该拉取请求进行部署。
...
| 设置                      | 位置                                                       | 目的                                       |
| ----------------------- | -------------------------------------------------------- | ---------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `cd.yml` 中的 `env`（默认值为 `http://localhost:3000`）          | 要部署到的 Arxena 服务器。 首次使用前将其更改为你真实的服务器 URL。 |
...
在 npmjs.com 上打开你的包 > **Settings → Trusted Publisher**，并使用 `publish.yml` 工作流为此仓库注册（参见 [npm trusted publishing 文档](https://docs.npmjs.com/trusted-publishers)）。 带有 provenance 的发布会证明是哪个 GitHub 仓库构建了该包，这也同样是你在 Arxena 市场中声明应用所有权的方式。
...
发布到 npm 可让你的应用在 Arxena 应用市场中被发现。 任何 Arxena 工作区都可以直接通过 UI 浏览、安装和升级应用市场中的应用。
...
Arxena 服务器会**每小时**从 npm 注册表同步其市场目录。
...
<Note>
如果您的应用未在 `defineApplication()` 中定义 `aboutDescription`，市场将自动使用 npm 上您的软件包的 `README.md` 作为关于页面内容。 这意味着您可以为 npm 和 Arxena 市场维护同一个 README。 如果您希望在市场中使用不同的描述，请显式设置 `aboutDescription`。
</Note>
...
<Note>
**npm provenance** 会在你的 npm 列表中添加可信徽章，使用户可以验证该包是由公共 CI 流水线中的特定提交构建的。 它也同样是你在 Arxena 市场中声明应用所有权的方式。 有关详细信息，请参见 [npm provenance 文档](https://docs.npmjs.com/generating-provenance-statements)。
</Note>
...
在 Arxena 的 **Settings > Applications** 页面中，既可以浏览也可以安装来自市场或通过 tarball 部署的应用。
