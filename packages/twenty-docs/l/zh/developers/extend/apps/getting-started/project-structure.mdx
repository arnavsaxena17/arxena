---
title: 项目结构
description: 脚手架生成的 Arxena 应用中包含哪些内容——文件、文件夹以及它们各自的作用。
icon: folder-tree
---
...
这两个 Arxena SDK 软件包都应放在 `devDependencies` 下，而不是 `dependencies`：
...
* **`twenty-client-sdk`** 会被你的应用代码（`CoreApiClient`、`MetadataApiClient`、`RestApiClient`）导入，但 Arxena 会在运行时提供它——逻辑函数从生成的 SDK 层获取它，前端组件则从服务器提供的模块中解析它。 你安装的副本仅用于类型检查和部署时的构建，因此不需要被打包进已部署的 bundle 中。
