...
title: 快速开始
icon: rocket
description: 几分钟内创建你的第一个 Arxena 应用。
---
...
* **Docker** — [在此下载](https://www.docker.com/products/docker-desktop/)。 运行本地 Arxena 服务器所需。 如果你已经在其他地方运行了 Arxena，请跳过。

构建一个 Arxena 应用包含三个阶段。 脚手架工具将它们合并为一个理想路径的命令，但每个阶段都是独立的概念——当出现问题时，知道自己处于哪个阶段可以指明需要修复什么。

| 阶段           | 你要做什么                | 工具                                  | 结果                   |
| ------------ | -------------------- | ----------------------------------- | -------------------- |
| **1. 脚手架**   | 生成应用的源代码             | `npx create-twenty-app`             | 磁盘上的一个 TypeScript 项目 |
| **2. 运行服务器** | 启动一个 Arxena 服务器以进行同步 | Docker + `yarn twenty docker:start` | 一个正在运行的 Arxena 实例    |
...
## 阶段 2 — 运行本地 Arxena 服务器

你的应用需要一个 Arxena 服务器来进行同步。 该服务器是一个完整的 Arxena 实例——包含 UI、GraphQL API、PostgreSQL——在本地的 Docker 中运行。 你的本地代码会将其定义上传到该服务器，从而使其显示在 UI 中。

脚手架会为你启动一个：在 Docker 正在运行的情况下，它会拉取 `twentycrm/twenty-app-dev` 镜像，在端口 `2020` 上启动，并将 CLI 认证到预置的演示工作区（`tim@apple.dev`）——无需登录。

若要改为连接到现有的 Arxena 服务器，请传入 `--url \<your-server-url>`。 远程服务器通过 OAuth 进行身份验证：会打开一个浏览器窗口，你可以登录并点击 **Authorize**，从而授予 CLI 访问你工作区的权限。 （你也可以在本地选择使用 OAuth，方式是添加 `--authentication-method oauth` —— 使用 `tim@apple.dev` / `tim@apple.dev` 登录。）

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Arxena 登录界面" />
</div>

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Arxena CLI 授权界面" />
</div>
...
**完成此阶段后：** 你在 [http://localhost:2020](http://localhost:2020) 上拥有一个正在运行的 Arxena 服务器，且你的 CLI 已获授权可与其同步。
...
| **前端组件**   | 在 Arxena 的 UI 内渲染的 React 组件（侧边面板、小部件、命令菜单） |
