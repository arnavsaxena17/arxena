...
当 Arxena 中发生事件时，Webhook 会实时将数据推送到您的系统 — 无需轮询。 使用它们来保持外部系统同步、触发自动化或发送警报。
...
为了安全起见，Arxena 会对每个 Webhook 请求进行签名。 请验证签名以确保请求真实有效。
...
| **工作流 Webhook 触发器** | IN  | 从外部系统接收数据到 Arxena    |
