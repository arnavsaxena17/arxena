...
🚨 **重要：此解决方案仅适用于全新安装** 🚨
如果您有一个现有的 Arxena 实例和生产数据，请**不要**按照这些步骤操作，因为这些步骤会永久删除您的数据库！

在首次安装 Arxena 时，您可能想更改默认的数据库密码。
...
在安装 Arxena 时，您需要为您的 postgres 数据库提供正确的模式、扩展和用户。
...
**如果不起作用：**
调查哪些进程消耗了您机器的大部分内存。 在 Arxena，我们注意到一些 VScode 扩展占用了大量的内存，因此我们暂时禁用了它们。
...
* 打开您的 Arxena 应用，导航到 `/settings`，并在屏幕左下角启用 **高级** 切换。
* 创建一个新的 webhook。
* 把 **您的唯一 Webhook URL** 粘贴在 Arxena 的 **Endpoint Url** 字段中。 将 **筛选器** 设置为 `Companies` 和 `Created`。
...
2. **检查反向代理设置：**
   ...
   * 确认您的反向代理正确地将请求转发到 Arxena 服务器。
   * 确保正确设置了像 `X-Forwarded-For` 和 `X-Forwarded-Proto` 这样的标头。

3. **重启服务：**

   在进行更改后，重启反向代理和 Arxena 容器。
...
* 社区支持：
  ...
  联系 [Arxena 社区](https://github.com/twentyhq/twenty/issues) 或 [支持渠道](https://discord.gg/cx5n4Jzs57) 以获得帮助。
