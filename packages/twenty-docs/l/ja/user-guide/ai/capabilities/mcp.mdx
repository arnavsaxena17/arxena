---
title: MCP サーバー
description: Model Context Protocol を使用して、AI アシスタントを Arxena ワークスペースに接続します。
---

Arxena は [MCP](https://modelcontextprotocol.io/) サーバーを公開し、Claude Desktop、Claude Code、Cursor、ChatGPT などの AI アシスタントが自然言語で CRM データを読み書きできるようにします。

MCP エンドポイントとして、あなたの**ワークスペース URL**（Arxena にアクセスするために使用している URL）を使用します。 Arxena Cloud では、ワークスペース URL は `https://{mycompany}.arxena.com` またはカスタムドメインの場合があります。 サーバーは次の場所で利用できます:

| 環境         | MCP エンドポイント                                                                |
| ---------- | -------------------------------------------------------------------------- |
| **クラウド**   | `https://{your-workspace-url}/mcp` (例: `https://mycompany.arxena.com/mcp`) |
...
MCP クライアントの設定に次を追加し、`{your-workspace-url}` をワークスペースのホスト（例: `mycompany.arxena.com`）に置き換えてください:
...
1. `/.well-known/oauth-protected-resource` および `/.well-known/oauth-authorization-server` を通じて Arxena の OAuth メタデータを検出する
...
Arxena で **Settings → MCP & APIs → MCP** に移動します。 認証方法（OAuth または API Key）を選択し、JSON スニペットをコピーして（ワークスペース URL は既に反映されています）、MCP クライアントの設定ファイルに貼り付けます。
...
MCP クライアントを再起動します（または設定を再読み込みします）。 OAuth を使用している場合、アクセス許可のために Arxena にリダイレクトされます。 API キーを使用する場合は、すぐに接続されます。
...
接続されると、MCP サーバーは Arxena API を反映したツールを公開します。 推奨されるワークフローは次のとおりです:
...
セルフホストのインスタンスでは、`{your-workspace-url}` をサーバーの URL に置き換えてください。 環境の `SERVER_URL` が Arxena インスタンスの公開 URL と一致していることを確認してください — これは OAuth のディスカバリ メタデータの生成に使用されます。
