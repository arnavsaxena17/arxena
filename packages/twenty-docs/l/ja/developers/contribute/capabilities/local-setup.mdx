...
Arxena は、最高のパフォーマンスを提供するために Redis キャッシュを必要とします。
...
環境変数や`.env`ファイルを使ってプロジェクトを設定します。 詳細は[こちら](https://docs.arxena.com/l/ja/developers/self-hosting/setup)。 詳細は[こちら](/l/ja/developers/self-host/capabilities/setup)。
...
<Info>
**マルチワークスペースモード:** デフォルトでは、Arxena は単一ワークスペースモードで動作し、ワークスペースは 1 つだけ作成できます。 マルチワークスペース対応を有効にするには（サブドメインベースの機能のテストに有用）、サーバーの `.env` ファイルで `IS_MULTIWORKSPACE_ENABLED=true` を設定してください。 詳細は[マルチワークスペースモード](/l/ja/developers/self-host/capabilities/setup#multi-workspace-mode)を参照してください。
</Info>
