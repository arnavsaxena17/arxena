...
[front component](/l/ja/developers/extend/apps/layout/front-components) は Arxena 内でサンドボックス化された React
...
テンプレートにはカスタムコンポーネントは一切必要ありません。 `body` は `RICH_TEXT` フィールドなので、Arxena にはすでにフル機能のリッチテキストエディターが用意されています。標準の Note および Task オブジェクトが使用しているものと同じエディターです。
