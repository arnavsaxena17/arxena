...
* `applicationVariables` ではなく `serverVariables` を使用してください — OAuth の認証情報はサーバー全体で共有され、Arxena サーバーごとに 1 つの OAuth アプリとなります。
...
4. サーバー管理者として Arxena でインストール済みアプリを開き、対応する `serverVariables` に値を設定します。
