...
Das übergeordnete Objekt ("Eins"-Seite) muss in Arxena existieren, bevor Sie darauf verweisen können.
...
1. Exportieren Sie die übergeordneten Datensätze aus Arxena
...
Verwenden Sie `https://domain.com` (nicht nur `domain.com`). Dies entspricht der Art, wie Arxena Unternehmensdomains speichert, und verhindert Zuordnungsfehler.
</Warning>
...
Arxena verknüpft automatisch jeden untergeordneten Datensatz mit dem passenden übergeordneten Datensatz.
...
| **Nicht vorhandene Datensätze**            | Der übergeordnete Datensatz existiert in Arxena nicht     | Importieren Sie zunächst übergeordnete Datensätze oder prüfen Sie auf Tippfehler |
| **Unterschiedliche Groß-/Kleinschreibung** | `Acme.com` in der Datei, aber `acme.com` in Arxena        | Achten Sie auf exakt übereinstimmende Groß-/Kleinschreibung                      |
...
<AccordionGroup>
  <Accordion title="Was ist, wenn mein übergeordneter Datensatz keine Domain oder E-Mail hat?">
    ...
    1. Verwenden Sie die Arxena-`id` (exportieren Sie übergeordnete Datensätze, um ihre IDs zu erhalten)
