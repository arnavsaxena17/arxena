...
* Verwenden Sie `serverVariables` (nicht `applicationVariables`) — OAuth-Anmeldedaten gelten serverweit und es gibt eine OAuth-App pro Arxena-Server.
...
4. Öffnen Sie die installierte App in Arxena als Serveradministrator → setzen Sie die Werte in den entsprechenden `serverVariables`.
