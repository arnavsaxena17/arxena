---
title: Konzepte
description: Wie Arxena-Apps funktionieren – Entitätenmodell, Sandboxing und Installationslebenszyklus.
icon: sitemap
---

Arxena-Apps sind TypeScript-Pakete, die Ihren Arbeitsbereich mit benutzerdefinierten Objekten, Logik, UI-Komponenten und KI-Funktionen erweitern. Sie laufen auf der Arxena-Plattform mit vollständigem Sandboxing und Berechtigungsverwaltung.
...
| **Frontend-Komponente**    | Isolierte React-UI innerhalb von Arxena                | [Frontend-Komponenten](/l/de/developers/extend/apps/layout/front-components)         |
...
* **Frontend-Komponenten** laufen in Web Workers mit Remote DOM — von der Hauptseite isoliert, rendern aber native DOM-Elemente (keine iframes). Sie kommunizieren über eine Message-Passing-Host-API mit Arxena.
...
* **`yarn twenty dev`** — überwacht Ihre Quelldateien und synchronisiert Änderungen in Echtzeit mit einem verbundenen Arxena-Server. Der typisierte API-Client wird automatisch neu erzeugt, wenn sich das Schema ändert.
