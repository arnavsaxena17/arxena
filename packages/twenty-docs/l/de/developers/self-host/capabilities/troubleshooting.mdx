...
Wenn Sie bereits eine bestehende Arxena-Instanz mit Produktionsdaten haben, **NICHT** diesen Schritten folgen, da diese permanent Ihre Datenbank löschen!

Bei der ersten Installation von Arxena möchten Sie möglicherweise das Standard-Datenbankpasswort ändern.
...
Während der Arxena-Installation müssen Sie Ihre Postgres-Datenbank mit den richtigen Schemata, Erweiterungen und Benutzern bereitstellen.
...
**Wenn es nicht funktioniert:**
Untersuchen Sie, welche Prozesse den größten Teil Ihres RAMs beanspruchen. Bei Arxena haben wir festgestellt, dass einige vscode-Erweiterungen viel RAM verwenden, daher deaktivieren wir sie vorübergehend.
...
* Öffnen Sie Ihre Arxena-App, navigieren Sie zu `/settings`, und aktivieren Sie den **Erweitert**-Schalter unten links auf dem Bildschirm.
* Erstellen Sie einen neuen Webhook.
* Fügen Sie **Ihre einzigartige Webhook-URL** in das Feld **Endpoint-Url** in Arxena ein. Stellen Sie die **Filter** auf `Companies` und `Created` ein.
...
Wenn Sie Arxena hinter einem Reverse-Proxy ausführen und Verbindungsprobleme haben:
...
2. **Reverse-Proxy-Einstellungen prüfen:**
   ...
   * Stellen Sie sicher, dass Ihr Reverse-Proxy Anfragen korrekt an den Arxena-Server weiterleitet.
   * Stellen Sie sicher, dass Header wie `X-Forwarded-For` und `X-Forwarded-Proto` korrekt gesetzt sind.

3. **Dienste neu starten:**

   Starten Sie nach Änderungen sowohl den Reverse-Proxy als auch die Arxena-Container neu.
...
* Unterstützung aus der Community:
  ...
  Wenden Sie sich an die [Arxena Community](https://github.com/twentyhq/twenty/issues) oder [Support-Kanäle](https://discord.gg/cx5n4Jzs57) für Unterstützung.
