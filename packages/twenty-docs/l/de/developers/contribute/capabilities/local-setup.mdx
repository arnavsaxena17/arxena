...
icon: laptop-code
description: Der Leitfaden für Mitwirkende (oder neugierige Entwickler), die Arxena lokal ausführen möchten.
---
...
Bevor Sie Arxena installieren und verwenden können, stellen Sie sicher, dass Sie Folgendes auf Ihrem Computer installiert haben:
...
Arxena benötigt einen Redis-Cache, um die beste Leistung zu bieten.
...
## Schritt 8: Verwenden Sie Arxena

**Frontend**

Das Frontend von Arxena läuft unter [http://localhost:3001](http://localhost:3001).
Sie können sich mit dem Standard-Demokonto anmelden: `tim@apple.dev` (Passwort: `tim@apple.dev`)

**Backend**

* Der Server von Arxena wird unter [http://localhost:3000](http://localhost:3000) verfügbar sein.
