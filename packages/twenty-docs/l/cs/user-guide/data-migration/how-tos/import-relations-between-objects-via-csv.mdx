...
Nadřazený („jedna“ strana) musí v Arxena existovat, než na něj budete moci odkazovat.
...
1. Exportujte nadřazené záznamy z Arxena
...
Použijte `https://domain.com` (ne pouze `domain.com`). To odpovídá způsobu, jak Arxena ukládá domény společností, a předchází chybám při párování.
</Warning>
...
Arxena automaticky propojí každý podřízený záznam s odpovídajícím nadřazeným.
...
| **Neexistující záznamy**     | Nadřazený záznam v Arxena neexistuje            | Nejprve importujte nadřazené záznamy nebo zkontrolujte překlepy |
| **Rozdílná velikost písmen** | `Acme.com` v souboru, ale v Arxena `acme.com`   | Zajistěte přesnou shodu velikosti písmen                        |
...
<AccordionGroup>
  <Accordion title="Co když můj nadřazený záznam nemá doménu ani e-mail?">
    Máte dvě možnosti:
    1. Použijte Arxena `id` (exportujte nadřazené záznamy, abyste získali jejich ID)
