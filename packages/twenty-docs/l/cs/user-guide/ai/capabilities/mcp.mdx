---
title: MCP Server
description: Připojte asistenty AI ke svému pracovnímu prostoru Arxena pomocí protokolu Model Context Protocol.
---

Arxena zpřístupňuje server [MCP](https://modelcontextprotocol.io/), aby AI asistenti — Claude Desktop, Claude Code, Cursor, ChatGPT a další — mohli číst a zapisovat data vašeho CRM pomocí přirozeného jazyka.

Použijte svou **adresu URL pracovní oblasti** (adresu URL, kterou používáte k přístupu k Arxena) jako koncový bod MCP. V Arxena Cloud může vaše adresa URL pracovní oblasti být `https://{mycompany}.arxena.com` nebo vlastní doména. Server je dostupný na:

| Prostředí           | Koncový bod MCP                                                               |
| ------------------- | ----------------------------------------------------------------------------- |
| **Cloud**           | `https://{your-workspace-url}/mcp` (např. `https://mycompany.arxena.com/mcp`) |
...
Přidejte toto do konfigurace svého klienta MCP a nahraďte `{your-workspace-url}` hostitelem vaší pracovní oblasti (např. `mycompany.arxena.com`):
...
1. Získá metadata OAuth služby Arxena pomocí `/.well-known/oauth-protected-resource` a `/.well-known/oauth-authorization-server`
...
V Arxena přejděte do **Settings → MCP & APIs → MCP**. Vyberte metodu ověření (OAuth nebo klíč API), zkopírujte úryvek JSON (už bude používat adresu URL vaší pracovní oblasti) a vložte jej do konfiguračního souboru klienta MCP.
...
Restartujte svého klienta MCP (nebo znovu načtěte konfiguraci). Pokud používáte OAuth, budete přesměrováni do Arxena k autorizaci přístupu. Pokud používáte klíč API, připojení je okamžité.
...
Po připojení server MCP zpřístupní nástroje, které odpovídají rozhraní Arxena API. Doporučený pracovní postup je:
...
U samostatně hostovaných instancí nahraďte `{your-workspace-url}` adresou URL svého serveru. Ujistěte se, že `SERVER_URL` ve vašem prostředí odpovídá veřejné adrese URL vaší instance Arxena — tato hodnota se používá k vygenerování metadat pro zjišťování OAuth.
