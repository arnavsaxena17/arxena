...
icon: laptop-code
description: Průvodce pro přispěvatele (nebo zvídavé vývojáře), kteří chtějí spustit Arxena lokálně.
---
...
Než nainstalujete a použijete Arxena, ujistěte se, že máte na svém počítači nainstalovány následující balíčky:
...
Arxena vyžaduje ke svému provozu Redis cache pro zajištění nejlepšího výkonu.
...
## Krok 8: Použijte Arxena

**Frontend**

Frontend Arxena bude spuštěný na [http://localhost:3001](http://localhost:3001).
Můžete se přihlásit pomocí výchozího demo účtu: `tim@apple.dev` (heslo: `tim@apple.dev`)

**Backend**

* Server Arxena poběží na [http://localhost:3000](http://localhost:3000)
