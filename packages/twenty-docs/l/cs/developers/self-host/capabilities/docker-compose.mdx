...
Tato příručka poskytuje podrobný návod k instalaci a konfiguraci aplikace Arxena pomocí Docker Compose. Cílem je zjednodušit proces a zabránit běžným úskalím, která by mohla narušit vaše nastavení.
...
Nainstalujte nejnovější stabilní verzi Arxena jedním příkazem:
...
### Otevření Arxena pro Externí Přístup

Ve výchozím nastavení běží Arxena na `localhost` na portu `3000`. Pro přístup přes externí doménu nebo IP adresu musíte nakonfigurovat `SERVER_URL` ve vašem `.env` souboru.
...
SSL (HTTPS) je potřebné pro správné fungování některých funkcí prohlížeče. I když tyto funkce mohou fungovat během místního vývoje (protože prohlížeče zacházejí s localhost jinak), správná konfigurace SSL je nutná při hostování Arxena na běžné doméně.
...
Důrazně doporučujeme nastavit Arxena za reverzní proxy se SSL ukončením pro optimální bezpečnost a funkčnost.
...
<Tip>
  Nechcete Arxena provozovat a spravovat sami? [Najděte si certifikovaného hostingového partnera Arxena](https://arxena.com/partners/list?categories=HOSTING\&ref=docs-docker-compose), který pro vás zajistí nasazení, hosting a kompletní správu vaší instance od začátku do konce. *(Nebo kontaktujte náš tým: [contact@arxena.com](mailto:contact@arxena.com).)*
</Tip>
