...
title: Rychlý start
icon: rocket
description: Vytvořte svou první aplikaci Arxena během několika minut.
---
...
* **Docker** — [Stáhnout](https://www.docker.com/products/docker-desktop/). Nutné pro spuštění lokálního serveru Arxena. Přeskočte, pokud už máte Arxena spuštěné jinde.

Vytváření aplikace Arxena má tři fáze. Generátor kostry je spojuje do jediného příkazu pro ideální scénář, ale každá fáze je samostatný koncept — když se něco nepovede, znalost aktuální fáze napoví, co opravit.
...
| **2. Spustit server**  | Spustit server Arxena, do kterého se bude synchronizovat | Docker + `yarn twenty docker:start` | Běžící instance Arxena                       |
...
## Fáze 2 — Spusťte lokální server Arxena

Vaše aplikace potřebuje server Arxena, do kterého se bude synchronizovat. Server je plnohodnotná instance Arxena — UI, GraphQL API, PostgreSQL — běžící lokálně v Dockeru. Váš lokální kód nahrává své definice na tento server, díky čemuž se objeví v UI.
...
Chcete-li se místo toho připojit k existujícímu serveru Arxena, předejte `--url \<your-server-url>`. Vzdálené servery se ověřují pomocí OAuth: otevře se prohlížeč, abyste se mohli přihlásit a kliknout na **Authorize**, čímž udělíte nástroji CLI přístup k vašemu pracovnímu prostoru. (Můžete se také rozhodnout pro OAuth lokálně pomocí `--authentication-method oauth` — přihlaste se pomocí `tim@apple.dev` / `tim@apple.dev`.)

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Přihlašovací obrazovka Arxena" />
</div>

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Autorizační obrazovka Arxena CLI" />
</div>
...
**Po této fázi:** máte spuštěný server Arxena na [http://localhost:2020](http://localhost:2020) a vaše CLI má oprávnění se k němu synchronizovat.
...
| **Frontendové komponenty** | Komponenty Reactu, které se vykreslují v uživatelském rozhraní Arxena (postranní panel, widgety, příkazová nabídka) |
