---
title: Pojmy
description: Jak aplikace Arxena fungují — model entit, sandboxing a životní cyklus instalace.
icon: sitemap
---

Aplikace Arxena jsou balíčky TypeScriptu, které rozšiřují váš pracovní prostor o vlastní objekty, logiku, komponenty UI a funkce AI. Běží na platformě Arxena s plnou izolací (sandboxingem) a řízením oprávnění.
...
| **Frontendová komponenta**    | Izolované React UI uvnitř Arxena                                | [Frontendové komponenty](/l/cs/developers/extend/apps/layout/front-components)     |
...
* **Frontendové komponenty** běží ve Web Workerech s využitím Remote DOM — jsou oddělené od hlavní stránky, ale vykreslují nativní prvky DOM (nikoli iframy). Komunikují s Arxena prostřednictvím hostitelského API pro předávání zpráv.
...
* **`yarn twenty dev`** — sleduje vaše zdrojové soubory a průběžně synchronizuje změny s připojeným serverem Arxena. Typovaný klient API se při změně schématu automaticky znovu vygeneruje.
