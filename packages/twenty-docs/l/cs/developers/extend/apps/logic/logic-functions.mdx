...
Logické funkce jsou serverové funkce v TypeScriptu, které běží na platformě Arxena. Mohou být spouštěny požadavky HTTP, plány cronu nebo databázovými událostmi — a lze je také zpřístupnit jako nástroje pro agenty AI.
...
* **`toolTriggerSettings`** — zpřístupní funkci AI funkcím Arxena (chat, MCP, volání funkcí). Používá standardní JSON Schema, formát, kterému modely LLM nativně rozumějí.
* **`workflowActionTriggerSettings`** — zobrazí funkci jako krok ve vizuálním builderu workflow. Používá bohaté `InputSchema` od Arxena, aby builder mohl vykreslit správné editory polí, voliče proměnných a štítky.
...
| `inputSchema`  | Rozšířený `InputSchema` Arxena — to, co tvůrce zobrazuje jako konfigurovatelná pole (s výběrem proměnných). Volitelné; pokud je vynecháno, odvodí se z handleru.                 |
...
Balíček `twenty-client-sdk` poskytuje dva typované klienty GraphQL pro práci s Arxena API z vašich logických funkcí a frontendových komponent.
...
<Note>
  Když váš kód běží na Arxena (logické funkce nebo frontendové komponenty), platforma vloží přihlašovací údaje jako proměnné prostředí:

  * `TWENTY_API_URL` — Základní URL Arxena API
