...
* Použijte `serverVariables` (nikoli `applicationVariables`) — pověření OAuth jsou celoserverová a na jeden server Arxena je jedna aplikace OAuth.
...
4. Otevřete nainstalovanou aplikaci v Arxena jako správce serveru → nastavte hodnoty na odpovídajících `serverVariables`.
