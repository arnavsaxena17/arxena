...
title: Publikování
icon: upload
description: Distribuujte svou aplikaci Arxena do Marketplace nebo ji nasaďte interně.
---
...
* **Nasaďte tarball** — nahrajte svou aplikaci přímo na konkrétní server Arxena pro interní nebo soukromé použití.
* **Publish to npm** — uveďte svou aplikaci v Marketplace Arxena, aby ji mohl kterýkoli pracovní prostor objevit a nainstalovat.
...
U aplikací, které nechcete zpřístupnit veřejně — proprietární nástroje, integrace pouze pro enterprise nebo experimentální buildy — můžete nasadit tarball přímo na server Arxena.
...
Pokud vaše aplikace používá funkci zavedenou v konkrétní verzi serveru Arxena (například poskytovatelé OAuth přidaní ve verzi 2.3.0), měli byste deklarovat minimální verzi serveru, kterou vaše aplikace vyžaduje, pomocí pole `engines.twenty` v `package.json`:
...
2. Spustí izolovanou testovací instanci Arxena pomocí složené akce `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` (ekvivalent v CI k `yarn twenty docker:start --test`).
...
* `TWENTY_VERSION` (env, výchozí hodnota `latest`) — uzamkněte v CI používanou verzi serveru Arxena úpravou této hodnoty v `ci.yml`.
...
Nasazuje vaši aplikaci na nakonfigurovaný server Arxena při každém pushi do `main` a volitelně také z pull requestu, pokud je přidán štítek `deploy`.
...
| Nastavení               | Kde                                                             | Účel                                                                                                 |
| ----------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` v `cd.yml` (výchozí `http://localhost:3000`)              | Server Arxena, na který se nasazuje. Před prvním použitím to změňte na skutečnou URL vašeho serveru. |
...
Publikování na npm zajistí, že bude vaše aplikace dohledatelná v Marketplace Arxena. Jakýkoli pracovní prostor Arxena může procházet, instalovat a aktualizovat aplikace z Marketplace přímo z UI.
...
Server Arxena synchronizuje svůj katalog tržiště z registru npm **každou hodinu**.
...
<Note>
Pokud vaše aplikace nedefinuje `aboutDescription` v `defineApplication()`, tržiště automaticky použije soubor `README.md` vašeho balíčku z npm jako obsah stránky O aplikaci. To znamená, že můžete spravovat jediný soubor README jak pro npm, tak pro tržiště Arxena. Pokud chcete v tržišti jiný popis, explicitně nastavte `aboutDescription`.
</Note>
...
Přejděte na stránku **Nastavení > Aplikace** v Arxena, kde lze procházet a instalovat jak aplikace z tržiště, tak aplikace nasazené jako tarball.
