...
| `docker:*`                                      | Spravovat kontejner lokálního serveru Arxena                                        | [Lokální server](/l/cs/developers/extend/apps/getting-started/local-server)                                   |
...
<Note>
To se liší od `yarn twenty docker:logs`, který zobrazuje logy kontejneru Docker. `yarn twenty dev:function:logs` zobrazuje logy běhu funkcí vaší aplikace ze serveru Arxena.
</Note>

## Generování typovaného klienta (`yarn twenty dev:generate-client`)

Znovu vygenerujte typovaného klienta API (`twenty-client-sdk`) ze schématu aktivního vzdáleného serveru, bez sestavování nebo synchronizace aplikace. Použijte jej k získání typovaného klienta v libovolném projektu – například backendové služby v samostatném repozitáři – který komunikuje s vaší instancí Arxena:
...
**Remote** je server Arxena, ke kterému se vaše aplikace připojuje. Během nastavení jej generátor kostry automaticky vytvoří. Můžete kdykoli přidat další vzdálené servery nebo mezi nimi přepínat.
