...
Esta guía proporciona instrucciones paso a paso para instalar y configurar la aplicación Arxena utilizando Docker Compose. El objetivo es simplificar el proceso y prevenir errores comunes que podrían arruinar tu configuración.
...
Instala la última versión estable de Arxena con un solo comando:
...
### Exponer Arxena para acceso externo

Por defecto, Arxena se ejecuta en `localhost` en el puerto `3000`. Para acceder a él mediante un dominio externo o dirección IP, necesitas configurar `SERVER_URL` en tu archivo `.env`.
...
Recomendamos encarecidamente configurar Arxena detrás de un proxy inverso con terminación SSL para una seguridad y funcionalidad óptimas.
...
<Tip>
  ¿Prefieres no ejecutar y mantener Arxena por tu cuenta? [Encuentra un partner de alojamiento certificado de Arxena](https://arxena.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) para implementar, alojar y gestionar tu instancia de principio a fin. *(O contacta con nuestro equipo: [contact@arxena.com](mailto:contact@arxena.com).)*
</Tip>
