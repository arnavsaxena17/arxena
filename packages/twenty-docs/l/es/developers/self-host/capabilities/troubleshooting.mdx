...
Si tiene una instancia de Arxena existente con datos de producción, **NO** siga estos pasos ya que borrarán permanentemente su base de datos.

Al instalar Arxena por primera vez, es posible que desee cambiar la contraseña predeterminada de la base de datos.
...
Durante la instalación de Arxena, debe aprovisionar su base de datos postgres con los esquemas, extensiones y usuarios correctos.
...
**Si no funciona:**
Investigue qué procesos están ocupando la mayor parte de la RAM de su máquina. En Arxena, notamos que algunas extensiones de VScode estaban ocupando mucha RAM, por lo que las desactivamos temporalmente.
...
* Abra la aplicación Arxena, navegue a `/settings` y active el interruptor **Avanzado** en la parte inferior izquierda de la pantalla.
* Cree un nuevo webhook.
* Pegue **Su URL de Webhook Única** en el campo **Url de EndPoint** en Arxena. Establezca los **Filtros** en `Companies` y `Created`.
...
Si está ejecutando Arxena detrás de un proxy inverso y experimenta problemas de conexión:
...
2. **Verifique la configuración del Proxy Reverso:**
   ...
   * Confirme que su proxy reverso está reenviando correctamente las solicitudes al servidor de Arxena.
...
3. **Reinicie los Servicios:**
   ...
   Después de hacer cambios, reinicie tanto el proxy inverso como los contenedores de Arxena.
...
* Soporte Comunitario:
  ...
  Póngase en contacto con la [comunidad de Arxena](https://github.com/twentyhq/twenty/issues) o [los canales de soporte](https://discord.gg/cx5n4Jzs57) para obtener asistencia.
