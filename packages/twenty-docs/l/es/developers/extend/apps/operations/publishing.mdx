...
title: Publicación
icon: upload
description: Distribuye tu aplicación de Arxena en el marketplace o despliégala internamente.
---
...
* **Desplegar un paquete tar** — sube tu aplicación directamente a un servidor Arxena específico para uso interno o privado.
* **Publicar en npm** — incluye tu aplicación en el marketplace de Arxena para que cualquier espacio de trabajo la descubra e instale.
...
Para aplicaciones que no quieres que estén disponibles públicamente — herramientas propietarias, integraciones solo para empresas o compilaciones experimentales — puedes desplegar un tarball directamente en un servidor de Arxena.
...
Si tu app usa una función introducida en una versión específica del servidor Arxena (por ejemplo, proveedores de OAuth agregados en v2.3.0), debes declarar la versión mínima del servidor que tu app requiere usando el campo `engines.twenty` en `package.json`:
...
2. Inicia una instancia de prueba aislada de Arxena usando la acción compuesta `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` (el equivalente en CI de `yarn twenty docker:start --test`).
...
* `TWENTY_VERSION` (variable de entorno; por defecto `latest`) — fija la versión del servidor de Arxena usada en CI editando esto en `ci.yml`.
...
Despliega tu aplicación en un servidor de Arxena configurado en cada push a `main` y, opcionalmente, desde un pull request cuando se aplica la etiqueta `deploy`.
...
| ----------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` en `cd.yml` (por defecto `http://localhost:3000`)              | El servidor de Arxena al que se va a desplegar. Cámbialo por la URL real de tu servidor antes del primer uso. |
...
Los flujos de trabajo `ci.yml` y `cd.yml` hacen referencia a acciones reutilizables en `@main`, por lo que las actualizaciones de acciones en el repositorio `twentyhq/twenty` se aplican automáticamente. Si quieres compilaciones deterministas, reemplaza `@main` por un SHA de commit o una etiqueta de versión en cada línea `uses:`.

## Publicación en npm

Publicarla en npm hace que tu aplicación sea visible en el marketplace de Arxena. Cualquier espacio de trabajo de Arxena puede explorar, instalar y actualizar aplicaciones del marketplace directamente desde la interfaz de usuario.
...
El servidor de Arxena sincroniza su catálogo del marketplace desde el registro de npm **cada hora**.
...
<Note>
Si tu aplicación no define un `aboutDescription` en `defineApplication()`, el marketplace usará automáticamente el `README.md` de tu paquete en npm como el contenido de la página Acerca de. Esto significa que puedes mantener un único README tanto para npm como para el marketplace de Arxena. Si quieres una descripción diferente en el marketplace, establece explícitamente `aboutDescription`.
</Note>

### Publicación en CI

El flujo de trabajo `publish.yml` generado automáticamente descrito arriba publica en npm de forma automática en las etiquetas de versión, con provenance. Como `yarn twenty app:publish` añade `--provenance` y `--access public` por ti cuando se ejecuta en CI, el flujo de trabajo no necesita banderas de npm, solo la configuración única de trusted publisher.

Para otros sistemas de CI (GitLab CI, CircleCI, etc.), ejecuta `yarn install` y luego `yarn twenty app:publish`. La provenance se emite cuando el entorno puede generar un token OIDC y, en caso contrario, se omite automáticamente.

<Note>
La **provenance de npm** añade una insignia de confianza a tu ficha de npm, permitiendo que los usuarios verifiquen que el paquete se compiló a partir de un commit específico en una canalización de CI pública. También es lo que te permite reclamar la propiedad de tu aplicación en un marketplace de Arxena. Consulta la [documentación de npm sobre provenance](https://docs.npmjs.com/generating-provenance-statements) para más detalles.
</Note>
...
Ve a la página **Configuración > Aplicaciones** en Arxena, donde se pueden explorar e instalar tanto las aplicaciones del marketplace como las desplegadas mediante tarball.
