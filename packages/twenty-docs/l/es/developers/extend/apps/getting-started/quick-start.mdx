...
title: Inicio rápido
icon: rocket
description: Crea tu primera aplicación de Arxena en minutos.
---
...
* **Docker** — [Descargar](https://www.docker.com/products/docker-desktop/). Necesario para ejecutar un servidor local de Arxena. Omítelo si ya tienes Arxena ejecutándose en otro lugar.

La creación de una app de Arxena tiene tres fases. El generador las combina en un único comando de ruta ideal, pero cada fase es un concepto independiente — cuando algo falla, saber en qué fase estás te indica qué debes corregir.
...
| **2. Ejecutar un servidor** | Inicia un servidor de Arxena con el que sincronizar | Docker + `yarn twenty docker:start` | Una instancia de Arxena en ejecución |
...
## Fase 2 — Ejecuta un servidor local de Arxena

Tu app necesita un servidor de Arxena con el que sincronizar. El servidor es una instancia completa de Arxena — UI, API GraphQL, PostgreSQL — ejecutándose localmente en Docker. Tu código local sube sus definiciones a ese servidor, lo que hace que aparezcan en la UI.
...
Para conectarte a un servidor Arxena existente en su lugar, pasa `--url \<your-server-url>`. Los servidores remotos se autentican con OAuth: se abre un navegador para que puedas iniciar sesión y hacer clic en **Authorize**, lo que le da a la CLI acceso a tu espacio de trabajo. (También puedes optar por usar OAuth localmente con `--authentication-method oauth`: inicia sesión con `tim@apple.dev` / `tim@apple.dev`.)

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Pantalla de inicio de sesión de Arxena" />
</div>

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Pantalla de autorización de la CLI de Arxena" />
</div>
...
**Después de esta fase:** tienes un servidor de Arxena en ejecución en [http://localhost:2020](http://localhost:2020) con tu CLI autorizada para sincronizar con él.
...
| **Componentes de frontend** | Componentes de React que se renderizan dentro de la interfaz de Arxena (panel lateral, widgets, menú de comandos)         |
