...
description: Qué hay dentro de una app de Arxena generada mediante scaffolding — archivos, carpetas y lo que hace cada uno.
icon: folder-tree
---
...
Ambos paquetes del SDK de Arxena pertenecen a `devDependencies`, no a `dependencies`:
...
* **`twenty-client-sdk`** *sí* es importado por el código de tu app (`CoreApiClient`, `MetadataApiClient`, `RestApiClient`), pero Arxena lo proporciona en tiempo de ejecución: las funciones lógicas lo obtienen de una capa SDK generada y los componentes de front lo resuelven desde módulos servidos por el servidor. Tu copia instalada solo se utiliza para la comprobación de tipos y el build en tiempo de despliegue, por lo que nunca necesita incluirse en el bundle desplegado.
