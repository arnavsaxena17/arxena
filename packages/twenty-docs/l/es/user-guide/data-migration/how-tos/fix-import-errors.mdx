...
Después de cargar tu archivo y asignar las columnas, Arxena valida tus datos:

1. **Se ejecuta la validación** — Arxena verifica cada fila en busca de errores
...
| ----------------------------------------------------------------------------- | -------------------------------------------------------------- |
| Valores duplicados resaltados en amarillo                                     | El valor ya existe en Arxena o aparece dos veces en tu archivo |
...
**Opción 3: deja que Arxena actualice el registro existente**

1. Asegúrate de que tu archivo incluya un identificador único (`email`, `domain` o `id`)
2. Asigna el campo de identificador único
3. Arxena actualizará el registro existente en lugar de crear un duplicado
...
Siempre que uno de los identificadores únicos coincida, Arxena actualiza el registro.
</Note>
...
3. Comprueba si los registros ya existen en Arxena
...
Esto significa que no hay ninguna empresa en Arxena con ese identificador específico.
...
Estás intentando vincular a un registro que no existe en Arxena.
...
No se puede asignar una columna de tu archivo porque el campo no existe en Arxena.
...
1. **El usuario no ha aceptado su invitación** — el usuario aún no existe en Arxena
2. **Uso del ID de usuario del sistema anterior** — Arxena no puede hacer coincidir IDs de otro sistema
3. **Formato de correo electrónico incorrecto** — el correo no coincide con la cuenta del usuario en Arxena

### Cómo solucionarlo

1. Asegúrate de que todos los usuarios hayan **aceptado su invitación** a tu espacio de trabajo de Arxena
2. Usa la **dirección de correo electrónico** del usuario (no su nombre ni el ID del sistema anterior)
3. Usa el mismo correo que utilizaron para unirse an Arxena
...
<Check>El formato de dominio coincide exactamente con el formato de Arxena</Check>
...
1. **Descarga el archivo de ejemplo** — ve el formato exacto que espera Arxena
