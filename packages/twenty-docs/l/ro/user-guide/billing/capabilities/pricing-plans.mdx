---
title: Planuri tarifare
description: Aflați despre planurile tarifare ale Arxena și cum să comutați între ele.
---

## Prezentare generală

Arxena oferă prețuri flexibile care se potrivesc echipelor de orice dimensiune, fie că preferați găzduirea în cloud sau autogăzduirea.

## Planuri Cloud

### Pro (Cloud)

Pentru echipe pregătite să se extindă:

* Toate funcționalitățile CRM de bază
* Sincronizare email și calendar
* Fluxuri de lucru și automatizări
* Asistență standard

<Note>
Funcționalitățile Premium (SSO, permisiuni la nivel de rând, date privind utilizarea AI și altele) nu sunt incluse în planul Pro.
</Note>

### Organization (Cloud)

Pentru echipe mai mari cu nevoi avansate:

* Tot ce include Pro
* **Funcționalități Premium**: integrare SSO, permisiuni la nivel de rând, date privind utilizarea AI și altele
* Suport prioritar

## Planuri autogăzduite

### Gratuit (Autogăzduit)

Găzduiți Arxena pe propria infrastructură, fără costuri:

* Toate funcționalitățile Pro incluse
* Asistență din partea comunității prin Discord
* Control deplin asupra datelor dumneavoastră

### Organization (Autogăzduit)

Pentru echipe care au nevoie de funcționalități premium în timp ce se autogăzduiesc:

* Toate funcționalitățile Pro
* **Funcționalități Premium**: integrare SSO, permisiuni la nivel de rând, date privind utilizarea AI și altele
* Asistență din partea echipei Arxena
* Fără obligația de a publica codul personalizat ca open-source înainte de distribuire

## Funcționalități premium

Funcționalitățile premium sunt disponibile doar în planurile Organization (Cloud sau Autogăzduit):

* **Integrare SSO**: autentificare unică (Single Sign-On) cu furnizorul dumneavoastră de identitate
* **Permisiuni la nivel de rând**: control de acces granular la nivel de înregistrare
* **Date privind utilizarea AI**: Urmăriți consumul de AI în spațiul de lucru
* **Jurnale de audit**: Verificați ce a făcut fiecare membru al spațiului de lucru
* **Campanii prin email**: Planificați ce emailuri să fie trimise (disponibil în curând)
* **Spații de lucru nelimitate**: Creați câte spații de lucru doriți (numai pentru auto-găzduire)
* **Cod sursă privat**: Fără obligația de a publica modificările într-un depozit public (numai pentru auto-găzduire)

## Schimbarea planurilor

### Faceți upgrade la Organization

1. Accesați **Setări → Facturare → Planuri**
2. Faceți clic pe **Schimbă la Organization**
3. Confirmați upgrade-ul

### Faceți downgrade la Pro

1. Accesați **Setări → Facturare → Planuri**
2. Faceți clic pe **Treceți la Pro**
3. Confirmați retrogradarea

### Treceți la facturarea anuală

1. Accesați **Setări → Facturare**
2. Faceți clic pe **Treceți la anual**
3. Economisiți cu facturarea anuală

### Treceți la facturarea lunară

1. Accesați **Setări → Facturare**
2. Faceți clic pe **Treceți la lunar**
3. Economisiți cu facturarea lunară

## Obțineți o cheie Enterprise pentru Organization (Self-Hosted)

Pentru a utiliza planul Organization (Self-Hosted), trebuie să obțineți o cheie Enterprise:

1. Mergeți la **Setări → Panou Admin → Enterprise**

<img src="/images/user-guide/billing/enterprise-key.png" alt="Cheie Enterprise" />

2. Faceți clic pe **Obțineți cheia Enterprise**
3. Când sunteți redirecționați către Stripe, introduceți detaliile de plată și confirmați
4. Când este afișată cheia Enterprise, lipiți-o în pagina de setări Enterprise și activați licența organizației
