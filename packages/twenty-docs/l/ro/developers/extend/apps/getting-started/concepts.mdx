---
title: Concepte
description: Cum funcționează aplicațiile Arxena — modelul de entități, izolarea (sandboxing) și ciclul de viață al instalării.
icon: sitemap
---

Aplicațiile Arxena sunt pachete TypeScript care vă extind spațiul de lucru cu obiecte personalizate, logică, componente UI și capabilități AI. Acestea rulează pe platforma Arxena, cu izolare completă (sandboxing) și controale de permisiuni.
...
| **Componentă front-end**          | Interfață React izolată (sandboxed) în Arxena              | [Componente front-end](/l/ro/developers/extend/apps/layout/front-components)       |
...
* **Componentele front-end** rulează în Web Workers folosind Remote DOM — izolate de pagina principală, dar randând elemente DOM native (nu iframes). Acestea comunică cu Arxena printr-un API al gazdei bazat pe transmiterea de mesaje.
...
* **`yarn twenty dev`** — monitorizează fișierele sursă și sincronizează în timp real modificările către un server Arxena conectat. Clientul API tipizat este regenerat automat atunci când schema se schimbă.
