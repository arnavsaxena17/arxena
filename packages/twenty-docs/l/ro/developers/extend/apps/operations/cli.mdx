...
| `docker:*`                                      | Administrează containerul serverului Arxena local                                         | [Server local](/l/ro/developers/extend/apps/getting-started/local-server)                                       |
...
<Note>
Acest lucru este diferit de `yarn twenty docker:logs`, care afișează jurnalele containerului Docker. `yarn twenty dev:function:logs` afișează jurnalele de execuție ale funcțiilor aplicației de pe serverul Arxena.
</Note>

## Generarea clientului tipizat (`yarn twenty dev:generate-client`)

Regenerați clientul tipizat al API-ului (`twenty-client-sdk`) din schema remote-ului activ, fără a construi sau sincroniza o aplicație. Folosiți-l pentru a obține un client tipizat în orice proiect — cum ar fi un serviciu backend aflat într-un repository separat — care comunică cu instanța dvs. Arxena:
...
Un „remote” este un server Arxena la care se conectează aplicația. În timpul configurării, Scaffolderul creează automat unul pentru dvs. Puteți adăuga mai multe remote-uri sau comuta între ele oricând.
