...
Fiecare obiect din Arxena este livrat cu un set de **câmpuri de sistem** pe care nu le declari tu însuți. Acestea sunt create automat de server atunci când obiectul este provizionat:
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` este disponibil începând cu versiunea 2.23 a `twenty-sdk` și necesită un server Arxena pe versiunea 2.23 sau o versiune ulterioară.
</Note>
...
## Obiecte standard Arxena

Pentru un obiect Arxena **standard** (Person, Company, Opportunity, …), nu trebuie să derivezi nimic: identificatorii câmpurilor de sistem sunt constante pre-calculate pe care le poți importa direct.
