...
Dacă aveți deja o instanță Arxena cu date de producție, **NU** urmați acești pași deoarece vor șterge permanent baza dvs. de date!

În timp ce instalați Arxena pentru prima dată, este posibil să doriți să schimbați parola implicită a bazei de date.
...
În timpul instalării Arxena, trebuie să aprovizionați baza de date postgres cu schemele, extensiile și utilizatorii corecți.
...
**Dacă nu funcționează:**
Investigați care procese consumă cea mai mare parte din RAM-ul mașinii dvs. La Arxena, am observat că unele extensii VScode consumau multă RAM, așa că le dezactivăm temporar.
...
* Deschideți aplicația Arxena, navigați la `/settings` și activați comutatorul **Avansat** în colțul din stânga jos al ecranului.
* Creați un nou webhook.
* Lipiți **Your Unique Webhook URL** în câmpul **Endpoint Url** din Arxena. Setați **Filtrele** pe `Companii` și `Create`.
...
Dacă rulați Arxena în spatele unui proxy invers și întâmpinați probleme de conexiune:
...
2. **Verificați Setările Proxy-ului Invers:**
   ...
   * Confirmați că proxy-ul invers transmite corect cererile către serverul Arxena.
   * Asigurați-vă că headerele precum `X-Forwarded-For` și `X-Forwarded-Proto` sunt setate corect.

3. **Reporniți Serviciile:**

   După ce faceți schimbări, reporniți atât proxy-ul invers, cât și containerele Arxena.
...
* Suport Comunitar:
  ...
  Contactați [comunitatea Arxena](https://github.com/twentyhq/twenty/issues) sau [canalele de suport](https://discord.gg/cx5n4Jzs57) pentru asistență.
