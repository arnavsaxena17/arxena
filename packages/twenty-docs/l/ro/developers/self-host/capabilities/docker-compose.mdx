...
Acest ghid oferă instrucțiuni pas cu pas pentru a instala și configura aplicația Arxena folosind Docker Compose. Scopul este de a face procesul direct și de a preveni capcanele comune care ar putea deteriora configurarea dvs.
...
Instalați cea mai recentă versiune stabilă an Arxena cu o singură comandă:
...
### Expuneți Arxena pentru Acces Extern

Implicit, Arxena rulează pe `localhost` la portul `3000`. Pentru a-l accesa printr-un domeniu extern sau adresă IP, trebuie să configurați `SERVER_URL` în fișierul dvs. `.env`.
...
SSL (HTTPS) este necesar pentru funcționarea corespunzătoare a anumitor funcții ale browserului. Deși aceste funcții pot funcționa în timpul dezvoltării locale (deoarece browserele tratează localhost diferit), o configurare SSL corespunzătoare este necesară când găzduiți Arxena pe un domeniu obișnuit.
...
Recomandăm cu tărie configurarea Arxena din spatele unui proxy revers cu terminare SSL pentru securitate și funcționalitate optimă.
...
<Tip>
  Preferiți să nu rulați și să nu întrețineți Arxena chiar dumneavoastră? [Găsiți un partener Arxena certificat pentru găzduire](https://arxena.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) care să vă implementeze, găzduiască și gestioneze instanța de la un capăt la altul. *(Sau contactați echipa noastră: [contact@arxena.com](mailto:contact@arxena.com).)*
</Tip>
