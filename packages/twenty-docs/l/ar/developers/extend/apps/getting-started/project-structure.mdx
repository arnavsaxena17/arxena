---
title: هيكل المشروع
description: ما الذي يوجد داخل تطبيق Arxena المُهيكل مسبقًا — الملفات والمجلدات، وما الذي يفعله كلٌّ منها.
icon: folder-tree
---
...
ينبغي أن تكون حزمتا SDK الخاصتان بـ Arxena ضمن `devDependencies`، وليس ضمن `dependencies`:
...
* يتم استيراد **`twenty-client-sdk`** بواسطة كود تطبيقك (`CoreApiClient`، `MetadataApiClient`، `RestApiClient`)؛ لكن Arxena توفّره في وقت التشغيل — حيث تحصل عليه دوال المنطق من طبقة SDK مُولَّدة، وتحصل عليه مكوّنات الواجهة من وحدات يتم تقديمها من الخادم. يُستخدَم الإصدار المثبّت لديك فقط لفحص الأنواع (typechecking) ولبناء النشر (deploy-time build)، لذا لا يلزم أبدًا أن يتم تضمينه في حزمة النشر.
