...
description: Guida passo-passo per migrare i tuoi dati da qualsiasi CRM an Arxena.
---

## Panoramica

Questa guida ti accompagna nella migrazione dei tuoi dati da qualsiasi CRM an Arxena. Il processo prevede la revisione dei tuoi dati, la preparazione del tuo spazio di lavoro Arxena, l'esportazione dal sistema attuale e l'importazione in Arxena.
...
Crea un documento di mappatura tra il tuo CRM attuale e Arxena:
...
* Il campo di destinazione di Arxena
...
## Passaggio 3: Configura il tuo spazio di lavoro Arxena

Prima di importare i dati, prepara il tuo spazio di lavoro Arxena:
...
Se i tuoi dati includono riferimenti a utenti (Proprietario dell'account, Assegnatario, ecc.), tali utenti devono esistere in Arxena prima dell'importazione. In caso contrario, tali relazioni non possono essere mappate.
</Warning>
...
Apri ogni file CSV esportato in un'applicazione di fogli di calcolo e preparalo per Arxena.

### Rimuovi i duplicati

1. Ordina per il campo univoco (email per Persone, dominio per Aziende)
2. Rimuovi o unisci le righe duplicate
3. Verifica che non esistano già duplicati in Arxena
...
Usa `https://domain.com` (non `domain.com` o `www.domain.com`). Questo corrisponde al formato di Arxena ed evita duplicati quando colleghi la sincronizzazione di email e calendario.
</Warning>
...
2. Usa gli stessi indirizzi email che gli utenti hanno utilizzato per unirsi al tuo spazio di lavoro Arxena
...
* Arxena inizierà a sincronizzare le email con i record dei contatti
...
<AccordionGroup>
  <Accordion title="Posso migrare i flussi di lavoro automaticamente?">
    Al momento no. I flussi di lavoro devono essere ricreati manualmente in Arxena.
  </Accordion>
...
<Tip>
  ...
  * **Fatto per te:** [trova un partner Arxena certificato](https://arxena.com/partners/list?categories=SOLUTIONING\&ref=docs-migrate-crm) per trasferire i tuoi dati, pipeline e utenti end-to-end.
  * **Pacchetto di onboarding:** una configurazione guidata di 4 ore con il nostro team — [contact@arxena.com](mailto:contact@arxena.com).
</Tip>
