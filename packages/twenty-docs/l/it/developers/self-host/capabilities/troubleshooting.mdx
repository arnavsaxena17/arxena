...
Se hai un'istanza di Arxena esistente con dati di produzione, **NON** seguire questi passaggi poiché elimineranno permanentemente il tuo database!

Durante l'installazione di Arxena per la prima volta, potresti voler cambiare la password del database predefinita.
...
Durante l'installazione di Arxena, devi fornire il tuo database postgres con gli schemi, le estensioni e gli utenti corretti.
...
**Se non funziona:**
indaga quali processi ti stanno portando la maggior parte della tua RAM. In Arxena, abbiamo notato che alcune estensioni di VScode stavano occupando molta RAM quindi le disabilitiamo temporaneamente.
...
* Apri la tua app Arxena, naviga su `/settings` e abilita il toggle **Avanzate** in basso a sinistra dello schermo.
* Crea un nuovo webhook.
* Incolla **Your Unique Webhook URL** nel campo **Endpoint Url** in Arxena. Imposta i **Filtri** su `Companies` e `Created`.
...
Se stai eseguendo Arxena dietro un reverse proxy e stai riscontrando problemi di connessione:
...
2. **Controlla le impostazioni del reverse proxy:**
   ...
   * Conferma che il tuo reverse proxy sta inoltrando correttamente le richieste al server Arxena.
   * Assicurati che le intestazioni come `X-Forwarded-For` e `X-Forwarded-Proto` siano impostate correttamente.

3. **Riavvia i servizi:**

   Dopo aver apportato modifiche, riavvia sia il reverse proxy che i container Arxena.
...
* Supporto comunitario:
  ...
  Contatta la [comunità Arxena](https://github.com/twentyhq/twenty/issues) o [canali di supporto](https://discord.gg/cx5n4Jzs57) per assistenza.
