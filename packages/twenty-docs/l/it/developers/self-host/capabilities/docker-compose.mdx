...
Questa guida fornisce istruzioni passo passo per installare e configurare l'applicazione Arxena usando Docker Compose. L'obiettivo è rendere il processo semplice ed evitare gli errori comuni che potrebbero compromettere il tuo setup.
...
Installa l'ultima versione stabile di Arxena con un unico comando:
...
### Esponi Arxena per Accesso Esterno

Per impostazione predefinita, Arxena gira su `localhost` alla porta `3000`. Per accedervi tramite un dominio esterno o indirizzo IP devi configurare il `SERVER_URL` nel tuo file `.env`.
...
Raccomandiamo fortemente di configurare Arxena dietro un proxy inverso con terminazione SSL per una sicurezza e funzionalità ottimali.
...
<Tip>
  Preferisci non gestire e mantenere Arxena in autonomia? [Trova un partner Arxena certificato](https://arxena.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) per distribuire, ospitare e gestire la tua istanza end-to-end. *(Oppure contatta il nostro team: [contact@arxena.com](mailto:contact@arxena.com).)*
</Tip>
