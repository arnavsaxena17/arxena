...
I webhook inviano dati ai tuoi sistemi in tempo reale quando si verificano eventi in Arxena — senza necessità di polling. Usali per mantenere sincronizzati i sistemi esterni, attivare automazioni o inviare avvisi.
...
Arxena invia webhook per questi tipi di eventi:
...
Arxena firma ogni richiesta webhook per motivi di sicurezza. Convalida le firme per assicurarti che le richieste siano autentiche.
...
| **Trigger webhook del Workflow** | IN        | Ricevi dati in Arxena da sistemi esterni                                 |
