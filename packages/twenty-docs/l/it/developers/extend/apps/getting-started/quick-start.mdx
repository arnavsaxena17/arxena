...
title: Avvio rapido
icon: rocket
description: Crea la tua prima app Arxena in pochi minuti.
---
...
* **Docker** — [Scarica](https://www.docker.com/products/docker-desktop/). Necessario per eseguire un server Arxena locale. Salta se hai già Arxena in esecuzione altrove.

La creazione di un'app Arxena ha tre fasi. Lo strumento di scaffolding le combina in un unico comando per il percorso ottimale, ma ogni fase è un concetto distinto — quando qualcosa fallisce, sapere in quale fase ti trovi indica cosa correggere.
...
| **2. Esegui un server** | Avvia un server Arxena con cui sincronizzare           | Docker + `yarn twenty docker:start` | Un'istanza Arxena in esecuzione    |
...
## Fase 2 — Esegui un server Arxena locale

La tua app ha bisogno di un server Arxena con cui sincronizzarsi. Il server è un'istanza Arxena completa — UI, API GraphQL, PostgreSQL — in esecuzione in locale su Docker. Il tuo codice locale carica le sue definizioni su quel server, che le rende visibili nell'UI.
...
**Dopo questa fase:** hai un server Arxena in esecuzione su [http://localhost:2020](http://localhost:2020) con la tua CLI autorizzata a sincronizzarsi con esso.
...
| **Componenti front-end** | Componenti React che vengono renderizzati all'interno dell'UI di Arxena (pannello laterale, widget, menu comandi) |
