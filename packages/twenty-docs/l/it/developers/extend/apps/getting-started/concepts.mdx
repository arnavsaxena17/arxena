---
title: Concetti
description: Come funzionano le app Arxena — modello di entità, sandboxing e ciclo di vita dell’installazione.
icon: sitemap
---

Le app di Arxena sono pacchetti TypeScript che estendono il tuo spazio di lavoro con oggetti personalizzati, logica, componenti dell'interfaccia utente (UI) e funzionalità di IA. Vengono eseguite sulla piattaforma Arxena con sandboxing completo e controlli delle autorizzazioni.
...
| **Componente front-end**         | UI React in sandbox all’interno di Arxena              | [Componenti front-end](/l/it/developers/extend/apps/layout/front-components)              |
...
* **I componenti front-end** vengono eseguiti in Web Workers utilizzando il Remote DOM — isolati dalla pagina principale ma renderizzando elementi DOM nativi (non iframe). Comunicano con Arxena tramite un'API host basata sul passaggio di messaggi.
...
* **`yarn twenty dev`** — monitora i file sorgente e sincronizza in tempo reale le modifiche su un server Arxena connesso. Il client API tipizzato viene rigenerato automaticamente quando lo schema cambia.
