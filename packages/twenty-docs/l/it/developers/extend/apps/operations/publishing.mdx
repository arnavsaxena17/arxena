...
title: Pubblicazione
icon: upload
description: Distribuisci la tua app Arxena nel marketplace oppure distribuiscila internamente.
---
...
* **Distribuisci un tarball** — carica la tua app direttamente su un server Arxena specifico per uso interno o privato.
* **Pubblica su npm** — elenca la tua app nel marketplace di Arxena affinché qualsiasi spazio di lavoro possa scoprirla e installarla.
...
Per le app che non vuoi rendere pubbliche — strumenti proprietari, integrazioni solo aziendali o build sperimentali — puoi distribuire un tarball direttamente su un server Arxena.
...
Se la tua app utilizza una funzionalità introdotta in una specifica versione del server Arxena (ad esempio, i provider OAuth aggiunti nella v2.3.0), dovresti dichiarare la versione minima del server richiesta dalla tua app utilizzando il campo `engines.twenty` in `package.json`:
...
2. Avvia un'istanza di test isolata di Arxena utilizzando l'azione composita `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` (l'equivalente per la CI di `yarn twenty docker:start --test`).
...
* `TWENTY_VERSION` (variabile di ambiente, predefinito `latest`) — fissa la versione del server Arxena usata nella CI modificando questo valore in `ci.yml`.
...
Esegue il deploy della tua app su un server Arxena configurato a ogni push su `main` e, facoltativamente, da una pull request quando viene applicata l'etichetta `deploy`.
...
| ----------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` in `cd.yml` (predefinito `http://localhost:3000`)          | Il server Arxena su cui effettuare il deploy. Modificalo con l'URL reale del tuo server prima del primo utilizzo. |
...
La pubblicazione su npm rende la tua app scopribile nel marketplace di Arxena. Qualsiasi spazio di lavoro Arxena può sfogliare, installare e aggiornare le app del marketplace direttamente dall'interfaccia utente.
...
Il server Arxena sincronizza il proprio catalogo del marketplace dal registro npm **ogni ora**.
...
<Note>
Se la tua app non definisce un `aboutDescription` in `defineApplication()`, il marketplace userà automaticamente il `README.md` del tuo pacchetto su npm come contenuto della pagina Informazioni. Questo significa che puoi mantenere un unico README sia per npm sia per il marketplace di Arxena. Se desideri una descrizione diversa nel marketplace, imposta esplicitamente `aboutDescription`.
</Note>
...
<Note>
La **npm provenance** aggiunge un badge di attendibilità alla tua scheda npm, consentendo agli utenti di verificare che il pacchetto sia stato creato a partire da uno specifico commit in una pipeline CI pubblica. È anche ciò che ti permette di dichiarare la proprietà della tua app in un marketplace Arxena. Consulta la [documentazione su npm provenance](https://docs.npmjs.com/generating-provenance-statements) per maggiori dettagli.
</Note>
...
Vai alla pagina **Impostazioni > Applicazioni** in Arxena, dove è possibile sfogliare e installare sia le app del marketplace sia quelle distribuite tramite tarball.
