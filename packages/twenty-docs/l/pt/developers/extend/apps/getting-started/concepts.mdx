---
title: Conceitos
description: Como as aplicações Arxena funcionam — modelo de entidade, sandboxing e ciclo de vida da instalação.
icon: sitemap
---

As aplicações Arxena são pacotes TypeScript que estendem seu espaço de trabalho com objetos personalizados, lógica, componentes de UI e recursos de IA. Elas são executadas na plataforma Arxena com sandboxing completo e controles de permissão.
...
| **Componente de front-end**   | UI React em sandbox dentro do Arxena                | [Componentes de front-end](/l/pt/developers/extend/apps/layout/front-components)        |
...
* **Componentes de front-end** executam em Web Workers usando Remote DOM — isolados da página principal, mas renderizando elementos DOM nativos (não iframes). Eles se comunicam com o Arxena por meio de uma API de host com passagem de mensagens.
...
* **`yarn twenty dev`** — observa seus arquivos-fonte e sincroniza ao vivo as alterações com um servidor Arxena conectado. O cliente de API tipado é regenerado automaticamente quando o esquema muda.
