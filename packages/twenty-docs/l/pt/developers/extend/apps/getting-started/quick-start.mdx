...
title: Início rápido
icon: rocket
description: Crie seu primeiro app do Arxena em minutos.
---
...
* **Docker** — [Baixar](https://www.docker.com/products/docker-desktop/). Necessário para executar um servidor Arxena local. Ignore se você já tiver o Arxena em execução em outro lugar.

A criação de um aplicativo Arxena tem três fases. A ferramenta de scaffolding as reúne em um único comando do fluxo ideal, mas cada fase é um conceito separado — quando algo falha, saber em que fase você está indica o que corrigir.
...
| **2. Executar um servidor** | Iniciar um servidor Arxena para o qual sincronizar | Docker + `yarn twenty docker:start` | Uma instância Arxena em execução      |
...
## Fase 2 — Executar um servidor Arxena local

Seu aplicativo precisa de um servidor Arxena para o qual sincronizar. O servidor é uma instância completa do Arxena — interface, API GraphQL, PostgreSQL — executando localmente no Docker. Seu código local envia suas definições para esse servidor, o que faz com que elas apareçam na interface.
...
Para se conectar a um servidor Arxena existente em vez disso, passe `--url \<your-server-url>`. Servidores remotos se autenticam com OAuth: um navegador é aberto para que você faça login e clique em **Authorize**, o que dá à CLI acesso ao seu workspace. (Você também pode optar por usar OAuth localmente com `--authentication-method oauth` — faça login com `tim@apple.dev` / `tim@apple.dev`.)

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Tela de login do Arxena" />
</div>

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Tela de autorização da CLI do Arxena" />
</div>
...
**Após esta fase:** você tem um servidor Arxena em execução em [http://localhost:2020](http://localhost:2020) com sua CLI autorizada a sincronizar com ele.
...
| **Componentes de front-end** | Componentes React que são renderizados na UI do Arxena (painel lateral, widgets, menu de comandos)                 |
