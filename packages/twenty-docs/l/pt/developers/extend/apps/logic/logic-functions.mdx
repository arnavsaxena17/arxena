...
As funções de lógica são funções TypeScript no lado do servidor que são executadas na plataforma Arxena. Elas podem ser acionadas por solicitações HTTP, agendamentos cron ou eventos de banco de dados — e também podem ser expostas como ferramentas para agentes de IA.
...
* **`toolTriggerSettings`** — torna a função disponível para os recursos de IA do Arxena (chat, MCP, chamadas de função). Usa o JSON Schema padrão, o formato que os LLMs entendem nativamente.
* **`workflowActionTriggerSettings`** — torna a função visível como uma etapa no construtor visual de fluxos de trabalho. Usa o `InputSchema` avançado do Arxena para que o construtor possa renderizar editores de campo adequados, seletores de variáveis e rótulos.
...
* `toolTriggerSettings.inputSchema` e `workflowActionTriggerSettings.inputSchema` são opcionais. Quando omitidos, o construtor de manifestos os infere a partir do código-fonte do handler (JSON Schema para a ferramenta de IA, `InputSchema` do Arxena para a ação de fluxo de trabalho). Forneça um explicitamente quando quiser uma tipagem mais rica — por exemplo, com campos compatíveis com `FieldMetadataType`, como `CURRENCY` ou `RELATION` para o construtor de fluxos de trabalho, ou com campos `description` que o agente de IA pode ler:
...
Para declarar seus parâmetros **uma vez** e atender a ambas as interfaces, defina um único JSON Schema (`InputJsonSchema`) e converta-o para a ação de fluxo de trabalho com `jsonSchemaToInputSchema` de `twenty-sdk/logic-function`. `toolTriggerSettings.inputSchema` recebe o JSON Schema diretamente, enquanto `workflowActionTriggerSettings.inputSchema` espera o `InputSchema` da Arxena:
...
| `inputSchema`  | O rico `InputSchema` da Arxena — o que o construtor renderiza como campos configuráveis (com seletores de variáveis). Opcional; inferido a partir do handler quando omitido.           |
...
O pacote `twenty-client-sdk` fornece dois clientes GraphQL tipados para interagir com a API do Arxena a partir das suas funções de lógica e componentes de front-end.
...
<Note>
  Quando seu código é executado no Arxena (funções de lógica ou componentes de front-end), a plataforma injeta credenciais como variáveis de ambiente:

  * `TWENTY_API_URL` — URL base da API do Arxena
