...
* Use `serverVariables` (não `applicationVariables`) — as credenciais OAuth são do servidor como um todo e há um app OAuth por servidor do Arxena.
...
4. Abra o app instalado no Arxena como administrador do servidor → defina os valores nos `serverVariables` correspondentes.
