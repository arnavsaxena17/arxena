...
Todo objeto no Arxena é fornecido com um conjunto de **campos de sistema** que você nunca declara manualmente. Eles são criados automaticamente pelo servidor quando o objeto é provisionado:
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` está disponível a partir da versão 2.23 do `twenty-sdk` e requer um servidor Arxena na versão 2.23 ou posterior.
</Note>
...
## Objetos Padrão do Arxena

Para um objeto **padrão** do Arxena (Person, Company, Opportunity, …), você não precisa derivar nada: os identificadores de campos de sistema são constantes pré-computadas que você pode importar diretamente.
