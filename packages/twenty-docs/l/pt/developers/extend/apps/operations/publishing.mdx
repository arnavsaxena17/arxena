...
title: Publicação
icon: upload
description: Distribua seu aplicativo Arxena no Marketplace ou implante-o internamente.
---
...
* **Implantar um tarball** — envie seu aplicativo diretamente para um servidor Arxena específico para uso interno ou privado.
* **Publicar no npm** — liste seu aplicativo no Marketplace da Arxena para que qualquer espaço de trabalho possa descobrir e instalar.
...
Para aplicativos que você não quer disponibilizar publicamente — ferramentas proprietárias, integrações apenas para empresas ou builds experimentais — você pode implantar um tarball diretamente em um servidor Arxena.
...
Se o seu aplicativo usar um recurso introduzido em uma versão específica do servidor Arxena (por exemplo, provedores OAuth adicionados na v2.3.0), você deve declarar a versão mínima do servidor que seu aplicativo requer usando o campo `engines.twenty` em `package.json`:
...
2. Inicia uma instância de teste do Arxena isolada usando a ação composta `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` (o equivalente em CI de `yarn twenty docker:start --test`).
...
* `TWENTY_VERSION` (env, padrão `latest`) — fixe a versão do servidor Arxena usada no CI editando isto em `ci.yml`.
...
Faz o deploy do seu app para um servidor Arxena configurado a cada push para `main` e, opcionalmente, a partir de um pull request quando o rótulo `deploy` é aplicado.
...
| ----------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `TWENTY_DEPLOY_URL`     | `env` em `cd.yml` (padrão `http://localhost:3000`)                   | O servidor Arxena para o qual fazer o deploy. Altere isto para a URL real do seu servidor antes do primeiro uso. |
...
Publicar no npm torna seu aplicativo descobrível no Marketplace da Arxena. Qualquer espaço de trabalho da Arxena pode navegar, instalar e atualizar aplicativos do Marketplace diretamente pela UI.
...
O servidor Arxena sincroniza seu catálogo do marketplace a partir do registro do npm **a cada hora**.
...
<Note>
A **proveniência do npm** adiciona um selo de confiança à sua listagem no npm, permitindo que os usuários verifiquem que o pacote foi construído a partir de um commit específico em um pipeline de CI público. É também o que permite que você reivindique a propriedade do seu app em um marketplace da Arxena. Consulte a [documentação de proveniência do npm](https://docs.npmjs.com/generating-provenance-statements) para mais detalhes.
</Note>
...
Vá para a página **Configurações > Aplicações** no Arxena, onde é possível navegar e instalar tanto apps do marketplace quanto apps implantados por tarball.
