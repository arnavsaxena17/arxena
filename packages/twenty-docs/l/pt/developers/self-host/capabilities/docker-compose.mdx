...
Este guia fornece instruções passo a passo para instalar e configurar o aplicativo Arxena usando Docker Compose. O objetivo é tornar o processo simples e evitar armadilhas comuns que possam quebrar sua configuração.
...
Instale a versão estável mais recente do Arxena com um único comando:
...
### Expor o Arxena para Acesso Externo

Por padrão, o Arxena é executado em `localhost` na porta `3000`. Para acessá-lo via um domínio externo ou endereço IP, você precisa configurar o `SERVER_URL` no seu arquivo `.env`.
...
Recomendamos fortemente configurar o Arxena atrás de um proxy reverso com terminação SSL para segurança e funcionalidade ótimas.
...
<Tip>
  Prefere não executar e manter o Arxena você mesmo? [Encontre um parceiro Arxena certificado de hospedagem](https://arxena.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) para implantar, hospedar e gerir a sua instância de ponta a ponta. *(Ou entre em contato com a nossa equipe: [contact@arxena.com](mailto:contact@arxena.com).)*
</Tip>
