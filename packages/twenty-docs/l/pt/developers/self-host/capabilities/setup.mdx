...
<Warning>
**Primeira vez instalando?** Siga o [guia de instalação do Docker Compose](/l/pt/developers/self-host/capabilities/docker-compose) para iniciar o Arxena, depois retorne aqui para configurar.
</Warning>

Arxena oferece **dois modos de configuração** para atender diferentes necessidades de implantação:
...
1. Acesse sua instância do Arxena (geralmente `http://localhost:3000`)
...
An Arxena usa duas chaves de criptografia definidas apenas por variáveis de ambiente:
...
Para compatibilidade retroativa, se `ENCRYPTION_KEY` não estiver definida, Arxena recorre a `APP_SECRET` para criptografia em repouso — correspondendo ao comportamento legado de implantações mais antigas. Novas instalações devem sempre definir uma `ENCRYPTION_KEY` dedicada.
...
Por padrão, o Arxena é executado no **modo de espaço de trabalho único** — ideal para a maioria das implantações auto-hospedadas em que você precisa de uma instância de CRM para sua organização.
...
* Um espaço de trabalho por instância do Arxena
...
Ative o modo de vários espaços de trabalho para implantações ao estilo SaaS em que várias equipes independentes precisam de seus próprios espaços de trabalho na mesma instância do Arxena.
...
<Warning>
Os usuários devem ter uma [Licença do Microsoft 365](https://admin.microsoft.com/Adminportal/Home) para poder usar as APIs de Calendário e Mensagem. Eles não poderão sincronizar sua conta no Arxena sem uma.
</Warning>
...
Ao usar o S3 com recursos dependentes de CORS (por exemplo, downloads de arquivos no navegador), verifique se o seu bucket permite a origem do seu frontend do Arxena na sua configuração de CORS.

## Funções lógicas e interpretador de código

O Arxena oferece suporte a funções lógicas para fluxos de trabalho e ao interpretador de código para análise de dados com IA. Ambos executam código fornecido pelo usuário e exigem configuração explícita por motivos de segurança.
...
<Note>
Ao usar `LOGIC_FUNCTION_TYPE=DISABLED` ou `CODE_INTERPRETER_TYPE=DISABLED`, qualquer tentativa de execução retornará um erro. Isso é útil se você quiser executar o Arxena sem esses recursos.
</Note>
