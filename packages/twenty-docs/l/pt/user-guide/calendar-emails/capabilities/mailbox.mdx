...
description: Entendendo os recursos de integração de e-mail no Arxena.
---
...
Arxena vincula automaticamente os emails de suas caixas de correio conectadas aos registros de CRM relevantes, mantendo todo o histórico de comunicação em um só lugar.
...
Arxena importa emails a cerca de **400 mensagens por minuto**. Atualmente, essa taxa é limitada pela limitação de taxa da API do Gmail (e aplica-se de forma semelhante a outros provedores).
...
Escolha se os e-mails internos são sincronizados no Arxena:
...
* **Emails Ausentes**: Verifique se:
  ...
  * A criação automática de contatos está desativada (os emails precisam de registros existentes no Arxena)
