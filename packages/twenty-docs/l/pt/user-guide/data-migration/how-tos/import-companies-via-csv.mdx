...
description: Guia completo passo a passo para importar empresas para o Arxena.
---

## Visão Geral

Este guia explica como importar as suas empresas para o Arxena. **As empresas devem ser importadas primeiro** porque Pessoas e Oportunidades se ligam a Empresas.
...
<Note>
**Dica profissional:** Clique em **Download sample file** durante a importação para ver os nomes de colunas exatos que o Arxena espera.
</Note>
...
1. O Arxena mostra os seus valores ao lado das opções existentes
2. Relacione cada valor no seu ficheiro a uma opção do Arxena
...
Antes de concluir a importação, o Arxena valida os seus dados:
...
| Erro                            | Causa                          | Solução                                            |
| ------------------------------- | ------------------------------ | -------------------------------------------------- |
| **Domínio duplicado**           | O domínio já existe no Arxena  | Remova do ficheiro ou atualize o registo existente |
...
2. O Arxena faz a correspondência dos registos por este identificador único
...
<AccordionGroup>
  <Accordion title="Porque é que duas Empresas não podem ter o mesmo domínio?">
    O domínio é um identificador único no Arxena. Isto evita empresas duplicadas e garante que a sincronização de e-mail associe corretamente os e-mails à empresa certa.
  </Accordion>
  ...
  <Accordion title="O que acontece se importar um domínio que já existe?">
    Se incluir um identificador único (domínio ou id) que corresponda a uma empresa existente, an Arxena atualiza essa empresa em vez de criar um duplicado.
  </Accordion>
