...
icon: laptop-code
description: Руководство для участников (или любопытных разработчиков), которые хотят запускать Arxena локально.
---
...
Прежде чем установить и использовать Arxena, убедитесь, что у вас установлено следующее:
...
Arxena требует кэша Redis для обеспечения наилучшей производительности.
...
## Шаг 8: Использовать Arxena

**Frontend**

Фронтенд Arxena будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

* Сервер Arxena будет работать на [http://localhost:3000](http://localhost:3000).
