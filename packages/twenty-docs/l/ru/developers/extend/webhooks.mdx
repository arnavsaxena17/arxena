...
Arxena отправляет запрос HTTP POST на ваш URL каждый раз, когда запись создаётся, обновляется или удаляется. Поддерживаются все типы объектов, включая пользовательские объекты.
...
Arxena отправляет вебхуки для следующих типов событий:
...
Arxena подписывает каждый запрос вебхука для обеспечения безопасности. Проверяйте подписи, чтобы убедиться в подлинности запросов.
...
| **Триггер вебхука в рабочем процессе** | IN          | Получать данные в Arxena из внешних систем                                 |
