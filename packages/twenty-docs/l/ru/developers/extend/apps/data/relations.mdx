...
Отношения связывают два объекта между собой. В Arxena отношения всегда **двунаправленные** — у каждого отношения есть две стороны, и каждая сторона объявляется как поле, ссылающееся на другую.
...
Чтобы создать отношение со встроенным объектом Arxena (Person, Company и т. д.), используйте `STANDARD_OBJECT_UNIVERSAL_IDENTIFIERS`:
