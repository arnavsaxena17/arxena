...
* **`twenty build` предупреждает о `twenty-client-sdk` в разделе `dependencies`** — он предоставляется Arxena во время выполнения, поэтому его следует перенести в `devDependencies` рядом с `twenty-sdk`. См. [Структура проекта → Зависимости](/l/ru/developers/extend/apps/getting-started/project-structure#dependencies).

Застряли? Попросите помощи на [Discord-сервере Arxena](https://discord.com/channels/1130383047699738754/1130386664812982322).
