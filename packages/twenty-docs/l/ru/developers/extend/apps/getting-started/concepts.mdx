---
title: Понятия
description: Как работают приложения Arxena — модель сущностей, изоляция в песочнице и жизненный цикл установки.
icon: sitemap
---

Приложения Arxena — это пакеты TypeScript, которые расширяют ваше рабочее пространство пользовательскими объектами, логикой, компонентами интерфейса и возможностями ИИ. Они работают на платформе Arxena с полной изоляцией в песочнице и контролем прав доступа.
...
| **Компонент фронтенда**   | Изолированный в песочнице интерфейс React внутри Arxena | [Компоненты фронтенда](/l/ru/developers/extend/apps/layout/front-components)         |
...
* **Компоненты фронтенда** запускаются в Web Workers с использованием Remote DOM — изолированы от основной страницы, но при этом рендерят нативные элементы DOM (не iframes). Они взаимодействуют с Arxena через хостовый API обмена сообщениями.
...
* **`yarn twenty dev`** — следит за исходными файлами и синхронизирует изменения в реальном времени с подключённым сервером Arxena. Типизированный клиент API автоматически пересоздаётся при изменении схемы.
