...
export default defineApplication({
  universalIdentifier: '...',
  displayName: 'Linear',
  description: 'Connect Linear to Arxena.',
  // OAuth client credentials live on the app registration (one OAuth app per
  // Arxena server, configured by the admin) — not per-workspace. Declare them
...
* Используйте `serverVariables` (не `applicationVariables`) — учетные данные OAuth являются общими для сервера, и на каждом сервере Arxena используется одно приложение OAuth.
...
4. Откройте установленное приложение в Arxena под учетной записью администратора сервера → задайте значения в соответствующих `serverVariables`.
