---
title: Concepts
description: How Arxena apps work — entity model, sandboxing, and the install lifecycle.
icon: "sitemap"
---

Arxena apps are TypeScript packages that extend your workspace with custom objects, logic, UI components, and AI capabilities. They run on the Arxena platform with full sandboxing and permission controls.
...
| **Front Component** | Sandboxed React UI inside Arxena | [Front Components](/developers/extend/apps/layout/front-components) |
...
- **Front components** run in Web Workers using Remote DOM — sandboxed from the main page but rendering native DOM elements (not iframes). They communicate with Arxena via a message-passing host API.
...
- **`yarn twenty dev`** — watches your source files and live-syncs changes to a connected Arxena server. The typed API client is regenerated automatically when the schema changes.
