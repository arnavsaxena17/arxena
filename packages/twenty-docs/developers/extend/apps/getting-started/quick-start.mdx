...
title: Quick Start
icon: "rocket"
description: Create your first Arxena app in minutes.
---
...
- **Docker** — [Download](https://www.docker.com/products/docker-desktop/). Needed to run a local Arxena server. Skip if you already have Arxena running elsewhere.

Building an Arxena app has three phases. The scaffolder collapses them into one happy-path command, but each phase is a separate concept — when something fails, knowing which phase you're in tells you what to fix.
...
| **2. Run a server** | Start an Arxena server to sync into | Docker + `yarn twenty docker:start` | A running Arxena instance |
...
## Phase 2 — Run a local Arxena server

Your app needs an Arxena server to sync into. The server is a full Arxena instance — UI, GraphQL API, PostgreSQL — running locally in Docker. Your local code uploads its definitions to that server, which makes them appear in the UI.
...
To connect to an existing Arxena server instead, pass `--url <your-server-url>`. Remote servers authenticate with OAuth: a browser opens so you can sign in and click **Authorize**, which gives the CLI access to your workspace. (You can also opt into OAuth locally with `--authentication-method oauth` — sign in with `tim@apple.dev` / `tim@apple.dev`.)

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/login.png" alt="Arxena login screen" />
</div>

<div style={{textAlign: 'center'}}>
  <img src="/images/docs/developers/extends/apps/authorize.png" alt="Arxena CLI authorization screen" />
</div>
...
**After this phase:** you have a running Arxena server at [http://localhost:2020](http://localhost:2020) with your CLI authorized to sync to it.
...
| **Front components** | React components that render inside Arxena's UI (side panel, widgets, command menu) |
