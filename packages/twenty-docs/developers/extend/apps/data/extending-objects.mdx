...
description: Add fields to standard Arxena objects (Person, Company, …) or to objects from other apps using defineField.
icon: "wand-magic-sparkles"
---

Use `defineField()` to add a field to an object you don't own — a standard Arxena object like Person or Company, or an object shipped by another installed app. Unlike inline fields declared inside [`defineObject`](/developers/extend/apps/data/objects), standalone fields require an `objectUniversalIdentifier` to specify which object they extend.
...
- `objectUniversalIdentifier` identifies the target object. For standard Arxena objects, import the constant from `twenty-sdk`:
