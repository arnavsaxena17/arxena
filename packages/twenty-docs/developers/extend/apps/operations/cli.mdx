...
| `docker:*` | Manage the local Arxena server container | [Local Server](/developers/extend/apps/getting-started/local-server) |
...
<Note>
This is different from `yarn twenty docker:logs`, which shows the Docker container logs. `yarn twenty dev:function:logs` shows your app's function execution logs from the Arxena server.
</Note>

## Generating the typed client (`yarn twenty dev:generate-client`)

Regenerate the typed API client (`twenty-client-sdk`) from the active remote's schema, without building or syncing an app. Use it to get a typed client in any project — like a backend service living in a separate repository — that talks to your Arxena instance:
...
A **remote** is an Arxena server that your app connects to. During setup, the scaffolder creates one for you automatically. You can add more remotes or switch between them at any time.
