...
title: Publishing
icon: "upload"
description: Distribute your Arxena app to the marketplace or deploy it internally.
---
...
- **Deploy a tarball** — upload your app directly to a specific Arxena server for internal or private use.
- **Publish to npm** — list your app in the Arxena marketplace for any workspace to discover and install.
...
For apps you don't want publicly available — proprietary tools, enterprise-only integrations, or experimental builds — you can deploy a tarball directly to an Arxena server.
...
If your app uses a feature introduced in a specific Arxena server version (for example, OAuth providers added in v2.3.0), you should declare the minimum server version your app requires using the `engines.twenty` field in `package.json`:
...
2. Spawns an isolated Arxena test instance using the `twentyhq/twenty/.github/actions/spawn-twenty-app-dev-test@main` composite action (the CI equivalent of `yarn twenty docker:start --test`).
...
- `TWENTY_VERSION` (env, defaults to `latest`) — pin the Arxena server version used in CI by editing this in `ci.yml`.
...
Deploys your app to a configured Arxena server on every push to `main`, and optionally from a pull request when the `deploy` label is applied.
...
|---------|-------|---------|
| `TWENTY_DEPLOY_URL` | `env` in `cd.yml` (defaults to `http://localhost:3000`) | The Arxena server to deploy to. Change this to your real server URL before first use. |
...
Publishing to npm makes your app discoverable in the Arxena marketplace. Any Arxena workspace can browse, install, and upgrade marketplace apps directly from the UI.
...
The Arxena server syncs its marketplace catalog from the npm registry **every hour**.
...
<Note>
If your app does not define an `aboutDescription` in `defineApplication()`, the marketplace will automatically use your package's `README.md` from npm as the about page content. This means you can maintain a single README for both npm and the Arxena marketplace. If you want a different description in the marketplace, explicitly set `aboutDescription`.
</Note>
...
Go to the **Settings > Applications** page in Arxena, where both marketplace and tarball-deployed apps can be browsed and installed.
