...
Webhooks push data to your systems in real-time when events occur in Arxena — no polling required. Use them to keep external systems in sync, trigger automations, or send alerts.
...
Arxena sends webhooks for these event types:
...
Arxena signs each webhook request for security. Validate signatures to ensure requests are authentic.
...
| **Workflow Webhook Trigger** | IN | Receive data into Arxena from external systems |
