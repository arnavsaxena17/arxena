...
<Warning>
**First time installing?** Follow the [Docker Compose installation guide](/developers/self-host/capabilities/docker-compose) to get Arxena running, then return here for configuration.
</Warning>

Arxena offers **two configuration modes** to suit different deployment needs:
...
1. Access your Arxena instance (usually `http://localhost:3000`)
...
Arxena uses two env-only encryption keys:
...
For backward compatibility, if `ENCRYPTION_KEY` is unset, Arxena falls back to `APP_SECRET` for at-rest encryption — matching the legacy behaviour for older deployments. New installs should always set a dedicated `ENCRYPTION_KEY`.
...
By default, Arxena runs in **single-workspace mode** — ideal for most self-hosted deployments where you need one CRM instance for your organization.
...
- One workspace per Arxena instance
...
Enable multi-workspace mode for SaaS-like deployments where multiple independent teams need their own workspaces on the same Arxena instance.
...
<Warning>
Users must have a [Microsoft 365 Licence](https://admin.microsoft.com/Adminportal/Home) to be able to use the Calendar and Messaging API. They will not be able to sync their account on Arxena without one.
</Warning>
...
<Warning>
By default, Arxena stores uploaded files on the local filesystem. For production deployments, use S3 or an S3-compatible service (MinIO, DigitalOcean Spaces, etc.) to ensure files persist across container restarts and scale across multiple server instances.
</Warning>
...
When using S3 with CORS-dependent features (e.g. in-browser file downloads), make sure your bucket allows your Arxena frontend origin in its CORS configuration.

## Logic Functions & Code Interpreter

Arxena supports logic functions for workflows and the code interpreter for AI data analysis. Both run user-provided code and require explicit configuration for security.
...
<Note>
When using `LOGIC_FUNCTION_TYPE=DISABLED` or `CODE_INTERPRETER_TYPE=DISABLED`, any attempt to execute will return an error. This is useful if you want to run Arxena without these capabilities.
</Note>
