...
| LinkedIn fetch CMIs | Command-menu **Fetch LinkedIn Messages / Posts / Profile** on candidate + person. Messages merge into `chatMessage` turns (deduped). Posts → `candidate.linkedinPosts` (+ `otherFields.linkedin_posts`). Profile → `candidate.linkedinProfile` (+ `otherFields.linkedin_profile`). Company profile CMI deferred (`FetchCompanyDetailsService` exists, no persist). Cmd `1785600000115`. | `LinkedinSelectionFetchService`, `ArxFetchLinkedinSelectionCommands`, `outreach-command-fields.data.ts`, `2-25-*-1785600000115-*` |
| Qualify Prospect CMI | Seeded command-menu **Qualify Prospect** on candidate + person (`ARX_QUALIFY_PROSPECT`). Headless action fetches LinkedIn profile, runs the same qualify prompt/schema as the sequencer AI node, and stamps `candidate.outreachProspectEnrichment` (no stage side effects). Person selection resolves via `peopleId`. Workspace cmd `1785600000114`. | `arxena-standard-command-menu-item.constant.ts`, `QualifyProspectService`, `ArxQualifyProspectCommand`, `2-25-*-1785600000114-*` |
...
| Outreach merged candidate sequencer (cut over) | `Outreach — Candidate Sequencer` merges Stage B + C. Cut over via `1785600000117`: QUEUED re-entry guard (`connectionSentAt` IS_EMPTY), deactivate B/C, rebind pause/experiments/`outreachWorkflowId`, activate Sequencer. | `outreach-workflow-graphs.ts`, `resolve-outreach-pause-resume-workflow-ids.util.ts`, `2-25-*-1785600000117-*` |
...
| People tab Date Added / Last Modified | Show `createdAt` / `updatedAt` on Outreach People HotTable (labels Date Added / Last Modified). Wire CRM candidate timestamps through live working set; always-show + commonColumns; also map `createdAt` in `ProcessedData`. | `OutreachPeoplePanel.tsx`, `useOutreachLiveWorkingSet.ts`, `TableColumns.tsx`, `ProcessedData.tsx`, `outreach-home.types.ts`, `fetch-outreach-project-candidates.ts` |
...
| Outreach home tabs: People default | Removed Workflow tab; default tab is People; tab order is Companies → People → Setup. Stage B workflow still auto-binds via `useOutreachWorkflowEmbed`. | `OutreachMainTabs.tsx`, `OutreachHomePage.tsx`, `useOutreachLiveWorkingSet.ts`, `outreach-home.types.ts` |
| Outreach home ICP helper crash | Local `isOutreachProject` in `useOutreachLiveWorkingSet` called `resolveOutreachConfigIcpSpecString` without importing it. `\|\|` short-circuit hid it when every project had `outreachWorkflowId` (e.g. Projective); Arxena workspaces with mixed projects crashed on filter. | `useOutreachLiveWorkingSet.ts` |
...
| Fix `projectIdAtom` TDZ shadow | Bad rename `const projectIdAtom = useAtomStateValue(projectIdAtom)` (and same for `tableStateAtom`) shadowed the atom import (TDZ crash). Restored value locals (`projectId` / `tableState`) at all siblings. | `GlobalArxJDUploadModal.tsx`, `ArxJDUploadModal.tsx`, `ProjectPage.tsx`, `DataTable.tsx`, candidate-table + arx command hooks, orgchart + spreadsheet-import |
| Outreach individual journey controls | Per-candidate Journey API + drawer Journey tab (pause/resume/snooze/skip/approve FORM); People stage filters + next-step from run progress; KPI strip; deferred resume via `outreachAnalytics.resumeAt` / `stageBeforeDefer` + cron | `outreach-candidate-journey.service.ts`, `CandidateOutreachJourneyTab.tsx`, `OutreachPeoplePanel.tsx`, `OutreachKpiStrip.tsx`, `outreach-deferred-resume*.ts`, `outreachAnalytics.ts` |
| Warm paths feature gate (default off) | `isWarmPathsEnabledEnv = false` in `twenty-shared/arx/warmPaths.ts`; front `isWarmPathsEnabledState` + `useWarmPathResolve` skips API; UI shows "Warm paths not enabled"; server `POST /warm-paths/resolve` → 403 when disabled. | `warmPaths.ts`, `isWarmPathsEnabledState.ts`, `useWarmPathResolve.ts`, `CandidateWarmPathTab.tsx`, `OrgChartWarmPathSummary.tsx`, `warm-paths.controller.ts` |
| Projects nav + outreach people empty | `get-all-projects` used full Project GraphQL with invalid `icpSpec` on Project + optional outreach columns → validation failed → `{ projects: [] }`. Nav query `graphqlToFindManyProjectsForNav` + paginated `fetchAllProjectEdgesForNav`. Outreach People tab used `useFindManyRecords` `limit: 100` → `get-candidates-by-project-id` REST (paginated server-side). | `queries.ts`, `candidate-workspace-graphql.service.ts`, `candidate-sourcing.controller.ts`, `fetch-outreach-project-candidates.ts`, `useOutreachLiveWorkingSet.ts` |
| Search skill ephemeral write contract | Preamble + people/companies/linkedin-harvest: mandatory `execute_tool` upsert after Find; field mapping tables; spill/cursor rules; Sales Nav list import recipe; companies learn_tools split Outreach vs CRM; data-manipulation scoped to CRM (not Redis upsert). | `contents/search-preamble.md`, `search-people.md`, `search-companies.md`, `search-linkedin-harvest.md`, `data-manipulation.md` |
...
| Ask AI enlarged `/chat` page (Twenty upstream) | Full-page chat at `AppPath.AiChat` (`/chat/:threadId?`) with centered composer + suggested prompts; expand from side panel via `useOpenAiChatPage`; legacy `/workspace-setup` redirects to `/chat`. Nav chat icon and **New chat** open the Ask AI side panel (not `/chat`); expand still goes full page. | `pages/ai-chat/AiChatPage.tsx`, `AiChatPageHeader.tsx`, `useIsAiChatComposerCentered.ts`, `AiChatEditorSection.tsx`, `SidePanelExpandAiChatButton.tsx`, `useOpenAiChatPage.ts`, `useProjectAiChatThreadToUrl.ts`, `MainNavigationDrawerTabsRow.tsx` |
...
| Outreach resume syncs in-flight run flow | Pause → edit/publish workflow (AI prompts, LinkedIn/WhatsApp/email send copy) → Resume now merges `workflowRun.state.flow` from latest published version (A/B experiment variant respected) before rescheduling pending steps; completed sends kept; changed AI draft prompts reset draft + downstream. | `merge-workflow-run-flow-from-version.util.ts`, `outreach-workflow-run-flow-sync.service.ts`, `outreach-project-outreach-control.service.ts` |
| Outreach resume kicks idle RUNNING runs | Resume only rescheduled `PENDING` steps; ~57 Harvest runs stuck with `Send LinkedIn connection` = `NOT_STARTED` after pre-steps `SUCCESS` (pause stopped job between steps). Resume now also enqueues `RunWorkflowJob` for runnable `NOT_STARTED` steps via `getRunnableStepIds`. | `outreach-project-outreach-control.service.ts` (`kickIdleRunningWorkflowRun`) |
...
| Workflow run Flow blank on null `connectedAccountId` | Completed DRAFT_EMAIL run had full `state` in DB but Flow tab blank: `parseWorkflowRunRecord` failed Zod (`connectedAccountId: z.string()` vs persisted `null`). Same for calendar-event settings. | `send-email-action-settings-schema.ts`, `create-calendar-event-action-settings-schema.ts`, `parseWorkflowRunRecord.test.ts` |
...
| Website domain `updatedByName` NOT NULL | `POST /website-tracker/domains` ORM insert set `createdBy` only. Direct insert skips GraphQL actor hooks, so `_websiteDomain.updatedByName` was null. Same gap on `websiteVisitor` insert. | `website-tracker.service.ts` |
| Website tracker nginx 405 | Settings + collect hit `https://arxena.arxena.com/website-tracker/*`. Prod nginx has `^~ /website/` (leads) which does not match `/website-tracker`, so GET served SPA `index.html` and POST/OPTIONS returned nginx 405. Add `location ^~ /website-tracker/` → `:3000` before `location /`. | `scripts/nginx/twenty-website-tracker-location.conf.snippet`, `docs/website-visitor-tracking.md` |
| Outreach command nginx SPA fallback | `/outreach-command/cache/{companies,people}` returned `index.html` (JSON parse `Unexpected token '<'`). Prefix missing from prod `twenty.conf` API regex. Add `\|outreach-command` to the regex. | `scripts/nginx/twenty-outreach-command-api-prefix.snippet` |
...
| GTM Stage B default workflow embed | Workflow tab always resolves / auto-creates `GTM Outreach — Per Candidate`, binds `Project.outreachWorkflowId` (new projects + missing binds). Prefill ICP kickoff stays conversational. | `useGtmWorkflowEmbed.ts`, `useGtmLiveWorkingSet.ts`, `gtm-command.constants.ts`, `GtmWorkflowPanel.tsx` |
...
| GTM multi-project scope | Shell scopes by `?projectId=` (Project.id). Companies via `gtmRunKey` (= project id / legacy slug); Candidates via `projectsId`. Dropped unused `Company.gtmProjectId`. Project picker + New project. | `useGtmLiveWorkingSet.ts`, `gtm-command-fields.data.ts`, `gtm-home/README.md` |
| GTM ephemeral companies | Cancelled projectCompany join. Companies tab = Redis cache `/outreach-command/cache/companies` per projectId. CRM Company only on people add/enroll. | `outreach-companies-cache.service.ts`, `OutreachCommandController`, `outreach-companies-cache.ts`, `useOutreachLiveWorkingSet.ts`, `useAddOutreachRecordsToCrm.ts` |
| GTM ICP onboarding Ask AI skill | Standard skill `gtm-icp-onboarding` + chat system-prompt example; `/gtm-home` PREFILL kickoff via `openAskAiPageWithPreprompt`; Workflow tab Ask AI prefills project context; workspace backfill cmd `1785600000012` | `skill-metadata/contents/gtm-icp-onboarding.md`, `standard-skill.constant.ts`, `chat-system-prompts.const.ts`, `GtmHomePage.tsx`, `gtm-home.types.ts`, `2-25-workspace-command-1785600000012-*` |
...
```bash
rg -n 'ARX_START_CHAT|ARX_CLONE_MULTIPLE|ARX_QUALIFY_PROSPECT|ARX_FETCH_LINKEDIN|arxena-standard-command-menu' packages/twenty-{server,front}/src
...
| Remap | Notes |
| --- | --- |
| `ARX_QUALIFY_PROSPECT` | Same prompt/schema as sequencer "Qualify prospect"; stamps `outreachProspectEnrichment` on candidate. Person CMI resolves via `peopleId`. REST `POST /outreach-command/qualify-prospect`. |
| `ARX_FETCH_LINKEDIN_MESSAGES` | Reuses `FetchLinkedinMessagesService` → `chatMessage` turns (dedupe). REST `…/fetch-linkedin-messages-for-selection`. |
| `ARX_FETCH_LINKEDIN_POSTS` | Unipile user posts → `candidate.linkedinPosts` + `otherFields.linkedin_posts`. |
| `ARX_FETCH_LINKEDIN_PROFILE` | Unipile profile → `candidate.linkedinProfile` + `otherFields.linkedin_profile` (prospect; not workspaceMember sender cache). |
