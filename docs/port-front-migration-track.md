...
| GTM ICP onboarding Ask AI skill | Standard skill `gtm-icp-onboarding` + chat system-prompt example; `/gtm-home` auto-SEND kickoff via `openAskAiPageWithPreprompt`; Workflow tab Ask AI prefills run context; workspace backfill cmd `1785600000012` | `skill-metadata/contents/gtm-icp-onboarding.md`, `standard-skill.constant.ts`, `chat-system-prompts.const.ts`, `GtmHomePage.tsx`, `gtm-home.types.ts`, `2-25-workspace-command-1785600000012-*` |
