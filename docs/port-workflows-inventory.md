...
- Record selection actions: workflows `action-menu` configs/hooks → `EngineComponentKey` + `ARXENA_STANDARD_COMMAND_MENU_ITEMS` + `command-menu-item/engine-command/record/arx/*`; HotTable bottom bar + All Actions → side panel (migration track §2.11). Needs workspace metadata sync for new CMIs. **Qualify Prospect** (`ARX_QUALIFY_PROSPECT`) on candidate + person stamps `outreachProspectEnrichment` via `POST /outreach-command/qualify-prospect` (cmd `1785600000114`). **Fetch LinkedIn Messages/Posts/Profile** (`ARX_FETCH_LINKEDIN_*`) via `LinkedinSelectionFetchService` (cmd `1785600000115`); company profile CMI deferred. **Fetch Contacts / Email / Phone** (`ARX_FETCH_CONTACT_DETAILS` / `ARX_FETCH_EMAIL` / `ARX_FETCH_PHONE`) + workflow LFs `enrich-contact` / `fetch-email` / `fetch-phone` + Ask AI `fetch_contacts` / `fetch_email` / `fetch_phone` (cmd `1785600000116`).
...
**Front runtime:** ARX modules must use `REACT_APP_SERVER_BASE_URL` from `~/config` (not `process.env.REACT_APP_SERVER_BASE_URL` — that becomes `/undefined/…` under Vite). Candidate-sourcing HTTP paths are dual-mounted as `get-all-projects` (+ legacy `get-all-jobs` aliases); see migration track §3. See also §2.8. Workspace GraphQL remaps for Attachment (`authorId`/`type` → `createdBy`/`fileCategory`; FILES-field upload + `target*Id` morph FKs + `getAttachmentDownloadUrl` — skill `attachment-files-field-migration`), CandidateEnrichment (`fields` → `filterFields`), record id scalars (`ID!` → `UUID!`), `FindOneProject` response key (`job` → `project`), Candidate nested relation (`projects` → `project` / `projectId`; collection query stays `projects(filter:)`), and metadata SDL (`isCustom`/`dataSourceId`/`relationDefinition` dropped) are in migration track §2.10 / §3. After `nx build twenty-shared`, restart nest so GraphQLExecutionService / workspace-modifications pick up new query strings.
