...
export class VideoInterviewController {
      ...
      ],
      {
        ...
        fileFilter: (req, file, callback) => {
          if (file.fieldname === 'audio' && file.mimetype !== 'audio/wav') {
            return callback(new BadRequestException('Only WAV files are allowed for audio'), false);
          }
          if (file.fieldname === 'video' && !file.mimetype.startsWith('video/')) {
            return callback(new BadRequestException('Only video files are allowed for video'), false);
          }
          callback(null, true);
        },
      },
  ...
  handleMultipartData(@UploadedFiles() files: { audio?: Express.Multer.File[]; video?: Express.Multer.File[] }, @Body() body: any) {
    console.log('Uploaded Audio File:', files.audio?.[0]);
    console.log('Uploaded Video File:', files.video?.[0]);
    console.log('Form Data:', body);
    ...
    return {
      message: 'Files and form data received successfully',
      audioFile: files.audio[0].filename,
